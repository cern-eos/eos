...
bool JeMallocHandler::IsJemallocLoader()
{
  ...
  if (!handle)
  {
    eos_static_err("error opening dl symbols : %s. libjemalloc is considered as NOT loaded",dlerror());
  ...
  if (dlerror() == NULL)
  {
    ...
    if (dlerror() == NULL)
    mallctl = reinterpret_cast<int (*)(const char *, void *, size_t *, void *, size_t )>(pmallctlnametomib);
    else
    isloaded = false;
  }
  dlclose(handle);
...
bool JeMallocHandler::IsProfEnabled()
{
  ...
  eos_static_err("error reading status of opt.prof : b=%d  s=%d  errc=%d",(int)b,(int)s,errc);
  return b;
}
