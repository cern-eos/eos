...
//------------------------------------------------------------------------------
//! Wrapper for copy-less C++ const strings
//------------------------------------------------------------------------------
class Slice
{
public:
...
//------------------------------------------------------------------------------
inline bool ExtractSliceFromSlice(const Slice& slice, size_t* pos,
                                  Slice* extracted)
{
  if (*pos + sizeof(size_t) > slice.size()) {
    // not enough data to read the size
    return false;
  }
  ...
  *pos += sizeof(size_t);
  ...
  if (*pos + size > slice.size()) {
    // not enough data to fit in the content
    return false;
  }
...
public:
  ...
  struct Tlogentry {
    ...
    std::string seqid;
    // Each map has a unique name which is reported for each entry in the log
    std::string writer;
    std::string key;
    std::string value;
    std::string comment;
...
std::ostream& operator << (std::ostream& os,
                           const DbMapTypes::Tlogentry& entry);
std::ostream& operator << (std::ostream& os,
                           const DbMapTypes::TlogentryVec& entryvec);

void TimeToStr(const time_t t, char* tstr);

//------------------------------------------------------------------------------
//! Interfaces
//! The following abstract classes are the interface to the db representation
//! for the DbMap and DbLog class. These classes work as a pair and should be
//! implemented as is.
//------------------------------------------------------------------------------

//------------------------------------------------------------------------------
//! This class provides an interface to implement the necessary functions for
//! the representation in a DB of the DbLog class. This should implement an
//! in-memory five columns table. Each column should contain a string (not a
//! null terminated). The names of the column are timestampstr, logid, key,
//! value, comment. There must be an uniqueness constraint on the timestamp
...
public:
  ...
  //! Destructor
  //----------------------------------------------------------------------------
  virtual ~DbLogInterface() = default;

  //----------------------------------------------------------------------------
  //! Change the file of the underlying log db. Volume splitting feature should
  //! also be provided splitting the db representation into chunks of the given
  //! duration
  //!
  //! @param[in] dbname the name of the new db file name
  //! @param[in] volumeduration the duration in seconds of each volume of the
  //!             dblog (if set to -1, no splitting is processed)
  //! @param[in] createperm the unix permission in case the file is to be created.
  //! @param[in] option a pointer to a structure holding some parameters for the
  //!            underlying db
  //!
  ...
  //!
  //! @param[out] retvec a pointer to a vector of entries to which the result of
  //!              the operation will be appended
  //! @param[in] nmax maximum number of elements to get at once
  //! @param[in,out] startafter before execution : position at which the search
  //!                 is to be started after  execution : position at which the
  //!                 search has to start at the next iteration
  //!
  //! @return the number of entries appended to the result vector retvec
  //----------------------------------------------------------------------------
  virtual size_t getAll(TlogentryVec* retvec, size_t nmax = 0,
                        Tlogentry* startafter = NULL) const = 0;

  //----------------------------------------------------------------------------
  //! Get the latest entries of the db representation
  //!
  //! @param[out] retvec a pointer to a vector of entries to which the result
  //!       of the operation will be appended
...
//------------------------------------------------------------------------------
//! Interface to implement the necessary functions for the representation in a
//! DB of the DbMap class. This should implement an in-memory five columns
//! table. Each column should contain stringthe names of the column are
//! timestampstr, logid, key, value, comment. There must be an uniqueness
//! constraint on the key (which should also be the primary key)
//------------------------------------------------------------------------------
...
public:
  ...
  //! Destructor
  //----------------------------------------------------------------------------
  virtual ~DbMapInterface() = default;
...
//----------------------------------------------------------------------------
  ...
  //! @param[in,out] startafter before execution : position at which the search
  //!                is to be started
  //!                after  execution : position at which the search has to
  //!                start at the next iteration
  //!
  //! @return the number of entries appended to the result vector retvec
  //----------------------------------------------------------------------------
  virtual size_t getAll(TlogentryVec* retvec, size_t nmax = 0,
  ...
  virtual bool attachDb(const std::string& dbname, bool repair = false,
                        int createperm = 0, void* option = NULL) = 0;
  ...
  //----------------------------------------------------------------------------
  //! Attach a log db.
  //! all operations are logged to the db. The name of the DbMap is refered as
  //! writer in the log.
  ...
  //!             dblog (if set to -1, no splitting is processed)
  //! @param[in] createperm the unix permission in case the file is to be created
  //! @param[in] option a pointer to a structure holding some parameters for
  //!            the underlying db.
  //!
  //! @return true if the dbname was attached, false otherwise (it maybe
  //! because the file is already attached to this DbMap)
//----------------------------------------------------------------------------
  ...
  //! @param[in] dblog an existing DbLogInterface. This DbLogInterface instance
  //! is not destroyed when the DbMapInterface is destroyed
  //!
  //! @return true if the dbname was attached, false otherwise (it maybe because
  //! the file is already attached to this DbMapInterface)
  //----------------------------------------------------------------------------
