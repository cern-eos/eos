...
uint64_t IoStat::cleanOldsMarks(Marks enumMark, size_t seconds) {
  if ((enumMark != Marks::READ && enumMark != Marks::WRITE)) {
    if (io::IoStatDebug) { IoStat::printInfo(std::cerr, "\033[031mNo marks found for\033[0m"); }
    return -1;
  }
  ...
  if (seconds > 0) {
    for (std::deque<IoMark>::const_iterator it = begin; it != mark.end(); it++) {
      if ((difftime(currentTime.tv_sec, it->io_time.tv_sec) < seconds)) { break; }
      end = it;
    }
  }
  // checks if in case where seconds > 0 if there is something to delete
  if (seconds > 0 && end == mark.end()) {
    if (io::IoStatDebug) { printInfo(std::cout, "\033[031mNothing to clean\033[0m"); }
    return 1;
  }
...
std::pair<double, double> IoStat::bandWidth(Marks enumMark, size_t* range, size_t seconds, double* limit) const {
  if ((enumMark != Marks::READ && enumMark != Marks::WRITE) || seconds == 0) {
    if constexpr (io::IoStatDebug) {
      if (seconds == 0) {
        ...
        IoStat::printInfo(std::cerr,
                          "\033[031mNo marks found for\033[0m" + std::to_string(static_cast<uint8_t>(enumMark)));
      }
    }
    return (std::pair(0, 0));
  }
  ...
  if (mark.empty()) { return (std::pair(0, 0)); }
  ...
  if (range) { *range = std::distance(begin, end); }
  if (begin == end) { return (std::pair(0, 0)); }
  ...
  if (limit) {
    ...
    if (std::distance(begin, end) > 0) { *limit = tmpLimit / std::distance(begin, end); }
  }
  ...
  if (std::distance(begin, end) > 0) { avrg = avrg / std::distance(begin, end); }
...
ssize_t IoStat::getSize(Marks enumMark) const {
  if (enumMark == Marks::READ) { return _readMarks.size(); }
  if (enumMark == Marks::WRITE) { return _writeMarks.size(); }
  return 0;
}
...
double IoStat::getIOPS(Marks enumMark, size_t seconds) const {
  if ((enumMark != Marks::READ && enumMark != Marks::WRITE) || seconds == 0) { return -1; }

  double iops = 0;
  auto& mark = (enumMark == Marks::READ) ? _readMarks : _writeMarks;

  if (mark.size() == 0) { return 0; }
...
std::ostream& operator<<(std::ostream& os, const IoStat& other) {
  ...
  os << std::fixed << std::setprecision(2) << C_BLUE << "[READ][avrg: " << read.first << "][std: " << read.second
     << "][s: " << rSize << "][limit: " << rLimit << "][IOPS: " << other.getIOPS(IoStat::Marks::READ) << "]";
  os << " / ";
  os << "[WRITE][avrg: " << write.first << "][std: " << write.second << "][s: " << wSize << "][limit: " << wLimit
     << "][IOPS: " << other.getIOPS(IoStat::Marks::WRITE) << "]" << C_RESET;
  return os;
}
...
std::ostream& operator<<(std::ostream& os,
                         const std::unordered_multimap<uint64_t, const std::shared_ptr<IoStat>>& other) {
  for (auto& it : other) {
...
std::ostream& operator<<(std::ostream& os, const std::optional<IoStatSummary>& opt) {
  ...
  if (other.readBandwidth.has_value()) {
    os << "[avrg: " << other.readBandwidth->first << "][std: " << other.readBandwidth->second << "][s: " << other.rSize
       << "][limit: " << other.rLimit << "][IOPS: " << other.rIops << "] / ";
  } else {
    os << "[empty] / ";
  }
  os << "[WRITE]";
  if (other.writeBandwidth.has_value()) {
    os << "[avrg: " << other.writeBandwidth->first << "][std: " << other.writeBandwidth->second
       << "][s: " << other.wSize << "][limit: " << other.wLimit << "][IOPS: " << other.wIops << "]";
  } else {
    os << "[empty]";
  }
  return os;
