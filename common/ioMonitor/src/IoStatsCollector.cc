...
std::shared_ptr<IoStatsEntry> IoStatsCollector::GetEntry(const std::string& app, uint32_t uid, uint32_t gid) {
...
void IoStatsCollector::RecordRead(const std::string& app, const uint32_t uid, const uint32_t gid, const size_t bytes) {
...
void IoStatsCollector::RecordWrite(const std::string& app, const uint32_t uid, const uint32_t gid, const size_t bytes) {
  const auto entry = GetEntry(app, uid, gid);
...
size_t IoStatsCollector::PruneStaleEntries(const int64_t max_idle_seconds) {
