...
public:
  ...
  int setTrack(size_t winTime, const T index) {
    ...
    if constexpr (io::IoAggregateMapDebug) { printInfo(std::cout, "set appName track for " + std::string(index)); }
    if (_aggregates.find(winTime) == _aggregates.end()) {
      if constexpr (io::IoAggregateMapDebug) { printInfo(std::cerr, "set appName track failed"); }
      return -1;
    }
    _aggregates[winTime]->setTrack(index);
    if constexpr (io::IoAggregateMapDebug) { printInfo(std::cout, "set appName track succeeded"); }
    return 0;
  }
  ...
  int setTrack(size_t winTime, io::TYPE type, const T index) {
    std::lock_guard<std::mutex> lock(_mutex);
    if constexpr (io::IoAggregateMapDebug) { printInfo(std::cout, "set id track for " + std::to_string(index)); }
    if (_aggregates.find(winTime) == _aggregates.end()) {
      if constexpr (io::IoAggregateMapDebug) { printInfo(std::cerr, "set id track failed"); }
      return -1;
    }
    _aggregates[winTime]->setTrack(type, index);
    if constexpr (io::IoAggregateMapDebug) { printInfo(std::cout, "set id track succeeded"); }
    return 0;
  }
  ...
  std::optional<IoStatSummary> getSummary(size_t winTime, const T index) {
    std::lock_guard<std::mutex> lock(_mutex);
    if (_aggregates.find(winTime) == _aggregates.end()) { return std::nullopt; }
