...
public:
  ...
  int setTrack(T index) {
    if (!(std::is_same_v<T, std::string> || std::is_same_v<T, const char*>)) { return -1; }
  ...
  void addSample(io::TYPE type, const T index, IoStatSummary& summary) {
    if constexpr (io::IoAggregateDebug) { printInfo(std::cout, "add sample for " + std::to_string(index)); }
    if (type == io::TYPE::UID) {
      ...
      if (static_cast<size_t>(std::distance(range.first, range.second)) >= _winTime / _intervalSec) {
        ...
        for (auto it = range.first; range.first != range.second;) {
          if (difftime(now.tv_sec, range.first->second.io_time.tv_sec) >
              difftime(now.tv_sec, it->second.io_time.tv_sec)) {
            it = range.first;
          }
          range.first++;
          if (range.first == range.second) { uid.erase(it); }
        }
      }
      uid.insert({index, summary});
      if constexpr (io::IoAggregateDebug) { printInfo(std::cout, "add uid sample succeedded"); }
    } else if (type == io::TYPE::GID) {
      ...
      if (static_cast<size_t>(std::distance(range.first, range.second)) >= _winTime / _intervalSec) {
        ...
        for (auto it = range.first; range.first != range.second;) {
          ...
          range.first++;
          if (range.first == range.second) { gid.erase(it); }
        }
      }
      gid.insert({index, summary});
      if constexpr (io::IoAggregateDebug) { printInfo(std::cout, "add gid sample succeedded"); }
    }
  }
  ...
  void addSample(const T index, IoStatSummary& summary) {
    if constexpr (io::IoAggregateDebug) { printInfo(std::cout, "add Sample for " + std::string(index)); }
    if (std::is_same_v<T, std::string> || std::is_same_v<T, const char*>) {
      ...
      if (static_cast<size_t>(std::distance(range.first, range.second)) >= _winTime / _intervalSec) {
        ...
        for (auto it = range.first; range.first != range.second;) {
          ...
          range.first++;
          if (range.first == range.second) { app.erase(it); }
        }
      }
      app.insert({std::string(index), summary});
      if constexpr (io::IoAggregateDebug) { printInfo(std::cout, "add app sample succeedded"); }
    }
  }
  ...
  std::optional<IoStatSummary> getSummary(const T index, size_t winTime) {
    ...
    if (!(std::is_same_v<T, std::string> || std::is_same_v<T, const char*>) || _apps.find(index) == _apps.end()) {
      return std::nullopt;
    }
