...
public:
  ...
  std::pair<double, double> calculeWeighted(std::map<std::pair<double, double>, size_t>& indexData,
                                            std::vector<double>& limit, double* finalLimit = NULL) const;
  ...
  //--------------------------------------------
  template <typename T>
  std::optional<std::pair<double, double>> getBandwidth(io::TYPE type, const T index, IoStat::Marks enumMark,
                                                        size_t seconds = 10, double* finalLimit = NULL) const {
    ...
    if (indexData.size() <= 0) { return std::nullopt; }
  ...
  std::optional<std::pair<double, double>> getBandwidth(const T index, IoStat::Marks enumMark, size_t seconds = 10,
    ...
    if (std::is_same_v<T, std::string> || std::is_same_v<T, const char*>) {
      ...
      if (io::IoMapDebug) { printInfo(std::cerr, "No match found for data type"); }
      return std::nullopt;
    }
    if (indexData.size() <= 0) { return std::nullopt; }
  ...
  std::optional<IoStatSummary> getSummary(io::TYPE type, const T index, size_t seconds = 10) const {
    ...
    if (io::IoMapDebug) { printInfo(std::cout, "GetSummary for " + std::to_string(index)); }

    if (type != io::TYPE::GID && type != io::TYPE::UID) { return std::nullopt; }
    ...
    if (writeData.size() <= 0 && readData.size() <= 0) { return std::nullopt; }
    if (writeData.size() <= 0) { summary.writeBandwidth = std::nullopt; }
    if (readData.size() <= 0) { summary.readBandwidth = std::nullopt; }
    ...
    if (summary.rSize > 0) { summary.rIops /= summary.rSize; }
    if (summary.wSize > 0) { summary.wIops /= summary.wSize; }

    if (io::IoMapDebug) { printInfo(std::cout, "GetSummary succeeded"); }
  ...
  std::optional<IoStatSummary> getSummary(const T index, size_t seconds = 10) const {
    ...
    if (io::IoMapDebug) { printInfo(std::cout, "GetSummary for " + std::string(index)); }
    if (seconds == 0) { return (IoStatSummary()); }
    ...
    if (rSize == 0 && wSize == 0) { return IoStatSummary(); }
