...
//!
//! When EOS_STACKTRACE_GDB is set in the environment, the handler additionally
//! obtains a gdb "thread apply all bt" of every thread. Since it must not fork
//! itself, Install() forks a dedicated crash-dumper helper process upfront and
//! the handler only writes one byte to it and waits for the answer, which are
//! both async-signal-safe. The dump is written to
//! <EOS_STACKTRACE_PATH>-<ISO8601> (default /var/eos/md/stacktrace-<ISO8601>)
//! and mirrored into the daemon log. This is opt-in because the crashing
//! daemon stays alive - and therefore unavailable - until gdb has walked all
//! its threads, which on a large MGM takes far longer than the default
//! immediate termination. That wait is deliberately unbounded: whoever enables
//! the dump wants the complete one, and a gdb killed on a deadline yields a
//! truncated trace on exactly the daemons that are big enough to need it.
//------------------------------------------------------------------------------
class CrashHandler {
public:
  //----------------------------------------------------------------------------
  //! Register the handler for SIGSEGV, SIGABRT and SIGBUS. Must be called as
  //! early as possible in the daemon startup, before it grows a heap and
  //! starts its worker threads: everything running before this call crashes
  //! unreported, and the crash dumper forked here (when EOS_STACKTRACE_GDB is
  //! set) keeps a copy-on-write twin of whatever the address space looks like
  //! at this point for the whole lifetime of the daemon.
  //!
