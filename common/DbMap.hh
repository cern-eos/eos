...
public:
	...
	virtual size_t GetAll( TlogentryVec &retvec ) const =0;
	virtual size_t GetByRegex( const std::string &regex, TlogentryVec &retvec ) const=0;
...
public:
	...
	size_t GetAll( TlogentryVec &retvec ) const;
	size_t GetByRegex( const std::string &regex, TlogentryVec &retvec ) const;
...
//! a timestamp (a double), a string representation of the timestamp, a sequence id ( an integer ).
...
private:
...
#endif
		return ts.tv_sec+(double)1e-9*ts.tv_nsec;
	}

	std::string DoubleTimeToStr(double d) const {
		char buffer[64];
		time_t sec=floor(d);
		time_t nsec=time_t((d-sec)*1e9);
		...
		if(secfract)  modp_dtoa(nsec, buffer+offset, 9);
		return buffer;
	}
	//------------------------------------------------------------------------
	...
	void DoSet(const Tkey& key, const std::string& value, const std::string& comment) {
		double d=Now();
		Tval val= {1,d,DoubleTimeToStr(d),value,comment};
		DoSet(key,val);
	}

	void DoSet(const double &time, const Tkey& key, const std::string& value, const std::string& comment) {
		double d=time;
		Tval val= {1,d,DoubleTimeToStr(d),value,comment};
		DoSet(key,val);
	}
...
public:
	...
	unsigned long Set (const Tkey& key, const std::string& value, const std::string& comment) {
		...
		if(setsequence) {
			...
			val.seqid=1;
			val.timestamp=Now();
			val.timestampstr=DoubleTimeToStr(val.timestamp);
	...
	//! this function returns the number of entries buffered in the seqlist. If means 0 if the set sequence is not enabled.
	//------------------------------------------------------------------------
	unsigned long Set( const double &time, const Tkey& key, const std::string& value, const std::string& comment) {
		mutex.LockWrite();
		if(setsequence) {
			Tval val;
			val.value=value;
...
public:
	...
	size_t GetAll( TlogentryVec &retvec ) const {
		...
		return db->GetAll(retvec);
	}
	size_t GetByRegex( const std::string &regex, TlogentryVec &retvec ) const {
		RWMutexReadLock lock(mutex);
		return db->GetByRegex(regex,retvec);
	}
