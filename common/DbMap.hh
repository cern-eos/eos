...
public:
  ...
  RegexBranch(const std::string& variable, const std::string& pattern)
  {
    ...
    if (variable == "key") {
      pContent->var = Content::var_key;
    } else if (variable == "value") {
      pContent->var = Content::var_val;
    } else if (variable == "writer") {
      pContent->var = Content::var_writer;
    } else if (variable == "seqid") {
      pContent->var = Content::var_seqid;
    } else if (variable == "comment") {
      pContent->var = Content::var_comment;
    } else if (variable == "timestampstr") {
      pContent->var = Content::var_tstampstr;
    } else {
      pContent->op = Content::op_error;
    }
  }
  ...
  bool eval(const DbMapTypes::Tlogentry& entry) const
  {
    ...
    case Content::op_expr:
      ...
      case Content::var_key:
        result = re_match(pContent->buffer, entry.key.data(), entry.key.size(), 0,
                          NULL) >= 0;
        break;

      case Content::var_val:
        result = re_match(pContent->buffer, entry.value.data(), entry.value.size(), 0,
                          NULL) >= 0;
        break;

      case Content::var_comment:
        result = re_match(pContent->buffer, entry.comment.data(), entry.comment.size(),
                          0, NULL) >= 0;
        break;

      case Content::var_seqid:
        result = re_match(pContent->buffer, entry.seqid.data(), entry.seqid.size(), 0,
                          NULL) >= 0;
        break;

      case Content::var_writer:
        result = re_match(pContent->buffer, entry.writer.data(), entry.writer.size(), 0,
                          NULL) >= 0;
        break;

      case Content::var_tstampstr:
        result = re_match(pContent->buffer, entry.timestampstr.data(),
                          entry.timestampstr.size(), 0, NULL) >= 0;
        break;
#endif
  ...
  bool hasError() const
  {
    ...
    default:
      ...
      if (pContent->left) {
        result |= pContent->left->hasError();
      }

      if (pContent->right) {
        result |= pContent->right->hasError();
      }
...
protected:
  void clear()
  {
    if (!--pContent->ptrcount) {
      if (pContent->left) {
        delete pContent->left;
      }

      if (pContent->left) {
        delete pContent->right;
      }
  ...
  struct Content {
    ...
    Content() : ptrcount(0), op(op_error), var(var_none), pattern(), buffer(NULL),
...
/*----------------------------------------------------------------------------*/
inline RegexBranch RegexAtom(const std::string& variable,
