...
SqliteInterfaceBase::execNoCallback(const char* str, int retrydelay, int nretry)
{
  ...
  if (retrydelay > 0)
    for (int retr = 0; (pRc != SQLITE_OK) && (pRc != SQLITE_DONE) &&
         (pRc != SQLITE_ROW) && retr < nretry; retr++) {
      usleep(retrydelay);
      pRc = sqlite3_exec(gDb, str, NULL, NULL, &pErrStr);
    }
...
int
SqliteInterfaceBase::execNoCallback2(const char* str, int retrydelay,
                                     int nretry)
{
  char* errstr;
  int rc;
  ...
  if (retrydelay > 0)
    for (int retr = 0; (rc != SQLITE_OK) && (rc != SQLITE_DONE) &&
         (rc != SQLITE_ROW) && retr < nretry; retr++) {
      usleep(retrydelay);
      rc = sqlite3_exec(gDb, str, NULL, NULL, &errstr);
    }
...
int
SqliteInterfaceBase::logWrapperCallback(void* isnull, int ncols, char** values,
                                        char** keys)
{
  if (ncols != 6) {
    return -1;  // given the query there should be 7 columns
  }
...
int
SqliteInterfaceBase::mapWrapperCallback(void* isnull, int ncols, char** values,
                                        char** keys)
{
  if (ncols != 6) {
    return -1;  // given the query there should be 7 columns
...
SqliteDbMapInterface::SqliteDbMapInterface():
  pGetStmt(NULL), pSetStmt(NULL), pRemoveStmt(NULL), pSizeStmt(NULL),
  pCountStmt(NULL)
{
}
...
SqliteDbMapInterface::~SqliteDbMapInterface()
{
  for (map<string, tOwnedSDLIptr>::iterator it = pAttachedDbLogs.begin();
       it != pAttachedDbLogs.end(); it = pAttachedDbLogs.begin()) {
  ...
  if (pGetStmt != NULL) {
    sqlite3_finalize(pGetStmt);
  }

  if (pSetStmt != NULL) {
    sqlite3_finalize(pSetStmt);
  }

  if (pRemoveStmt != NULL) {
    sqlite3_finalize(pRemoveStmt);
  }

  if (pSizeStmt != NULL) {
    sqlite3_finalize(pSizeStmt);
  }

  if (pCountStmt != NULL) {
    sqlite3_finalize(pCountStmt);
  }
}
...
SqliteDbMapInterface::prepareStatements()
{
  ...
  if (!pAttachedDbName.empty()) {
    ...
    sprintf(pStmt, "INSERT OR REPLACE INTO DbMap%p.dbmap VALUES(?,?,?,?,?,?);",
            this);
...
SqliteDbMapInterface::prepareExportStatement()
{
  ...
  for (vector<sqlite3_stmt*>::iterator it = pExportStmts.begin();
       it != pExportStmts.end(); it++) {
    sqlite3_finalize((*it));
  }
...
#define sqlitecheck  TestSqliteError("Compiled Statement",&pRc,&pErrStr,this);
  ...
  if (!pAttachedDbName.empty()) {
    ...
    if (pRc == SQLITE_ROW) {
      val->value = std::string((char*)sqlite3_column_blob(pGetStmt, 1),
                               (size_t)sqlite3_column_bytes(pGetStmt, 1));
...
#define sqlitecheck  TestSqliteError("Compiled Statement",&pRc,&pErrStr,this);
  ...
  if (val.seqid != 0 && !pAttachedDbName.empty()) {
    ...
    sqlitecheck;
    pRc = sqlite3_bind_text(pSetStmt, 3, val.comment.data(), val.comment.size(),
                            NULL);
    sqlitecheck;
    pRc = sqlite3_bind_text(pSetStmt, 4, val.timestampstr.data(),
                            val.timestampstr.size(), NULL);
    sqlitecheck;
    pRc = sqlite3_bind_int(pSetStmt, 5, val.seqid);
    sqlitecheck;
    pRc = sqlite3_bind_text(pSetStmt, 6, val.writer.data(), val.writer.size(),
                            NULL);
    sqlitecheck;
  ...
  if (!pAttachedDbLogs.empty()) {
    for (int i = 0; i < (int) pAttachedDbLogs.size(); i++) {
      pRc = sqlite3_bind_text(pExportStmts[i], 1, val.timestampstr.data(),
                              val.timestampstr.size(), NULL);
      sqlitecheck;
      ...
      sqlitecheck;
      pRc = sqlite3_bind_blob(pExportStmts[i], 5, val.value.data(), val.value.size(),
                              NULL);
      sqlitecheck;
      pRc = sqlite3_bind_text(pExportStmts[i], 6, val.comment.data(),
                              val.comment.size(), NULL);
      sqlitecheck;
...
SqliteDbMapInterface::attachDbLog(DbLogInterface* dblogint)
{
  ...
  if (pAttachedDbLogs.find(sname) != pAttachedDbLogs.end()) {
    pAttachedDbLogs[sname] = tOwnedSDLIptr(static_cast<SqliteDbLogInterface*>
                                           (dblogint), false);
...
bool
SqliteDbMapInterface::attachDb(const std::string& dbname, bool repair,
                               int createperm, void* option)
{
  ...
  if (pAttachedDbName.empty()) {
    ...
    sprintf(pStmt, "ATTACH \'%s\' AS DbMap%p;", pAttachedDbName.c_str(),
            this); // attached dbs for the logs are
    ...
    sprintf(pStmt,
            "CREATE TABLE IF NOT EXISTS DbMap%p.dbmap (key BLOB, value BLOB,comment TEXT, timestampstr TEXT, seqid INTEGER, writer TEXT, PRIMARY KEY(key) );",
            this);
...
bool SqliteDbMapInterface::trimDb()
{
  if (!pAttachedDbName.empty()) {
    sqlite3* trimdb;
    int rc = sqlite3_open_v2(pAttachedDbName.c_str(), &trimdb,
                             SQLITE_OPEN_READWRITE | SQLITE_OPEN_WAL, NULL);
...
size_t SqliteDbMapInterface::getAll(TlogentryVec* retvec, size_t nmax = 0,
                                    Tlogentry* startafter = NULL) const
{
  if (!pAttachedDbName.empty()) {
    ...
    if (nmax) {
      sprintf(limit, " LIMIT %lu", nmax);
    }

    if (startafter) {
      sprintf(id, " WHERE KEY>(SELECT KEY FROM DbMap%p.dbmap WHERE KEY=\"%s\")", this,
              startafter->timestampstr.c_str());
    }

    sprintf(pStmt, "SELECT * FROM DbMap%p.dbmap%s ORDER BY KEY%s;", this, id,
            limit);
    ...
    pRc = sqlite3_exec(gDb, pStmt, staticMapWrapperCallback, (void*) this,
                       &pErrStr);
  ...
  if (sqlite3_prepare_v2(gDb, pStmt , -1, &statement, 0) == SQLITE_OK) {
    ...
    while (true) {
      ...
      if (result == SQLITE_ROW) {
        ...
        Tval val;
        val.value = std::string((char*)sqlite3_column_blob(statement, 1),
...
bool
SqliteDbMapInterface::attachDbLog(const string& dbname, int volumeduration,
                                  int createperm, void* option)
{
  if (pAttachedDbLogs.find(dbname) == pAttachedDbLogs.end()) {
    pAttachedDbLogs[dbname] = tOwnedSDLIptr(new SqliteDbLogInterface(dbname,
...
SqliteDbLogInterface::SqliteDbLogInterface(const string& dbname,
    int volumeduration, int createperm, void* option)
{
  init();
...
SqliteDbLogInterface::init()
{
  ...
  if (gFile2SqName.empty()) for (int k = 0; k < 64; k++) {
      gIdPool.insert(k);  // 62 should be enough according to sqlite specs
    }
}
...
SqliteDbLogInterface::archiveThread(void* dummy)
{
  ...
  pthread_cleanup_push(SqliteDbLogInterface::archiveThreadCleanup,
...
SqliteDbLogInterface::archive(const tTimeToPeriodedFile::iterator& entry)
{
  ...
  if (sqlite3_exec(gDb, stmt, NULL, NULL, NULL) != SQLITE_OK) {
    delete[] stmt;
    delete[] archivename;
    return 1;
  }
  ...
  sprintf(stmt,
          "CREATE TABLE IF NOT EXISTS archive.ondisk (timestampstr TEXT, seqid INTEGER, writer TEXT, key BLOB, value BLOB,comment TEXT, PRIMARY KEY(timestampstr) );");
  execNoCallback2(stmt);
  TimeToStr(tt = mktime(&t2), dbuf1);
  sprintf(stmt,
          "INSERT INTO archive.ondisk SELECT * FROM %s.ondisk WHERE timestampstr<\"%s\";",
          gFile2SqName[filename].first.c_str(), dbuf1);
  execNoCallback2(stmt);
  sprintf(stmt, "DETACH archive;");
  execNoCallback2(stmt);
  sprintf(stmt, "DELETE FROM %s.ondisk WHERE timestampstr<\"%s\";",
          gFile2SqName[filename].first.c_str(), dbuf1);
  execNoCallback2(stmt);
  gTransactionMutex.UnLockWrite();
  gUniqMutex.UnLock();
...
int
SqliteDbLogInterface::updateArchiveSchedule(const tTimeToPeriodedFile::iterator&
    entry)
{
  tm t;
...
int
SqliteDbLogInterface::setArchivingPeriod(const string& dbname,
    int volumeduration)
{
  if (volumeduration > 0) {
    gArchMutex.Lock();
...
bool
SqliteDbLogInterface::setDbFile(const string& dbname, int volumeduration,
                                int createperm, void* option)
{
  Option* opt = option ? (Option*)option : &gDefaultOption;
  ...
  if (!dbname.empty()) {
    if (gFile2SqName.find(dbname) == gFile2SqName.end()) {
      ...
      if (halttransaction) {
        execNoCallback("END TRANSACTION;");
      }

      gTransactionMutex.LockWrite();
      sprintf(pStmt, "ATTACH \'%s\' AS %s_%u;", dbname.c_str(), "testattach",
              (unsigned int) pthread_self());
      execNoCallback(pStmt);

      if (pRc != SQLITE_OK) {
        if (halttransaction) {
          execNoCallback("BEGIN TRANSACTION;");
        }
  ...
  if (!this->pDbName.empty()) {
    ...
    if (csqn.second > 1) {
      ...
      gTransactionMutex.LockWrite();
      sprintf(stmt, "DETACH %s;",
              csqn.first.c_str()); // attached dbs for the logs are
      execNoCallback(stmt);
  ...
  if (!dbname.empty()) {
    if (gFile2SqName.find(dbname) != gFile2SqName.end()) {
      ...
      sprintf(stmt, "log%2.2d",
              *(gIdPool.begin())); // take the first id available in the pool
      pSqName = stmt;
      ...
      gTransactionMutex.LockWrite();
      sprintf(stmt, "ATTACH \'%s\' AS %s;", dbname.c_str(),
              pSqName.c_str());// attached dbs for the logs are
      execNoCallback(stmt);
      sprintf(stmt,
              "CREATE TABLE IF NOT EXISTS %s.ondisk (timestampstr TEXT, seqid INTEGER, writer TEXT, key BLOB, value BLOB,comment TEXT, PRIMARY KEY(timestampstr) );",
              pSqName.c_str());
      execNoCallback(stmt);
...
SqliteDbLogInterface::getTail(int nentries, TlogentryVec* retvec) const
{
  ...
  size_t count = retvec->size();
  sprintf(stmt,
          "SELECT * FROM (SELECT * FROM %s.ondisk ORDER BY timestampstr DESC LIMIT %d) ORDER BY timestampstr ASC;",
          this->GetSqName().c_str(), nentries);
  this->pRetVecPtr = retvec;
...
size_t
SqliteDbLogInterface::getAll(TlogentryVec* retvec, size_t nmax,
                             Tlogentry* startafter) const
{
  ...
  if (startafter) if (startafter->timestampstr.length()) {
      sprintf(id,
              " WHERE TIMESTAMPSTR>(SELECT TIMESTAMPSTR FROM %s.ondisk WHERE timestampstr=\"%s\")",
              this->GetSqName().c_str(), startafter->timestampstr.c_str());
    }

  sprintf(pStmt, "SELECT * FROM %s.ondisk%s ORDER BY timestampstr%s;",
          this->GetSqName().c_str(), id, limit);
  this->pRetVecPtr = retvec;
  ...
  pRc = sqlite3_exec(gDb, pStmt, staticLogWrapperCallback, (void*) this,
                     &pErrStr);
