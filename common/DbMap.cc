...
DbMapSqlite   DbMapLevelDb2Sqlite  (const DbMapLeveldb& toconvert) {
...
DbMapLeveldb  DbMapSqlite2LevelDb  (const DbMapSqlite& toconvert) {
...
bool           ConvertSqlite2LevelDb(const std::string &sqlpath, const std::string &lvdbpath, const std::string &sqlrename) {
  ...
  if(!sqdbl.IsOpen()) { // cannot read the source file
...
bool           ConvertLevelDb2Sqlite(const std::string &lvdbpath, const std::string &sqlpath, const std::string &lvdbrename) {
  ...
  if(!lvdbl.IsOpen()) { // cannot read the source file
    if(!lvdbrename.empty()) rename(lvdbrename.c_str(),sqlpath.c_str()); // revert the source renaming
    return false;
  }

  DbMapSqlite sqdbm;
  if(!sqdbm.AttachLog(sqlpath,0,st.st_mode & ~0111)) { // forget the executable mode related to directories
...
#define sqlitecheck  TestSqliteError("Compiled Statement",&rc,&errstr,this);
  ...
  rc=sqlite3_bind_int64(insert_stmt,1,val.timestamp); 					sqlitecheck;
  ...
  if(!attacheddbs.empty()) {
    for(int i=0;i<(int)attacheddbs.size();i++) {
      rc=sqlite3_bind_text(export_stmts[i],1,writer.c_str(),-1,NULL); sqlitecheck;
      rc=sqlite3_bind_text(export_stmts[i],2,key.c_str(),-1,NULL); 	sqlitecheck;
...
#define sqlitecheck  TestSqliteError("Compiled Statement",&rc,&errstr,this);
  const string &writer=val.writer.empty()?this->name:val.writer;
  rc=sqlite3_bind_int64(change_stmt,1,val.timestamp);					sqlitecheck;
...
size_t SqliteDbLogInterface::GetByRegex( const string &regex, TlogentryVec &retvec, size_t nmax, Tlogentry *startafter) const {
...
size_t SqliteDbLogInterface::GetAll( TlogentryVec &retvec , size_t nmax, Tlogentry *startafter) const {
  size_t count=retvec.size();
  char limit[1024],id[1024];
  limit[0]=id[0]=0;
  if(nmax) sprintf(limit," LIMIT %lu",nmax);
...
size_t LvDbDbLogInterface::GetByRegex( const string &sregex, TlogentryVec &retvec , size_t nmax, Tlogentry *startafter) const {
  size_t count=retvec.size();
  regex_t regex;
  ...
  if(startafter) {
    if(!startafter->timestamp.empty()) {
      ...
      string skey; skey=startafter->timestamp; skey.reserve(64);// this size fits for a number of seconds on 9 figures (300 years in the future starting from 1970) and for a precision of 1 nanosecond
      skey+=":"; skey+=startafter->writer;
      it->Seek(skey); it->Next();
    }
  }
  ...
  for (; it->Valid() && n<nmax; it->Next()) {
    ...
    if (!(ret=regexec(&regex, input_string.c_str(), 0, NULL, 0))) {
      ...
      int c=0; while(entry.timestamp[c]!=':') c++; entry.timestamp.resize(c);
...
size_t LvDbDbLogInterface::GetAll( TlogentryVec &retvec , size_t nmax, Tlogentry *startafter) const {
  size_t count=retvec.size();
  leveldb::Iterator *it = db->NewIterator(leveldb::ReadOptions());
  it->SeekToFirst();
  if(startafter) {
...
bool LvDbDbMapInterface::InsertEntry(const Tkey &key, const Tval &val) {
  ...
  string skey; skey.reserve(64); skey.resize(20,'0'); // this size fits for a number of seconds on 9 figures (300 years in the future starting from 1970) and for a precision of 1 nanosecond
  string tab("\t");
  char sseqid[24];
  ...
  skey[19]=':'; skey+=val.writer.empty()?this->name:val.writer;
