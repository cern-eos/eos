...
// Configure the view
//------------------------------------------------------------------------------
void FsHierarchicalView::configure(std::map<std::string, std::string>& config)
{
  if (!pContainerSvc)
  {
...
// Initialize the view
//------------------------------------------------------------------------------
void FsHierarchicalView::initialize()
{
  initialize1();
  initialize2();
  initialize3();
...
// Initialize1
//------------------------------------------------------------------------------
void FsHierarchicalView::initialize1()
{
  ...
  try
  {
    pRoot = pContainerSvc->getContainerMD("/");
  }
  catch (MDException& e)
  {
...
// Initialize2
//------------------------------------------------------------------------------
void FsHierarchicalView::initialize2()
{
  pFileSvc->initialize();
}
...
// Initialize3
//------------------------------------------------------------------------------
void FsHierarchicalView::initialize3()
{
  // empty
}
...
// Finalize the view
//------------------------------------------------------------------------------
void FsHierarchicalView::finalize()
{
...
//------------------------------------------------------------------------------
IFileMD* FsHierarchicalView::getFile(const std::string& uri, bool follow,
                                   size_t* link_depths)
{
...
//------------------------------------------------------------------------------
IFileMD*
FsHierarchicalView::createFile(const std::string& uri, uid_t uid, gid_t gid)
{
...
//------------------------------------------------------------------------
void FsHierarchicalView::createLink(const std::string& uri,
...
// Remove link
//------------------------------------------------------------------------------
void FsHierarchicalView::removeLink(const std::string& uri)
{
...
//------------------------------------------------------------------------------
void FsHierarchicalView::unlinkFile(const std::string& uri)
{
  char uriBuffer[uri.length() + 1];
  strcpy(uriBuffer, uri.c_str());
  std::vector<char*> elements;
...
// Remove the file
//------------------------------------------------------------------------------
void FsHierarchicalView::removeFile(IFileMD* file)
{
...
//------------------------------------------------------------------------------
IContainerMD* FsHierarchicalView::getContainer(const std::string& uri,
...
//------------------------------------------------------------------------------
IContainerMD* FsHierarchicalView::createContainer(const std::string& uri,
    bool createParents)
{
  // Split the path
  if (uri == "/")
...
//------------------------------------------------------------------------------
void FsHierarchicalView::removeContainer(const std::string& uri,
                                       bool recursive)
{
  // Find the container
  if (uri == "/")
...
//------------------------------------------------------------------------------
IContainerMD* FsHierarchicalView::findLastContainer(std::vector<char*>& elements,
...
//------------------------------------------------------------------------------
void FsHierarchicalView::cleanUpContainer(IContainerMD* cont)
{
...
//------------------------------------------------------------------------------
std::string FsHierarchicalView::getUri(const IContainerMD* container) const
{
  // Check the input
  if (!container)
  {
...
//------------------------------------------------------------------------------
std::string FsHierarchicalView::getUri(const IFileMD* file) const
{
  // Check the input
  if (!file)
  {
...
//------------------------------------------------------------------------------
IQuotaNode* FsHierarchicalView::getQuotaNode(const IContainerMD* container,
    bool search)
{
...
//------------------------------------------------------------------------------
IQuotaNode* FsHierarchicalView::registerQuotaNode(IContainerMD* container)
{
  // Initial sanity check
  if (!container)
  {
...
//------------------------------------------------------------------------------
void FsHierarchicalView::removeQuotaNode(IContainerMD* container)
{
  // Sanity checks
  if (!container)
  {
...
// Rename container
//------------------------------------------------------------------------------
void FsHierarchicalView::renameContainer(IContainerMD* container,
...
// Rename file
//------------------------------------------------------------------------------
void FsHierarchicalView::renameFile(IFileMD* file, const std::string& newName)
{
  if (!file)
  {
    MDException ex;
...
//------------------------------------------------------------------------------
void FsHierarchicalView::absPath(std::string& mypath)
{
