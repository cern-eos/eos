...
//------------------------------------------------------------------------------
//! Serialization shared by the two json sinks. They differ only in what
//! separates the records -- a newline, or a comma inside an array -- so the
//! serializing of one record lives here.
//------------------------------------------------------------------------------
class JsonSink : public OutputSink
{
public:
  //----------------------------------------------------------------------------
  //! Constructor
  //----------------------------------------------------------------------------
  JsonSink(std::ostream& out, std::ostream& err);

protected:
  //----------------------------------------------------------------------------
  //! Write line to mOut as a compact json object, no separator, no newline
  //----------------------------------------------------------------------------
  void writeRecord(const std::map<std::string, std::string>& line);
...
//------------------------------------------------------------------------------
class JsonStreamSink : public JsonSink
{
public:
  //----------------------------------------------------------------------------
  //! Constructor
  ...
  //----------------------------------------------------------------------------
  //! Destructor - closes the array and flushes
  //----------------------------------------------------------------------------
  ~JsonStreamSink();
  ...
  //----------------------------------------------------------------------------
  //! Print interface, json object implementation. Overridden so that an
  //! object printed directly still lands inside the array.
  //----------------------------------------------------------------------------
  virtual void print(const Json::Value& jsonObj) override;

private:
  //----------------------------------------------------------------------------
  //! Open the slot for the next element: the comma, then the newline
  //----------------------------------------------------------------------------
  void separate();
...
class JsonLinedStreamSink : public JsonSink
{
public:
  //----------------------------------------------------------------------------
  //! Constructor
