...
//------------------------------------------------------------------------------
// Update interval of the standalone accounting instances below. Long enough
// for the periodic propagation to never fire during a test: every propagation
// is then driven by the explicit Flush() calls, which keeps the assertions
// deterministic while still exercising the production path, i.e. both
// accounting threads running and the flush going through the queue barrier.
// The destructor does not wait for it, ThreadAssistant::wait_for returns as
// soon as the thread is asked to stop.
//------------------------------------------------------------------------------
static constexpr uint32_t sNoPeriodicPropagationSec = 3600;
...
//------------------------------------------------------------------------------
// Flush() alone must apply the queued updates, without the periodic
// propagation ever firing
//------------------------------------------------------------------------------
TEST_F(ContainerAccountingF, FlushWithoutPeriodicPropagation)
{
  ...
  eos::QuarkContainerAccounting accounting(containerSvc(), sNoPeriodicPropagationSec);
...
//------------------------------------------------------------------------------
TEST_F(ContainerAccountingF, RecordingUpdatedContIds)
{
