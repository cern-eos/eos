...
1. **Queueing thread** (`AssistedQueueForUpdate`): the single consumer of the
   ...
   operations.
2. **Propagation thread** (`AssistedPropagateUpdates`): every `mUpdateIntervalSec`
...
Both threads always run: the update interval is clamped to at least one second
at construction. A null interval used to mean "no thread, the caller drives the
propagation itself" — a mode of the in-memory to QuarkDB conversion tool, which
was removed in 2021. Without a consumer on the queue the barrier could never be
acknowledged, so that mode would have needed its own inline drain in `Flush()`
for no remaining user.
