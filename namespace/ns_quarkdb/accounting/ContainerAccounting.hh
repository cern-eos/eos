...
public:
  ...
  //!        Clamped to at least one second: the propagation is always
  //!        asynchronous, use Flush() to force the counters up to date.
  //----------------------------------------------------------------------------
  QuarkContainerAccounting(IContainerMDSvc* svc, uint32_t update_interval = 5);
...
private:
  ...
  //----------------------------------------------------------------------------
  //! Propagate updates in the hierarchical structure every update interval,
  //! until the thread is asked to stop. Method ran by the asynchronous thread.
  //!
  ...
  //----------------------------------------------------------------------------
  //! Queue containerIds and size to update in the hierarchical structure.
  //! Consumes the update queue until the stop sentinel is popped or the thread
  //! is asked to stop. Method ran by the asynchronous thread.
  //!
  //! @param assistant thread doing the queueing
  //----------------------------------------------------------------------------
