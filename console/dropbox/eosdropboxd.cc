...
int main(int argc, char* argv[])
{
  ...
  XrdOucString logfile = "/var/tmp/eosdropbox.";
  logfile += (int)getuid();
  logfile += ".log";
  ...
  syskill += (int) getuid();
  syskill += " | grep -v grep | grep -v ";
  syskill += (int) getpid();
  syskill += " | awk '{printf(\"%s \",$1)}' `";
  ...
  do {
    ...
    while ((entry = readdir(dir))) {
      ...
      if (nr < 0) {
        ...
        buffer[std::min(nr, (ssize_t)4095)] = 0;
    ...
    for (it = syncmap.begin(); it != syncmap.end(); it++) {
      ...
      // do the remote scan
      // -----------------------------------------------------------------------------------
      XrdOucString eosfind = "eos -b find -f --mtime --size ";
      eosfind += it->first.c_str();
      ...
      if (!pf) {
        ...
        while ((item = fscanf(pf, "path=%s size=%llu mtime=%llu.%llu\n", path, &size,
          ...
          eos_static_debug("[find] path=%s mtime=%llu.%llu size=%llu syncpath=%s localpath=%s",
                           path, mtime, mtime_nsec, size, endpath.c_str(), localpath.c_str());
      ...
      // do the local scan
      // -----------------------------------------------------------------------------------
      XrdOucString localfind = "find ";
      localfind += it->second.c_str();
      localfind += " -type f -printf \"path=/%P size=%s mtime=%C@\\n\"";
      ...
      for (pathit = localsizemap.begin(); pathit != localsizemap.end(); pathit++) {
        XrdOucString localpath = it->second.c_str();
        localpath += pathit->first.c_str();
        XrdOucString remotepath = it->first.c_str();
        remotepath += pathit->first.c_str();
        ...
        if (stat(localpath.c_str(), &filebuf)) {
          ...
          eos_static_err("[local] file %s cannot be stat'ed errno=%d", localpath.c_str(),
                         errno);
          continue;
        }
        ...
        if ((stat(dropboxtagfile.c_str(), &buf))) {
          ...
          if (!remotesizemap.count(pathit->first)) {
            ...
            if (remotemtimemap[pathit->first].tv_sec <
                localmtimemap[pathit->first].tv_sec) {
        ...
        if (action == "upload") {
          XrdOucString uploadline = "eos cp -n -s ";
          uploadline += localpath;
          uploadline += " ";
          ...
          if (WEXITSTATUS(rc)) {
            eos_static_err("[upload] upload %s=>%s failed!", localpath.c_str(),
                           remotepath.c_str());
          }

          // set the tag file to the mtime of the remote dropboxfile
          XrdOucString getmtime = "eos -b file info ";
          getmtime += remotepath;
          getmtime += " -m";
          ...
          int item = fscanf(mfile, "file=%*s size=%*s mtime=%llu.%llu", &newmtime,
                            &newmtime_ms);
        ...
        if (action == "download") {
          XrdOucString downloadline = "eos cp -n -s ";
          downloadline += remotepath;
          downloadline += " ";
          ...
          if (WEXITSTATUS(rc)) {
            eos_static_err("[download] download %s=>%s failed!", remotepath.c_str(),
                           localpath.c_str());
          }
        ...
        if ((stat(dropboxtagfile.c_str(), &buf))) {
          ...
          if (!localsizemap.count(pathit->first)) {
            if (stat(localpath.c_str(), &filebuf)) {
              ...
              eos_static_err("[delete] file %s cannot be stat'ed errno=%d", localpath.c_str(),
                             errno);
              continue;
            }
