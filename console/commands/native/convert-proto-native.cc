...
public:
  ...
  bool ParseCommand(const char* arg) override {
    ...
    if (token == "config") {
      ...
      if (!tokenizer.NextToken(token)) return false;
      if (token == "list") { config->set_op(ConvertProto_ConfigProto::LIST); }
      else if (token == "set") {
        if (!tokenizer.NextToken(token)) { return false; }
        size_t pos = token.find('=');
        if (pos == std::string::npos || pos == token.length() - 1) return false;
      ...
      eos::console::ConvertProto_ConversionProto* conversion = ParseConversion(tokenizer);
      if (conversion == nullptr) return false;
    ...
    } else if (token == "list") { convert->mutable_list(); }
    else if (token == "clear") { convert->mutable_clear(); }
    else { return false; }
    return true;
  }

private:
  eos::console::ConvertProto_IdentifierProto* ParseIdentifier(std::string spath) {
    XrdOucString path = spath.c_str(); auto* identifier = new eos::console::ConvertProto_IdentifierProto{}; unsigned long long id = 0ull;
    if (Path2FileDenominator(path, id)) identifier->set_fileid(id);
    else if (Path2ContainerDenominator(path, id)) identifier->set_containerid(id);
    else identifier->set_path(abspath(path.c_str()));
    return identifier;
  }

  eos::console::ConvertProto_ConversionProto* ParseConversion(eos::common::StringTokenizer& tokenizer) {
    std::string token, layout, space, placement, checksum; int replica = 0; size_t pos; bool ok;
    auto validLayout = [](const std::string& l){ return eos::common::LayoutId::GetLayoutFromString(l) != -1; };
    auto validPlacement = [](const std::string& p){ return (p=="scattered"||p=="hybrid"||p=="gathered"); };
    auto validChecksum = [](const std::string& c){ using eos::common::LayoutId; auto xs_id = LayoutId::GetChecksumFromString(c); return ((xs_id > -1) && (xs_id != LayoutId::eChecksum::kNone)); };
    if (!tokenizer.NextToken(token)) return nullptr;
    if ((pos = token.find(":")) == std::string::npos) return nullptr;
    layout = token.substr(0, pos);
    try { replica = std::stol(token.substr(pos + 1)); } catch (...) { return nullptr; }
    if (!validLayout(layout)) return nullptr;
    if (replica < 1 || replica > 32) return nullptr;
    while (tokenizer.NextToken(token)) {
      if ((ok = validChecksum(token))) checksum = std::move(token);
      else if ((ok = validPlacement(token))) placement = std::move(token);
      else if ((ok = space.empty())) space = std::move(token);
      if (!ok) return nullptr;
    }
    auto* conversion = new eos::console::ConvertProto_ConversionProto{};
    conversion->set_layout(layout); conversion->set_replica(replica); conversion->set_space(space); conversion->set_placement(placement); conversion->set_checksum(checksum);
    return conversion;
  }
};
