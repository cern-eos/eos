...
public:
  const char* name() const override { return "fileinfo"; }
  ...
  int run(const std::vector<std::string>& args, CommandContext& ctx) override {
    ...
    if (!fileCmd) { fprintf(stderr, "error: 'file' command not available\n"); return -1; }
    std::vector<std::string> forwarded; forwarded.reserve(args.size() + 1);
...
void RegisterFileInfoAliasCommand()
{
