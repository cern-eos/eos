...
public:
  ...
  int run(const std::vector<std::string>& args, CommandContext&) override {
    ...
    if ((option.length()) && ((option != "f") && (option != "d"))) { fprintf(stderr, "error: unknown option \"%s\"\n", option.c_str()); global_retc = EINVAL; return 0; }
    path = abspath(path.c_str()); XrdOucString url = serveruri.c_str(); url += "/"; url += path;
    ...
    if (!XrdPosixXrootd::Stat(url.c_str(), &buf)) {
      ...
      if (S_ISDIR(buf.st_mode)) { fprintf(stdout, " directory\n"); }
      ...
      } else { fprintf(stdout, " symbolic link\n"); }
      global_retc = 0;
    } else { fprintf(stderr, "error: failed to stat %s\n", path.c_str()); global_retc = EFAULT; return 0; }
    return 0;
  }
...
void RegisterStatNativeCommand()
{
