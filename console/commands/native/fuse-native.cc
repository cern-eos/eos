...
public:
  ...
  int run(const std::vector<std::string>& args, CommandContext&) override {
    ...
    params += (url.GetHostName() == "localhost") ? "localhost.localdomain" : url.GetHostName().c_str();
    params += ":"; params += url.GetPath().c_str();
    if (cmd == "mount") {
      if (!mountpoint.length()) { printHelp(); global_retc = EINVAL; return 0; }
      struct stat buf{};
      if (stat(mountpoint.c_str(), &buf)) {
        XrdOucString createdir = "mkdir -p "; createdir += mountpoint; createdir += " >& /dev/null";
        fprintf(stderr, ".... trying to create ... %s\n", mountpoint.c_str());
        int rc = system(createdir.c_str()); if (WEXITSTATUS(rc)) fprintf(stderr, "error: creation of mountpoint failed");
      }
      if (stat(mountpoint.c_str(), &buf)) { fprintf(stderr, "error: cannot create mountpoint %s !\n", mountpoint.c_str()); return -1; }
#ifdef __APPLE__
      ...
      XrdOucString mount; mount = "eosxd "; mount += mountpoint.c_str(); mount += " -o"; mount += params; mount += " >& /dev/null";
      int rc = system(mount.c_str()); if (WEXITSTATUS(rc)) { fprintf(stderr, "error: failed mount\n"); return -1; }
      fprintf(stderr, "info: successfully mounted EOS [%s] under %s\n", serveruri.c_str(), mountpoint.c_str());
    } else {
      if (!mountpoint.length()) { printHelp(); global_retc = EINVAL; return 0; }
#ifdef __APPLE__
      XrdOucString umount = "umount -f "; umount += mountpoint.c_str(); umount += " >& /dev/null";
#else
      XrdOucString umount = "fusermount -z -u "; umount += mountpoint.c_str();
#endif
      int rc = system(umount.c_str()); if (WEXITSTATUS(rc)) fprintf(stderr, "error: umount failed\n");
    }
    return 0;
  }
...
void RegisterFuseNativeCommand()
{
