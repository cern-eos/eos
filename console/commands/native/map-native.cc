...
public:
  ...
  const char* description() const override { return "Mapping utilities"; }
  ...
  int run(const std::vector<std::string>& args, CommandContext& ctx) override {
    ...
    if (!t) { printHelp(); global_retc = EINVAL; return 0; }
    ...
    if (subcommand.beginswith("-")) {
      XrdOucString option = subcommand; option.erase(0,1);
    ...
    if (!subcommand.length() || ((subcommand != "ls") && (subcommand != "link") && (subcommand != "unlink"))) {
    ...
    if (subcommand == "ls") {
      ...
      if (!key.length() || !value.length()) { printHelp(); global_retc = EINVAL; return 0; }
      in += "&mgm.subcmd=link&mgm.map.src="; in += key;
      in += "&mgm.map.dest="; in += value;
    } else if (subcommand == "unlink") {
      XrdOucString key = arg;
      if (!key.length()) { printHelp(); global_retc = EINVAL; return 0; }
