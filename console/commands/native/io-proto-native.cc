  ...
  bool ParseCommand(const char* arg) override {
    ...
    if (token == "stat") {
      ...
      while (tokenizer.NextToken(token)) {
        if (token == "-a") stat->set_details(true);
        else if (token == "-m") stat->set_monitoring(true);
        else if (token == "-n") stat->set_numerical(true);
        else if (token == "-t") stat->set_top(true);
        else if (token == "-d") stat->set_domain(true);
        else if (token == "-x") stat->set_apps(true);
        else if (token == "--ss") stat->set_sample_stat(true);
        else if (token == "--sa") { if (!tokenizer.NextToken(token)) return false; if (token.find_first_not_of("0123456789")!=std::string::npos) return false; stat->set_time_ago(std::stoull(token)); }
        else if (token == "--si") { if (!tokenizer.NextToken(token)) return false; if (token.find_first_not_of("0123456789")!=std::string::npos) return false; stat->set_time_interval(std::stoull(token)); }
        else if (token == "-l") stat->set_summary(true);
        else return false;
      }
    } else if (token == "ns") {
      eos::console::IoProto_NsProto* ns = io->mutable_ns();
      while (tokenizer.NextToken(token)) {
        if (token == "-m") ns->set_monitoring(true);
        else if (token == "-b") ns->set_rank_by_byte(true);
        else if (token == "-n") ns->set_rank_by_access(true);
        else if (token == "-w") ns->set_last_week(true);
        else if (token == "-f") ns->set_hotfiles(true);
        else if (token == "-100") ns->set_count(eos::console::IoProto_NsProto::ONEHUNDRED);
        else if (token == "-1000") ns->set_count(eos::console::IoProto_NsProto::ONETHOUSAND);
        else if (token == "-10000") ns->set_count(eos::console::IoProto_NsProto::TENTHOUSAND);
        else if (token == "-a") ns->set_count(eos::console::IoProto_NsProto::ALL);
        else return false;
      }
    } else if (token == "report") {
      if (!tokenizer.NextToken(token)) return false; eos::console::IoProto_ReportProto* report = io->mutable_report(); report->set_path(token);
    } else if (token == "enable" || token == "disable") {
      eos::console::IoProto_EnableProto* enable = io->mutable_enable(); enable->set_switchx(token == "enable");
      while (tokenizer.NextToken(token)) {
        if (token == "-r") enable->set_reports(true);
        else if (token == "-p") enable->set_popularity(true);
        else if (token == "-n") enable->set_namespacex(true);
        else if (token == "--udp") { if (!tokenizer.NextToken(token) || token.find('-')==0) return false; enable->set_upd_address(token); }
        else return false;
      }
    } else { return false; }
    return true;
