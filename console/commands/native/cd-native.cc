...
public:
  ...
  int run(const std::vector<std::string>& args, CommandContext& ctx) override {
    if (!args.empty() && wants_help(args[0].c_str())) {
      fprintf(stdout, "usage: cd <path> | cd - | cd ~\n");
    ...
    XrdOucString arg = args.empty() ? XrdOucString("") : XrdOucString(args[0].c_str());
    ...
    global_retc = ctx.outputResult(ctx.clientCommand(lsminuss, false, nullptr), true);
    if (global_retc) {
      gPwd = oldpwd;
    }
    return 0;
