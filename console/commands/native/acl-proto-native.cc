...
public:
  const char* name() const override { return "acl"; }
  const char* description() const override { return "Acl Interface"; }
  ...
  int run(const std::vector<std::string>& args, CommandContext& ctx) override {
    ...
    for (size_t i = 0; i < args.size(); ++i) {
      ...
      if (t == "-l" || t == "--list") { list = true; continue; }
      if (t == "-R" || t == "--recursive") { recursive = true; continue; }
      if (t == "--user") { userAcl = true; continue; }
      if (t == "--sys") { sysAcl = true; continue; }
      // ignore position/front for now (ordering handled server-side)
      if (t == "-p" || t == "--position") { if (i + 1 < args.size()) { ++i; } continue; }
      if (t == "-f" || t == "--front") { continue; }
    ...
    // Set rule
    if (positionals.size() < 2) { printHelp(); global_retc = EINVAL; return 0; }
    // Interpret last positional as identifier, prior ones as rule segments (join with space to preserve quoted input)
    ...
    if (recursive) attrArgs.push_back("-r");
  ...
  void printHelp() const override {
    fprintf(stdout,
            "Usage: eos acl [-l|--list] [-R|--recursive] [-p|--position <pos>] [-f|--front] [--sys|--user] [<rule>] <identifier>\n"
...
void RegisterAclProtoNativeCommand()
{
