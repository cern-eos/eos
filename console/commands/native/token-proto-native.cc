...
std::string MakeTokenHelp()
{
  ...
  oss << "Usage: token --token  <token> | --path <path> --expires <expires> "
      ...
      << "             --scope <scope>               : restrict gRPC command usage, e.g. "
         "grpc.exec.quota.get; multiple scope parameters can be provided\n"
      ...
      << "          eos token --path /eos/project/ --permission rx --tree --scope "
         "grpc.exec.quota.get\n"
      << "                                           : token limited to read-only gRPC "
         "quota lookups under /eos/project/\n"
