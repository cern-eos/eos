...
public:
  ...
  const char* description() const override { return "Accounting tools"; }
  ...
  int run(const std::vector<std::string>& args, CommandContext& ctx) override {
    ...
    if (sub == "report") {
      ...
      ConsoleArgParser p; p.addOption({"", 'f', false, false, "", "force synchronous report", ""});
      std::vector<std::string> rest(args.begin()+1, args.end()); auto r = p.parse(rest);
      if (r.has("f")) { in += "&mgm.option=f"; }
    } else if (sub == "config") {
      ...
      for (size_t i=0;i<rest.size();) {
        ...
        if (tok == "-e" || tok == "-i") {
          if (i+1 >= rest.size()) { printHelp(); global_retc = EINVAL; return 0; }
          const std::string& val = rest[i+1];
          if (tok == "-e") in += "&mgm.accounting.expired="; else in += "&mgm.accounting.invalid=";
          in += val.c_str(); i += 2;
...
void RegisterAccountingNativeCommand()
{
