...
public:
  ...
  int run(const std::vector<std::string>& args, CommandContext&) override {
    ...
    class LocalHelper : public ICmdHelper { public: using ICmdHelper::ICmdHelper; bool ParseCommand(const char* arg) override { eos::console::DebugProto* debugproto = mReq.mutable_debug(); eos::common::StringTokenizer tokenizer(arg); tokenizer.GetLine(); std::string token; if (!tokenizer.NextToken(token)) { return false; } if (token == "get") { auto* get = debugproto->mutable_get(); get->set_placeholder(true); } else if (token == "this") { global_debug = !global_debug; gGlobalOpts.mDebug = global_debug; fprintf(stdout, "info: toggling shell debugmode to debug=%d\n", global_debug); mIsLocal = true; } else { auto* set = debugproto->mutable_set(); set->set_debuglevel(token); if (tokenizer.NextToken(token)) { if (token == "--filter") { if (!tokenizer.NextToken(token)) return false; set->set_filter(token); } else { set->set_nodename(token); if (tokenizer.NextToken(token)) { if (token != "--filter") return false; else { if (!tokenizer.NextToken(token)) return false; set->set_filter(token); } } } } } return true; } };
