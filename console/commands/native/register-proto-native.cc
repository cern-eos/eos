...
public:
  ...
  int run(const std::vector<std::string>& args, CommandContext& ctx) override {
    ...
    // Build mgm request from tags (uid/gid/size/path/xattr/ctime/mtime/atime/atimeifnewer/mode/location/layoutid/checksum)
    XrdOucString in = "mgm.cmd=register";
    bool update=false; std::vector<std::string> tags;
    for (const auto& a: args) { if (a == "-u") { update = true; continue; } tags.push_back(a); }
    if (update) in += "&mgm.update=1";
    for (const auto& t: tags) { size_t eq = t.find('='); if (eq == std::string::npos) { in += "&mgm.path="; in += abspath(t.c_str()); continue; } std::string k=t.substr(0,eq), v=t.substr(eq+1);
      if (k=="uid"||k=="username") { in += "&mgm.owner."; in += k.c_str(); in += "="; in += v.c_str(); }
      else if (k=="gid"||k=="groupname") { in += "&mgm.owner."; in += k.c_str(); in += "="; in += v.c_str(); }
      else if (k=="size"||k=="mode"||k=="layoutid"||k=="checksum") { in += "&mgm."; in += k.c_str(); in += "="; in += v.c_str(); }
      else if (k=="location") { in += "&mgm.location="; in += v.c_str(); }
      else if (k=="path") { in += "&mgm.path="; in += abspath(v.c_str()); }
      else if (k=="xattr") { in += "&mgm.xattr="; in += v.c_str(); }
      else if (k=="ctime"||k=="mtime"||k=="btime"||k=="atime") { in += "&mgm."; in += k.c_str(); in += "="; in += v.c_str(); }
      else if (k=="atimeifnewer") { in += "&mgm.atime="; in += v.c_str(); in += "&mgm.atimeifnewer=1"; }
      else { /* ignore unknown */ }
    }
