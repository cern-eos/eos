...
com_daemon_usage:
  fprintf(
      ...
      "[name] [subcmd]                                     :  \n");
  fprintf(stdout, "                <service> := mq | mgm | fst | qdb\n");
  ...
  fprintf(stdout, "\n");
  fprintf(stdout, "      examples: eos daemon config qdb qdb coup              "
                  "          -  try to make instance [qdb] a leader of QDB\n");
  fprintf(stdout, "                eos daemon config qdb qdb info              "
                  "          -  show raft-info for the [qdb] QDB instance\n");
  fprintf(stdout, "                eos daemon config qdb qdb remove host:port  "
                  "          -  remove a member of the qdb cluster\n");
  fprintf(stdout, "                eos daemon config qdb qdb add host:port     "
                  "          -  add an observer to the qdb cluster\n");
  fprintf(stdout,
          "                eos daemon config qdb qdb promote host:port         "
          "  -  promote an observer to a full member of the qdb cluster\n");
  fprintf(stdout, "                eos daemon config qdb qdb new observer      "
                  "          -  create a new observer\n");
  fprintf(stdout, "                eos daemon config fst fst.1                 "
                  "          -  show the init,sysconfig and xrootd config for "
                  "the [fst.1] FST service\n");
  fprintf(stdout, "                eos daemon kill mq                          "
                  "          -  shoot the MQ service with signal -9\n");
  fprintf(stdout,
          "                eos daemon stop mq                                  "
          "  -  gracefully shut down the MQ service with signal -15\n");
  fprintf(stdout, "                eos daemon stack mgm                        "
                  "          -  take an 'eu-stack' of the MGM service\n");
  fprintf(stdout, "                eos daemon run fst fst.1                    "
