...
public:
  ...
  const char* description() const override { return "Member management"; }
  ...
  int run(const std::vector<std::string>& args, CommandContext& ctx) override {
    ...
    do {
      option = tok.GetToken();
      if (!option || !strlen(option)) break;
      std::string soption = option;
      if (soption == "--help" || soption == "-h") { printHelp(); global_retc = 0; return 0; }
      if (soption == "--update") { update = true; continue; }
      if (egroup.empty()) { egroup = soption; } else { fprintf(stderr, "error: command accepts only one egroup argument\n"); global_retc = EINVAL; return 0; }
    } while (option && strlen(option));

    if (egroup.empty()) { fprintf(stderr, "error: no egroup argument given\n"); global_retc = EINVAL; return 0; }

    XrdOucString in = "mgm.cmd=member";
    in += "&mgm.egroup="; in += egroup.c_str();
    if (update) { in += "&mgm.egroupupdate=true"; }
...
void RegisterMemberNativeCommand()
{
