      ...
      while (tokenizer.NextToken(token)) {
        if (token == "-s") {
          ...
          if (!tokenizer.NextToken(token) || !eos::common::StringTokenizer::IsUnsignedNumber(token)) {
            std::cerr << "error: geodepth invalid" << std::endl; return false;
          }
          try { ls->set_outdepth(std::stoi(token)); } catch (...) { std::cerr << "error: geodepth must be integer" << std::endl; return false; }
