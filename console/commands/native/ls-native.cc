...
public:
  ...
  int run(const std::vector<std::string>& args, CommandContext& ctx) override {
    ...
    p.addOption({"", 'y', false, false, "", "show long listing with backend(tape) status", ""});
    ...
    p.addOption({"", 'c', false, false, "", "add checksum value (implies -l)", ""});
    p.addOption({"", 'n', false, false, "", "show numerical user/group ids", ""});
    p.addOption({"", 'F', false, false, "", "append indicator '/' to directories", ""});
    p.addOption({"", 's', false, false, "", "checks only if the directory exists without listing", ""});
    p.addOption({"no-globbing", 'N', false, false, "", "disables globbing", ""});
    ...
    std::string option;
    auto add_flag = [&](char c){ option += '-'; option.push_back(c); };
    if (r.has("l")) add_flag('l');
    if (r.has("y")) add_flag('y');
    if (r.has("a")) add_flag('a');
    if (r.has("i")) add_flag('i');
    if (r.has("c")) { add_flag('c'); if (!r.has("l")) add_flag('l'); }
    if (r.has("n")) add_flag('n');
    if (r.has("F")) add_flag('F');
    if (r.has("s")) add_flag('s');
    if (r.has("no-globbing") || r.has("N")) { option += "-N"; }
    ...
    if (path.empty()) path = gPwd.c_str();

    // Unescape blanks (legacy behaviour: replace "\\ " -> " ")
    {
      std::string replaced; replaced.reserve(path.size());
      for (size_t i = 0; i < path.size(); ++i) {
        if (path[i] == '\\' && i+1 < path.size() && path[i+1] == ' ') { replaced.push_back(' '); ++i; }
        else replaced.push_back(path[i]);
      }
      path.swap(replaced);
    }
    ...
    if (xpath.beginswith("as3:")) {
      XrdOucString hostport; XrdOucString protocol; XrdOucString sPath; const char* v = 0;
      if (!(v = eos::common::StringConversion::ParseUrl(xpath.c_str(), protocol, hostport))) {
        fprintf(stderr, "error: illegal url <%s>\n", xpath.c_str());
        global_retc = EINVAL; return 0;
      }
      sPath = v;
      if (hostport.length()) setenv("S3_HOSTNAME", hostport.c_str(), 1);

      XrdOucString envString = xpath; int qpos = 0;
      if ((qpos = envString.find("?")) != STR_NPOS) {
        envString.erase(0, qpos + 1); XrdOucEnv env(envString.c_str());
        if (env.Get("s3.key")) setenv("S3_SECRET_ACCESS_KEY", env.Get("s3.key"), 1);
        if (env.Get("s3.id")) setenv("S3_ACCESS_KEY_ID", env.Get("s3.id"), 1);
        xpath.erase(xpath.find("?")); sPath.erase(sPath.find("?"));
      }
      ...
      if (!getenv("S3_ACCESS_KEY_ID") || !getenv("S3_HOSTNAME") || !getenv("S3_SECRET_ACCESS_KEY")) {
      ...
      XrdOucString s3arg = sPath.c_str(); XrdOucString listcmd = "bash -c \""; listcmd += s3env; listcmd += " s3 list "; listcmd += s3arg; listcmd += " "; listcmd += "\"";
    ...
    if (xpath.beginswith("file:") || xpath.beginswith("root:")) {
      ...
      XrdOucString protocol; XrdOucString hostport; XrdOucString sPath; const char* v = 0;
      if (!(v = eos::common::StringConversion::ParseUrl(xpath.c_str(), protocol, hostport))) {
        global_retc = EINVAL; return 0;
      }
      sPath = v; std::string Path = v;
      if (sPath == "" && (protocol == "file")) {
        sPath = getenv("PWD"); Path = getenv("PWD");
        if (!sPath.endswith("/")) { sPath += "/"; Path += "/"; }
      }
      XrdOucString url = "";
      eos::common::StringConversion::CreateUrl(protocol.c_str(), hostport.c_str(), Path.c_str(), url);
      DIR* dir = (isXrd) ? XrdPosixXrootd::Opendir(url.c_str()) : opendir(url.c_str());
      if (dir) {
        struct dirent* entry;
        while ((entry = (isXrd) ? XrdPosixXrootd::Readdir(dir) : readdir(dir))) {
          struct stat buf; XrdOucString curl = ""; XrdOucString cpath = Path.c_str(); cpath += entry->d_name;
          eos::common::StringConversion::CreateUrl(protocol.c_str(), hostport.c_str(), cpath.c_str(), curl);
          if (option.find("a") == std::string::npos) { if (entry->d_name[0] == '.') continue; }
          if (!((isXrd) ? XrdPosixXrootd::Stat(curl.c_str(), &buf) : stat(curl.c_str(), &buf))) {
            if (option.find("l") == std::string::npos) {
              fprintf(stdout, "%s\n", entry->d_name);
            } else {
              char t_creat[14]; char modestr[11]; eos::modeToBuffer(buf.st_mode, modestr);
              XrdOucString suid = ""; suid += (int)buf.st_uid; XrdOucString sgid = ""; sgid += (int)buf.st_gid; XrdOucString sizestring = "";
              struct tm* t_tm; struct tm t_tm_local; t_tm = localtime_r(&buf.st_ctime, &t_tm_local); strftime(t_creat, 13, "%b %d %H:%M", t_tm);
              XrdOucString dirmarker = ""; if (option.find("F") != std::string::npos) dirmarker = "/"; if (modestr[0] != 'd') dirmarker = "";
              fprintf(stdout, "%s %3d %-8.8s %-8.8s %12s %s %s%s\n", modestr,
                      (int)buf.st_nlink,
                      suid.c_str(), sgid.c_str(),
                      eos::common::StringConversion::GetSizeString(sizestring, (unsigned long long)buf.st_size),
    ...
    XrdOucString esc = eos::common::StringConversion::curl_escaped(ap.c_str()).c_str();
    XrdOucString in = "mgm.cmd=ls";
    in += "&mgm.path="; in += esc;
    in += "&eos.encodepath=1";
    in += "&mgm.option="; in += option.c_str();
    global_retc = ctx.outputResult(ctx.clientCommand(in, false, nullptr), true);
    return 0;
  }

  void printHelp() const override {
    fprintf(stdout, "usage: ls [-laniyFN] [--no-globbing] <path>\n");
