...
std::string MakeFindHelp()
{
  ...
  oss << " usage\n"
      ...
      << "\t     --name <pattern> : find by name, filtering by 'egrep' style regex "
         "match\n"
      ...
      << "\t -flag <n>,-nflag <n> : find entries with / without specified UNIX access "
         "flag, e.g. 755\n"
      << "\t   --ctime <+n>, <-n> : find files with ctime older (+n) or younger (-n) "
         "than "
         "<n> days\n"
      << "\t   --mtime <+n>, <-n> : find files with mtime older (+n) or younger (-n) "
         "than "
         "<n> days\n"
      ...
      << "\t  --layoutstripes <n> : apply new layout with <n> stripes to the files "
         "found\n"
