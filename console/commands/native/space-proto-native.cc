...
std::string MakeSpaceHelp()
{
  ...
  oss << " usage:\n"
      << "space ls [-s|-g <depth>] [-m|-l|--io|--fsck] [<space>] : list in all spaces or "
         "select only <space>. <space> is a substring match and can be a comma separated "
         "list\n"
      << "\t      -s : silent mode\n"
      << "\t      -m : monitoring key=value output format\n"
      << "\t      -l : long output - list also file systems after each space\n"
      << "\t      -g : geo output - aggregate space information along the instance "
      ...
      << "space config <space-name> space.attr.<key> =[<>|]<value>               : "
         "configure a space extended attribute which is added to all directories "
         "referencing this space via sys.forced.space\n"
      << "                                                                        "
         "space.attr.sys.acl=<u:1000:rwx (the < sign indicates to add to the acl on the "
         "left side\n"
      << "                                                                        "
         "space.attr.sys.acl=>u:1000:rwx (the > sign indicates to add to the acl on the "
         "right side\n"
      << "                                                                        "
         "space.attr.sys.acl=|u:1000:rwx (the | sign indicates to set the acl if there "
         "is none defined\n"
      << "                                                                        "
         "space.attr.sys.foo=bar ( the sys.foo attribute is overwriting the local "
         "sys.foo attribute\n"
      << "                                                                        "
         "space.attr.sys.foo=|bar ( the sys.foo attribute is set only if there is no "
      ...
      << "space config <space-name> space.balancer.node.rate=<MB/s>             : "
         "configure the nominal transfer bandwidth per running transfer on a node [ "
         "default=25 (MB/s)   ]\n"
      ...
      << "space config <space-name> space.balancer.max-queue-jobs=<#>           : "
         "configure the maximum number of queued jobs allowed in the balancer thread "
         "pool [ default=1000 (jobs) ]\n"
      << "space config <space-name> space.balancer.max-thread-pool-size=<#>     : "
         "configure the maximum number of threads to be used in the balancer thread pool "
         "[ default=100 (threads) ]\n"
      << "space config <space-name> space.balancer.update.interval=<#>          : "
         "configure the update interval of the balancing statistics used for spawning "
         "transfers [ default=60 (seconds) min=1 max=300]\n"
      << "space config <space-name> space.drainer.tx.minrate=<MB/s >            : "
         "configure the minimum transfer bandwidth per running transfer used for "
      ...
      << "space config <space-name> space.groupbalancer.ntx=<ntx>               : "
         "configure the number of parallel group balancer jobs per 10s [ default=10 ]\n"
      << "space config <space-name> space.groupbalancer.engine=[value]          : "
         "configure the groupbalancer engine - std/minmax/freespace [ default=std ]\n"
      << "space config <space-name> space.groupbalancer.min_threshold=<v>       : "
         "configure the groupbalancer min threshold(%), groups below this will be picked "
         "as targets [default=60]\n"
      << "space config <space-name> space.groupbalancer.max_threshold=<v>       : "
         "configure the groupbalancer max threshold(%), groups above this will be picked "
         "as sources [default=95]\n"
      << "space config <space-name> space.groupbalancer.min_file_size=<#K/M/G/T>: "
         "configure the min file size to move between groups [ default=1G ]\n"
      << "space config <space-name> space.groupbalancer.max_file_size=<#K/M/G/T>: "
         "configure the max file size to move between groups [ default=16G ]\n"
      << "space config <space-name> space.groupbalancer.file_attempts=<#>       : "
         "configure the no of attempts to find a file within sizes [ default=50 ]\n"
      << "space config <space-name> space.groupbalancer.threshold=<threshold>   : "
         "[Deprecated use <..>.min/max_threshold (see above)] configure the threshold "
         "when a group is balanced\n"
      << "space config <space-name> space.groupbalancer.blocklist=<list>        : comma "
         "list eg. group1, group2 of groups blocklisted (only available for freespace "
         "engine)\n"
      ...
      << "space config <space-name> space.groupdrainer.group_refresh_interval   : "
         "configure time in seconds for refreshing cached groups info [default=300]\n"
      << "space config <space-name> space.groupdrainer.retry_interval           : "
         "configure time in seconds for retrying failed drains [default=4*3600]\n"
      ...
      << "space config <space-name> fs.max.wopen=<n>                            : allow "
         "more than <n> write streams per disk in the given space\n"
      ...
      << "space config <space-name> space.headroom=<size>                       : "
         "configure the default disk headroom if not defined on a filesystem (see fs for "
         "details)\n"
      << "space config <space-name> space.scaninterval=<sec>                    : "
         "configure the default scan interval if not defined on a filesystem (see fs for "
         "details)\n"
      << "space config <space-name> space.scan_rain_interval=<sec>              : "
         "configure the default rain scan interval if not defined on a filesystem (see "
         "fs for details)\n"
      << "space config <space-name> space.scanrate=<MB/S>                       : "
         "configure the default scan rate if not defined on a filesystem     (see fs for "
         "details)\n"
      << "space config <space-name> space.scan_disk_interva=<sec>               : time "
         "interval after which the disk scanner will run, default 4h\n"
      << "space config <space-name> space.scan_ns_interval=<sec>                : time "
         "interval after which the namespace scanner will run, default 3 days\n"
      << "space config <space-name> space.scan_ns_rate=entry/sec                : "
         "namespace scan rate in terms of number of stat requests per second done "
         "against the local disk\n"
      << "space config <space-name> space.scheduler.type=<type>                 : "
         "which placement engine the space uses. 'geotree' (alias 'legacy') is "
         "the legacy engine and the default for a space that never set this "
         "key. Everything else is a flat scheduler strategy: 'geoscheduler' "
         "(alias 'geo') for the geo-aware descent - note this is NOT the "
         "geotree engine - plus 'roundrobin', 'threadlocalroundrobin' "
         "(alias 'tlrr'), 'random', 'fidrandom', 'weightedrandom' and "
         "'weightedroundrobin' (alias 'weightedrr'). An unrecognised value "
         "falls back to 'geotree'; check with 'sched show type <space>'\n"
      << "space config <space-name> space.scheduler.fillratiolimit=<percent>    : "
         "fill level at which a disk stops taking new placements "
         "(flat scheduler only, default 95)\n"
      << "space config <space-name> space.scheduler.fillratiowarn=<percent>     : "
         "fill level at which a disk's placement weight starts to decay, must stay "
         "below fillratiolimit (flat scheduler only, default 80)\n"
      << "space config <space-name> space.drainperiod=<sec>                     : "
         "configure the default drain  period if not defined on a filesystem (see fs for "
         "details)\n"
      << "space config <space-name> space.graceperiod=<sec>                     : "
         "configure the default grace  period if not defined on a filesystem (see fs for "
         "details)\n"
      ...
      << "space config <space-name> space.inspector.interval=<sec>              : time "
         "interval after which the inspector will run, default 4h\n"
      << "space config <space-name> space.inspector.price.currency=[0-5]        : "
         "currency printed by the cost evaluation ( 0=EOS, 1=CHF, 2=EUR, 3=USD, 4=AUD, "
         "5=YEN )\n"
      << "space config <space-name> space.inspector.price.disk.tbyear=<price>   : set "
         "the price of a tb year of data on disk without redundancy (default=20)\n"
      << "space config <space-name> space.inspector.price.tape.tbyear=<price>   : set "
         "the price of a tb year of data on disk without redundancy (default=10)\n"
      << "space config <space-name> space.geo.access.policy.write.exact=on|off  : if "
         "'on' use exact matching geo replica (if available), 'off' uses weighting [ for "
         "write case ]\n"
      << "space config <space-name> space.geo.access.policy.read.exact=on|off   : if "
         "'on' use exact matching geo replica (if available), 'off' uses weighting [ for "
         "read  case ]\n"
      << "space config <space-name> fs.<key>=<value>                            : "
         "configure file system parameters for each filesystem in this space (see help "
         "of 'fs config' for details)\n"
      << "space config <space-name> "
         "space.policy.[layout|nstripes|checksum|blockchecksum|blocksize|bw|schedule|"
         "iopriority|iotype]=<value>      \n"
      << "                                                                      : "
         "configure default file layout creation settings as a space policy - a "
         "value='remove' deletes the space policy\n"
      << std::endl
      << "space config <space-name> space.altxs=on|off                          : "
         "enable/disable the alternative checksums computation when the file is "
         "uploaded\n"
      << "space config <space-name> space.groupsize=<#>                         : "
         "configure how many filesystems can end up in one scheduling group\n"
      << "space config <space-name> space.groupmod=<#>                          : "
         "configure the maximum number of groups in the space\n"
      << "space config <space-name> space.quota=on|off                          : "
         "enable/disable quota for this space\n"
      << std::endl
      << "Read-through cache parameters (configure on the backend space, typically "
         "default):\n"
      << "space config <space-name> space.cachespace=<cache-space>              : bind a "
         "cache space used for read-through caching [ default=none ]\n"
      << "  (directory override: sys.forced.cachespace=<cache-space>|none on the parent "
         "dir)\n"
      << "                                                                        => "
         "value 'remove' or empty clears the binding\n"
      << "space config <space-name> space.cache.low_watermark=<0-100>           : cache "
         "used-capacity low watermark percent [ default=70 ]\n"
      << "space config <space-name> space.cache.high_watermark=<0-100>          : cache "
         "used-capacity high watermark percent [ default=85 ]\n"
      << std::endl
      << "TAPE REST API specific parameters:\n"
      << "space config default " << eos::mgm::rest::TAPE_REST_API_SWITCH_ON_OFF
      << "=on|off                         : enable/disable the tape REST API handler [ "
         "default=off ]\n"
      << "space config default " << eos::mgm::rest::TAPE_REST_API_STAGE_SWITCH_ON_OFF
      << "=on|off                         : enable/disable the tape REST API STAGE "
         "resource [ default=off ]\n"
      << "                                => both keys above also accept the 'space.' "
         "prefix, e.g. space."
      << eos::mgm::rest::TAPE_REST_API_SWITCH_ON_OFF << "=on|off\n"
      << std::endl
      ...
      << "=<path>            : optional path to a script used to determine the number of "
         "free bytes in a given EOS space [ default='"
      << eos::mgm::tgc::TGC_DEFAULT_FREE_BYTES_SCRIPT << "' ]\n"
      << "                                                                        => an "
         "empty or invalid path means the compile time default way of determining free "
         "space will be used\n"
      << "space config <space-name> space." << eos::mgm::tgc::TGC_NAME_AVAIL_BYTES
      << "=<#>                    : configure the number of available bytes the space "
         "should have [ default="
      << eos::mgm::tgc::TGC_DEFAULT_AVAIL_BYTES << " ] \n"
      << "space config <space-name> space." << eos::mgm::tgc::TGC_NAME_TOTAL_BYTES
      << "=<#>                    : configure the total number of bytes the space should "
         "have before the tape-aware GC kicks in [ default="
      << eos::mgm::tgc::TGC_DEFAULT_TOTAL_BYTES << " ] \n"
      << std::endl
      << "space config rm <space-name> <key>                   : remove the given key "
         "from the space configuration\n"
      << std::endl
      << "space define <space-name> [<groupsize> [<groupmod>]] : define how many "
         "filesystems can end up in one scheduling group <groupsize> [ default=0 ]\n"
      << "                                                       => <groupsize>=0 means "
         "that no groups are built within a space, otherwise it should be the maximum "
         "number of nodes in a scheduling group\n"
      << "                                                       => <groupmod> maximum "
         "number of groups in the space, which should be at least equal to the maximum "
         "number of filesystems per node\n"
      << std::endl
      << "space inspector [--current|-c] [--last|-l] [-m] [-p] [-e] [-s|--space "
         "<space_name>] [--all|-a] [--cost|-C] [--usage|-U] [--birth|-B] [--access|-A] "
         "[--vs|-V] [--layouts|-L] : show namespace inspector output\n"
      << "\t  -c  : show current scan\n"
      ...
      << std::endl
      << "space node-set <space-name> <node.key> <file-name> : store the contents of "
         "<file-name> into the node configuration variable <node.key> visible to all "
         "FSTs\n"
      << "                                                     => if <file-name> matches "
         "file:<path> the file is loaded from the MGM and not from the client\n"
      << "                                                     => local files cannot "
         "exceed 512 bytes - MGM files can be arbitrary length\n"
      << "                                                     => the contents gets "
         "base64 encoded by default\n"
      << std::endl
      << "space node-get <space-name> <node.key> : get the value of <node.key> and "
         "base64 decode before output\n"
      << "                                         => if the value for <node.key> is "
         "identical for all nodes in the referenced space, it is dumped only once, "
         "otherwise the value is dumped for each node separately\n"
      << std::endl
      << "space reset <space-name> "
         "[--egroup|mapping|drain|scheduledrain|schedulebalance|ns|nsfilesystemview|"
         "nsfilemap|nsdirectorymap] : reset different space attributes\n"
      ...
      << "                                => <on> value will enable all nodes, <off> "
         "value won't affect nodes\n"
      << std::endl
      ...
      << std::endl
      << "space groupdrainer status <space-name> [--detail(-d)|-m]  : print groupdrainer "
         "status\n"
      << "space groupdrainer reset <space-name> <--failed|--all>    : reset failed "
         "transfers/all caches\n"
      << std::endl;
  return oss.str();
}
