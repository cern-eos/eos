...
std::string MakeFsHelp()
{
  ...
  oss << "Usage: fs add|boot|clone|compare|config|dropdeletion|dropghosts|"
      ...
      << "    add and assign a filesystem based on the unique identifier of "
         "the disk <uuid>"
      << std::endl
      << "    -m|--manual  : add with user specified <fsid> and <space>" << std::endl
      << "    <fsid>       : numeric filesystem id 1...65535" << std::endl
      << "    <uuid>       : unique string identifying current filesystem" << std::endl
      << "    <node-queue> : internal EOS identifier for a node e.g "
         "/eos/<host>:<port>/fst"
      << std::endl
      << "                   it is preferable to use the host:port syntax" << std::endl
      << "    <host>       : FQDN of host where filesystem is mounter" << std::endl
      << "    <port>       : FST XRootD port number [usually 1095]" << std::endl
      << "    <mountpoint> : local path of the mounted filesystem e.g /data/" << std::endl
      ...
      << "    replicate files from the source to the target filesystem" << std::endl
      ...
      << "    configure the filesystem parameter, where <key> and <value> "
         "can be:"
      << std::endl
      << "    configstatus=rw|wo|ro|drain|off|empty [--comment "
         "\"<comment>\"]"
      << std::endl
      << "      the legacy compatibility key: one word covering what the "
         "filesystem"
      << std::endl
      << "      accepts, whether it is draining and whether it is in service" << std::endl
      ...
      << "      drain     : read-only, and start draining it" << std::endl
      << "      off       : disable filesystem" << std::endl
      << "      empty     : empty filesystem, possible only if there are no" << std::endl
      << "                  more files stored on it" << std::endl
      << "      --comment : pass a reason for the status change" << std::endl
      << "    sched=<spec> [--comment \"<comment>\"]" << std::endl
      << "      what the filesystem accepts, per traffic class. Client traffic"
      << std::endl
      << "      is what users ask for; internal traffic is drain, balancing," << std::endl
      << "      conversion and fsck. Never starts or stops a drain." << std::endl
      << "      <spec> is either a preset:" << std::endl
      << "        rw       : everything" << std::endl
      << "        ro       : reads only, both classes" << std::endl
      << "        wo       : writes only, both classes" << std::endl
      << "        drain    : same bits as ro, see drain= for the verb" << std::endl
      << "        internal : no client traffic at all, internal unrestricted" << std::endl
      << "        clientro : clients may read, internal unrestricted" << std::endl
      << "        none     : nothing at all" << std::endl
      << "      or the explicit form, a comma separated list of per-class terms"
      << std::endl
      ...
      << "      entry rescan interval (default 7 days), 0 disables scanning" << std::endl
      ...
      << "      disk consistency thread scan interval (default 4h)" << std::endl
      << "    scan_ns_interval=<seconds>\n"
      << "      namespace consistency thread scan interval (default 3 days)" << std::endl
      ...
      << "    drainperiod=<seconds>\n"
      << "      period a drain job is allowed to finish the drain procedure" << std::endl
      ...
      << "      the sharedfs this filesystem is to be assigned to. To "
         "unlabel set the sharedfs name to 'none' !"
      << std::endl
      << std::endl
      ...
      << "    -d|--drain : display filesystems in drain status" << std::endl
      ...
      << "    [matchlist]" << std::endl
      << "       -> can be the name of a space or a comma separated list of" << std::endl
      << "          spaces e.g 'default,spare'" << std::endl
      << "       -> can be a grep style list to filter certain filesystems" << std::endl
      << "          e.g. 'fs ls -d drain,bootfailure'" << std::endl
      << "       -> can be a combination of space filter and grep e.g." << std::endl
      ...
      << "    src_grp   : all filesystems from scheduling group are moved" << std::endl
      << "    src_space : all filesystems from space are moved" << std::endl
      << "    dst_grp   : destination scheduling group" << std::endl
      << "    dst_space : destination space - best match scheduling group" << std::endl
      ...
      << "    remove filesystem by various identifiers, where <mnt> is the " << std::endl
      << "    mountpoint" << std::endl
      << std::endl
      << "  fs status [-r] [-l] <identifier>" << std::endl
      << "    return all status variables of a filesystem and calculates" << std::endl
      << "    the risk of data loss if this filesystem is removed" << std::endl
      << "    <identifier> can be: " << std::endl
      << "       <fsid> : filesystem id" << std::endl
      << "       [<host>] <mountpoint> : if host is not specified then it's" << std::endl
      << "       considered localhost" << std::endl
      << "    -l : list all files which are at risk and offline files" << std::endl
