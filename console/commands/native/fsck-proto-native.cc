...
public:
  ...
  int run(const std::vector<std::string>& args, CommandContext& ctx) override {
    ...
    if (sub == "stat") {
      in += "&mgm.subcmd=stat"; if (std::find(args.begin()+1,args.end(),"-m")!=args.end()) in += "&mgm.option=m";
    } else if (sub == "config") {
      if (args.size() < 3) { printHelp(); global_retc = EINVAL; return 0; }
      in += "&mgm.subcmd=config&mgm.key="; in += args[1].c_str(); in += "&mgm.value="; in += args[2].c_str();
    } else if (sub == "report") {
      in += "&mgm.subcmd=report";
      for (size_t i=1;i<args.size();++i){ const auto& a=args[i]; if(a=="-a") in+="&mgm.option=a"; else if(a=="-h") in+="&mgm.option=h"; else if(a=="-i") in+="&mgm.option=i"; else if(a=="-l") in+="&mgm.option=l"; else if(a=="-j"||a=="--json") in+="&mgm.option=j"; else if(a=="--error" && i+1<args.size()){ in+="&mgm.error="; in+=args[++i].c_str(); } }
    } else if (sub == "repair") {
      in += "&mgm.subcmd=repair"; for (size_t i=1;i<args.size();++i){ const auto& a=args[i]; if(a=="--fxid"&&i+1<args.size()){ in+="&mgm.fxid="; in+=args[++i].c_str(); } else if(a=="--fsid"&&i+1<args.size()){ in+="&mgm.fsid="; in+=args[++i].c_str(); } else if(a=="--error"&&i+1<args.size()){ in+="&mgm.error="; in+=args[++i].c_str(); } else if(a=="--async"){ in+="&mgm.async=1"; } }
    } else if (sub == "clean_orphans") {
      in += "&mgm.subcmd=clean_orphans"; for (size_t i=1;i<args.size();++i){ const auto& a=args[i]; if(a=="--fsid"&&i+1<args.size()){ in+="&mgm.fsid="; in+=args[++i].c_str(); } else if(a=="--force-qdb-cleanup"){ in+="&mgm.forceqdb=1"; } }
    } else {
