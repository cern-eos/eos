...
std::string MakeSchedHelp()
{
  return "Usage: sched configure|show|disable|ls [OPTIONS]\n\n"
         ...
         "    its own. NOT persisted - it is lost on an MGM restart; use\n"
         "    \"space config <space> space.scheduler.type=\" for a setting that\n"
         "    survives one\n"
         "    <schedtype> names the engine first:\n"
         "      geotree  - the legacy engine, and the default\n"
         "      flat:<strategy> - the flat scheduler, one of geo, roundrobin,\n"
         "                 threadlocalroundrobin|tlrr, random, fidrandom,\n"
         "                 weightedrandom, weightedroundrobin|weightedrr\n"
         "    the pre-prefix spellings still work but are deprecated\n"
         "  configure weight <space> <fsid> <weight>\n"
         "    configure weight for fsid in space. NOT persisted - it is lost\n"
         "    on an MGM restart and on any topology rebuild, which recomputes\n"
         "    the weight from the disk capacity\n"
