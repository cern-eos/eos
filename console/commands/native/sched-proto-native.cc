...
struct SchedHelper: public ICmdHelper {
  ...
  bool ParseCommand(const char* arg) override {
    ...
    if (token == "configure" || token == "config") {
      ...
      if (!tokenizer.NextToken(token)) return false;
      if (token == "type") { auto* type = config->mutable_type(); if (!tokenizer.NextToken(token)) return false; type->set_schedtype(token); }
      else if (token == "weight") { auto* w = config->mutable_weight(); std::string space, id_str, weight_str; if (!(tokenizer.NextToken(space) && tokenizer.NextToken(id_str) && tokenizer.NextToken(weight_str))) return false; int32_t item_id=0; uint8_t weight=0; try { item_id = std::stoi(id_str); unsigned long wtmp = std::stoul(weight_str); if (wtmp > 255) return false; weight = static_cast<uint8_t>(wtmp); } catch (...) { return false; } w->set_id(item_id); w->set_weight(weight); w->set_spacename(space); }
      else if (token == "show") { auto* showp = config->mutable_show(); if (!tokenizer.NextToken(token)) return false; if (token == "type") { showp->set_option(eos::console::SchedProto_ShowProto::TYPE); if (tokenizer.NextToken(token)) showp->set_spacename(token); } }
      else if (token == "forcerefresh") { config->mutable_refresh(); }
      else { return false; }
    } else if (token == "ls") {
      auto* ls = sched->mutable_ls(); if (!tokenizer.NextToken(token)) return false; ls->set_spacename(token); if (!tokenizer.NextToken(token)) return false; if (token == "bucket") ls->set_option(eos::console::SchedProto_LsProto::BUCKET); else if (token == "disk") ls->set_option(eos::console::SchedProto_LsProto::DISK); else ls->set_option(eos::console::SchedProto_LsProto::ALL);
    } else { return false; }
    return true;
  }
};
