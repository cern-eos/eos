...
com_squash(char* arg1)
{
  ...
  if (cmd == "relabel") {
    ...
    if (stat(squashpack.c_str(), &buf)) {
      fprintf(stderr,"error: the squashfs package file is missing for this label!\n");
      global_retc = ENOENT;
      return (0);
    }
  ...
  if (cmd == "rm-release") {
    ...
    if (!global_retc) {
      ...
      eos::common::Path packagepath(path.c_str());
      std::string nextrelease = std::string(packagepath.GetPath()) + std::string("/next");
      std::string currentrelease = std::string(packagepath.GetPath()) + std::string("/current");
      std::string archive = std::string(packagepath.GetPath()) + std::string("/.archive");
  ...
  if (cmd == "new-release") {
    ...
    std::string archivepath = std::string(packagepath.GetPath()) + std::string("/") + std::string(".archive/");
  ...
  if (cmd == "pack-release") {
    ...
    std::string nextrelease = std::string(packagepath.GetPath()) + std::string("/next");
    std::string currentrelease = std::string(packagepath.GetPath()) + std::string("/current");
    std::string hiddencurrentrelease = std::string(packagepath.GetPath()) + std::string("/.current");
  ...
  if (cmd == "trim-release") {
    ...
    if (!keepdays.length()) {
      fprintf(stderr,"error: you have to specify the number of days you want to keep releases : squash trim-release <path> <n-days> [<max-versions]\n");
      global_retc = EINVAL;
      return (0);
    }
    ...
    if (!n_keepdays) {
      fprintf(stderr,"error: you have to specify the number of days you want to keep releases : squash trim-release <path> <n-days>\n");
      global_retc = EINVAL;
      return (0);
    }

    size_t n_keepversions = keepversions.length()?strtol(keepversions.c_str(),0,10):0;
