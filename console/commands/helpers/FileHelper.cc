...
FileHelper::ParseCommand(const char* arg)
{
  ...
  if (subcommand == "info") {
    return ParseInfo(tokenizer);
  } else if (subcommand == "touch") {
    return ParseTouch(tokenizer);
  } else if (subcommand == "adjustreplica") {
    return ParseAdjustreplica(tokenizer);
  } else if (subcommand == "check") {
    return ParseCheck(tokenizer);
  } else if (subcommand == "convert") {
    return ParseConvert(tokenizer);
  } else if (subcommand == "copy") {
    return ParseCopy(tokenizer);
  } else if (subcommand == "drop") {
    return ParseDrop(tokenizer);
  } else if (subcommand == "layout") {
    return ParseLayout(tokenizer);
  } else if (subcommand == "move") {
    return ParseMove(tokenizer);
  } else if (subcommand == "purge") {
    return ParsePurge(tokenizer);
  } else if (subcommand == "rename") {
    return ParseRename(tokenizer);
  } else if (subcommand == "rename_with_symlink") {
    return ParseRenameWithSymlink(tokenizer);
  } else if (subcommand == "replicate") {
    return ParseReplicate(tokenizer);
  } else if (subcommand == "share") {
    return ParseShare(tokenizer);
  } else if (subcommand == "symlink") {
    return ParseSymlink(tokenizer);
  } else if (subcommand == "tag") {
    return ParseTag(tokenizer);
  } else if (subcommand == "verify") {
    return ParseVerify(tokenizer);
  } else if (subcommand == "version") {
    return ParseVersion(tokenizer);
  } else if (subcommand == "versions") {
    return ParseVersions(tokenizer);
  } else if (subcommand == "workflow") {
    return ParseWorkflow(tokenizer);
  } else {
...
FileHelper::Execute(bool print_err, bool add_route)
{
  ...
  if (mReq.has_file() && mReq.file().has_check()) {
    ...
    if (retc == 0 && !mOutcome.result.empty()) {
      ...
      FormatCheckOutput(mOutcome.result, options);
    }
                              ...
                              const std::string& options)
{
  ...
  XrdOucEnv env(response.c_str());
  const char* ns_path = env.Get("mgm.nspath");
  const char* checksum = env.Get("mgm.checksum");
  const char* checksumtype = env.Get("mgm.checksumtype");
  const char* size_str = env.Get("mgm.size");
  const char* nrep = env.Get("mgm.nrep");
  const char* fid0 = env.Get("mgm.fid0");
  ...
  uint64_t mgm_size = std::stoull(size_str);
  int stripes = stripes_str ? atoi(stripes_str) : 0;
  int nrep_count = atoi(nrep);
  bool silent_cmd = (options.find("%silent") != std::string::npos) || mIsSilent;

  // Print header
  if (!silent_cmd) {
    fprintf(stdout, "path=\"%s\" fxid=\"%s\" size=\"%llu\" nrep=\"%s\" "
            "checksumtype=\"%s\" checksum=\"%s\"\n",
            ns_path, fid0 ? fid0 : "unknown",
            (unsigned long long)mgm_size, nrep,
            checksumtype ? checksumtype : "unknown", checksum);
  }
  ...
  for (int i = 0; i < 255; ++i) {
    err_label = "none";
    std::string repurl_key = "mgm.replica.url" + std::to_string(i);
    std::string repfid_key = "mgm.fid" + std::to_string(i);
    std::string repfsid_key = "mgm.fsid" + std::to_string(i);
    std::string repbootstat_key = "mgm.fsbootstat" + std::to_string(i);
    std::string repfstpath_key = "mgm.fstpath" + std::to_string(i);
    const char* repurl = env.Get(repurl_key.c_str());
    const char* repfid = env.Get(repfid_key.c_str());
    const char* repfsid = env.Get(repfsid_key.c_str());
    const char* repbootstat = env.Get(repbootstat_key.c_str());
    const char* repfstpath = env.Get(repfstpath_key.c_str());
    ...
    bool down = (strcmp(repbootstat, "booted") != 0);
    ...
    if ((retc = GetRemoteFmdFromLocalDb(repurl, repfid, repfsid, fmd))) {
      if (!silent_cmd) {
