...
com_fs(char* arg1)
{
  ...
  if (subcommand == "verify") {
    ...
    for (int i = 0; i < 5; i++) {
      if (options[i].length() &&
          (options[i] != "-checksum") && (options[i] != "-commitchecksum") &&
          (options[i] != "-commitsize") && (options[i] != "-rate")) {
        goto com_fs_usage;
      }
  ...
  if (subcommand == "dumpmd") {
    ...
    if (!monitor) {
      ...
      if (option3.length() && (option3 != "-path") && (option3 != "-fid") &&
          (option3 != "-size")) {
        goto com_fs_usage;
      }
    }
...
com_fs_usage:
  ...
  fprintf(stdout, "fs config <fsid> proxygroup=<proxygroupname> : \n");
  fprintf(stdout,
          "                                                  schedule a proxy for this fs by taking it from the given proxygroup\n");
  fprintf(stdout,
          "                                                  the special value \"<none>\" is the same as no value and means no proxy scheduling\n");
  fprintf(stdout, "fs config <fsid> filestickyproxydepth=<depth> : \n");
  fprintf(stdout,
          "                                                  depth of the subtree to be considered for file-stickyness\n");
  fprintf(stdout,
          "                                                  a negative value means no file-stickiness\n");
  fprintf(stdout, "fs config <fsid> forcegeotag=<geotag> : \n");
  fprintf(stdout,
          "                                                  set the filsystem's geotag, overriding the host geotag value\n");
  fprintf(stdout,
          "                                                  the special value \"<none>\" is the same as no value and means no override\n");
  fprintf(stdout, "\n");
  fprintf(stdout,
