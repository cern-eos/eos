...
com_scitoken_usage:
  ...
  std::cerr << "       scitoken create                       : create a sci "
               "token for agiven keyid,issuer, profiel containing claims "
               "[default profile=wlcg]"
            << std::endl;
  std::cerr << std::endl;
  std::cerr
      << "       scitoken create --issuer <issuer> --keyid <keyid> "
         "[--profile=<profile>] --claim <claim-1> {... --claim <claim-N>} "
         "[--privkey <private-key-file>] [--pubkey <public-key-file>] [--expires unix-tst]"
      << std::endl;
  std::cerr
      << "                                    <issuer> : URL of the issuer"
      << std::endl;
  std::cerr << "                                     <keyid> : key id to "
               "request from the issuer"
            << std::endl;
  std::cerr << "                                   <profile> : token profile - "
               "can be 'wlcg' 'scitokens1' 'scitokens2' 'atjwt'"
            << std::endl;
  std::cerr << "                                    <claims> : <key>=<value> "
               "e.g. scope=storage.read:/eos/, scope=storage.modify:/eos/ ..."
            << std::endl;
  std::cerr
      << "                          <private-key-file> : file with the private "
         "key in PEM format - default is /etc/xrootd/<keyid>-key.pem"
      << std::endl;
  std::cerr
      << "                           <public-key-file> : file with the public "
         "key in PEM format - default is /etc/xrootd/<keyid>-pkey.pem"
      << std::endl;
  std::cerr << std::endl;
  std::cerr << "       scitoken dump                         : base64 decode a "
               "scitoken - this does not verify the token"
            << std::endl;
  std::cerr << std::endl;
  std::cerr << "       scitoken dump <token>" << std::endl;
  std::cerr << std::endl;
  std::cerr << "       scitoken create-keys [--keyid <keyid>]: create a pem "
               "key pair and a JSON public web key "
            << std::endl;
  std::cerr << "                                               - if keyid is "
               "specified the public and private keys are found under "
               "/etc/xrootd/<keyid>-key.pem /etc/xrootd/<keyid>-pkey.pem"
            << std::endl;
  std::cerr
      << "                                                 - otherwise they "
         "are stored in the CWD under default-key.pem default-pkey.pem"
      << std::endl;
  std::cerr << "                                               - the JSON web "
               "key is printed on stdout, the key locations on stderr"
            << std::endl;
  std::cerr << "       Examples:                               eos scitoken "
               "create --issuer eos.cern.ch --keyid eos --profile wlcg --claim "
               "sub=foo --claim scope=storage.read:/eos"
            << std::endl;
  std::cerr << "                                               eos scitoken "
               "dump eyJhb..."
            << std::endl;
  std::cerr << "                                               eos scitoken "
               "create-keys --keyid=eos > /etc/xrootd/eos.json"
            << std::endl;
  global_retc = EINVAL;
  return (0);
}
