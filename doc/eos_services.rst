...
It’s the same for stop, status and restart.
You can't start the daemon if it is not configured
...
systemctl stop eos@*    -   for stopping
systemctl status eos@*  -   for status
systemctl restart eos@* -   for restarting
