...
RUN dnf config-manager --add-repo https://linuxsoft.cern.ch/cern/rhel/9/CERN/$(uname -i)/ --set-enabled \
...
#RUN dnf install rpm-sign 
RUN dnf install --nogpg -y ccache cmake3 gcc-c++ git make rpm-build which\
