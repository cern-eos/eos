     ...
     if (isRW)
     {
       ...
       if (raw_file->IsOpen ())
       {
         ...
         if ((path = fabst->GetUtimes (ut)))
         {
           ...
           this->utimes (path, ut, uid, gid, pid);
         }
         else
         {
       ...
       else
       {
         ...
         {
           eos_static_debug ("CLOSEDEBUG closing touched file %s open with flag %d and utiming", raw_file->GetOpenPath ().c_str (), (int) raw_file->GetOpenFlags ());
           // run the utimes command now after the close
           this->utimes (path, ut, uid, gid, pid);
         }
       }
