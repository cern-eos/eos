...
Stat::PrintOutTotal (XrdOucString &out, bool details, bool monitoring, bool numerical)
{
  ...
  for (it = tags_ext.begin(); it != tags_ext.end(); ++it)
  {
    ...
    if (details)
    {
      if (!monitoring)
      {
        sprintf(outline, "ALL        %-32s %12s %8s %8s %8s %8s\n", tag, "spl", n5, n60, n300, n3600);
        out += outline;
        sprintf(outline, "ALL        %-32s %12s %8s %8s %8s %8s\n", tag, "min", m5, m60, m300, m3600);
        out += outline;
        sprintf(outline, "ALL        %-32s %12s %8s %8s %8s %8s\n", tag, "avg", a5, a60, a300, a3600);
        out += outline;
        sprintf(outline, "ALL        %-32s %12s %8s %8s %8s %8s\n", tag, "max", M5, M60, M300, M3600);
        out += outline;
      }
      else
      {
  ...
  if (details)
  {
    ...
    for (tuit_ext = StatExtUid.begin(); tuit_ext != StatExtUid.end(); tuit_ext++)
    {
      ...
      for (it = tuit_ext->second.begin(); it != tuit_ext->second.end(); ++it)
      {
        ...
        if (!monitoring)
        {
          sprintf(outline, "%-10s %-32s %12s %8s %8s %8s %8s\n", identifier, tag, "spl", n5, n60, n300, n3600);
          out += outline;
          sprintf(outline, "%-10s %-32s %12s %8s %8s %8s %8s\n", identifier, tag, "min", m5, m60, m300, m3600);
          out += outline;
          sprintf(outline, "%-10s %-32s %12s %8s %8s %8s %8s\n", identifier, tag, "avg", a5, a60, a300, a3600);
          out += outline;
          sprintf(outline, "%-10s %-32s %12s %8s %8s %8s %8s\n", identifier, tag, "max", M5, M60, M300, M3600);
          out += outline;
        }
        else
        {
          sprintf(outline, "%s cmd=%s:spl 5s=%s 60s=%s 300s=%s 3600s=%s\n", identifier, tag, n5, n60, n300, n3600);
          out += outline;
          sprintf(outline, "%s cmd=%s:min 5s=%s 60s=%s 300s=%s 3600s=%s\n", identifier, tag, m5, m60, m300, m3600);
          out += outline;
          sprintf(outline, "%s cmd=%s:avg 5s=%s 60s=%s 300s=%s 3600s=%s\n", identifier, tag, a5, a60, a300, a3600);
          out += outline;
