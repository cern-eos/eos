...
ProcCommand::Transfer()
{
  ...
  XrdOucString option = pOpaque->Get("mgm.txoption") ?
                        pOpaque->Get("mgm.txoption") : "";
  XrdOucString noauth = pOpaque->Get("mgm.txnoauth") ?
                        pOpaque->Get("mgm.txnoauth") : "";

  if ((mSubCmd != "submit") && (mSubCmd != "ls") && (mSubCmd != "cancel") &&
      (mSubCmd != "enable") && (mSubCmd != "disable") && (mSubCmd != "reset") &&
      (mSubCmd != "clear") && (mSubCmd != "resubmit") && (mSubCmd != "kill") &&
      (mSubCmd != "log") && (mSubCmd != "purge")) {
    retc = EINVAL;
  ...
  if ((mSubCmd == "submit")) {
    ...
    if (usegsi) {
      ...
      if (eos::common::SymKey::Base64Encode((char*) Credential.c_str(),
                                            Credential.length() + 1, CredentialB64)) {
        ...
        if (krb5buf) {
          ...
          if (fd >= 0) {
            if ((::read(fd, krb5buf, krbbuf.st_size)) == krbbuf.st_size) {
              if (eos::common::SymKey::Base64Encode((char*) krb5buf, krbbuf.st_size,
                                                    CredentialB64)) {
                Credential = CredentialB64;
              }
            }
    ...
    retc = gTransferEngine.Submit(src, dst, rate, streams, group, stdOut, stdErr,
                                  *pVid, 86400, Credential, (option.find("s") != STR_NPOS) ? true : false,
                                  (noauth == "1"));
  }
