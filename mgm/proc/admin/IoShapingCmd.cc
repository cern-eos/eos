              ...
              eos::console::ReplyProto& reply)
{
  ...
  case eos::console::IoProto_ShapingProto_ConfigAction::kSet: {
    ...
    if (set_req.has_system_stats_window_seconds()) {
      const auto debug = set_req.system_stats_window_seconds();
      eos_static_info("msg=\"TEST setting system stats time window\" window_seconds=%u",
                      debug);
      engine.SetSystemStatsWindowSeconds(set_req.system_stats_window_seconds());
