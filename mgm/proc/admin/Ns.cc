...
ProcCommand::Ns()
{
  ...
  if (mSubCmd == "mutex") {
    if (pVid->uid == 0) {
      ...
      if (smplrate1 || smplrate10 || smplrate100 || toggleorder || toggletiming) {
        nooption = false;
      }
  ...
  if ((mSubCmd != "mutex") && (mSubCmd != "compact")) {
    ...
    unsigned long long d = (unsigned long long)
                           gOFS->eosDirectoryService->getNumContainers();
    ...
    if ((!::stat(gOFS->MgmNsFileChangeLogFile.c_str(), &statf)) &&
      ...
      eos::common::StringConversion::GetReadableSizeString(clfsize,
          (unsigned long long) statf.st_size, "B");
      eos::common::StringConversion::GetReadableSizeString(cldsize,
          (unsigned long long) statd.st_size, "B");
      eos::common::StringConversion::GetReadableSizeString(clfratio,
          (unsigned long long) f ? (1.0 * statf.st_size) / f : 0, "B");
      eos::common::StringConversion::GetReadableSizeString(cldratio,
          (unsigned long long) d ? (1.0 * statd.st_size) / d : 0, "B");
    }
    ...
    if (!monitoring) {
      ...
      stdOut += eos::common::StringConversion::GetReadableSizeString(sizestring,
                (unsigned long long) mem.vmsize, "B");
      stdOut += "\n";
      stdOut += "ALL      memory resident                  ";
      stdOut += eos::common::StringConversion::GetReadableSizeString(sizestring,
                (unsigned long long) mem.resident, "B");
      stdOut += "\n";
      stdOut += "ALL      memory share                     ";
      stdOut += eos::common::StringConversion::GetReadableSizeString(sizestring,
                (unsigned long long) mem.share, "B");
      stdOut += "\n";

      if (pstat.vsize > gOFS->LinuxStatsStartup.vsize) {
        stdOut += "ALL      memory growths                   ";
        stdOut += eos::common::StringConversion::GetReadableSizeString(sizestring,
                  (unsigned long long)(pstat.vsize - gOFS->LinuxStatsStartup.vsize), "B");
        stdOut += "\n";
      } else {
        stdOut += "ALL      memory growths                  -";
        stdOut += eos::common::StringConversion::GetReadableSizeString(sizestring,
      ...
      stdOut += eos::common::StringConversion::GetSizeString(clfsize,
                (unsigned long long) statf.st_size);
      stdOut += "\n";
      stdOut += "uid=all gid=all ns.total.directories.changelog.size=";
      stdOut += eos::common::StringConversion::GetSizeString(cldsize,
                (unsigned long long) statd.st_size);
      stdOut += "\n";
      stdOut += "uid=all gid=all ns.total.files.changelog.avg_entry_size=";
      stdOut += eos::common::StringConversion::GetSizeString(clfratio,
                (unsigned long long) f ? (1.0 * statf.st_size) / f : 0);
      stdOut += "\n";
      stdOut += "uid=all gid=all ns.total.directories.changelog.avg_entry_size=";
      stdOut += eos::common::StringConversion::GetSizeString(cldratio,
                (unsigned long long) d ? (1.0 * statd.st_size) / d : 0);
      stdOut += "\n";
      ...
      if (pstat.vsize > gOFS->LinuxStatsStartup.vsize) {
        ...
        stdOut += eos::common::StringConversion::GetSizeString(sizestring,
                  (unsigned long long)(pstat.vsize - gOFS->LinuxStatsStartup.vsize));
        stdOut += "\n";
      } else {
        stdOut += "uid=all gid=all ns.memory.growth=-";
        stdOut += eos::common::StringConversion::GetSizeString(sizestring,
        ...
        if ((action != "on") && (action != "off")) {
          ...
          if (action == "on") {
            if ((type != "files") &&
              ...
              gOFS->MgmMaster.ScheduleOnlineCompacting((time(NULL) + atoi(delay.c_str())),
                  atoi(interval.c_str()));
