...
ProcCommand::Accounting()
{
  ...
  {
    static const auto processAccountingAttribute = [](std::pair<std::string, std::string> attr, Json::Value& storageShare)
    {
      ...
      if(attr.first.find(accountingAttrPrefix) == 0) {
        ...
        if(attr.second.find(',') != std::string::npos) {
          for(auto&& attrValue : eos::common::StringTokenizer::split(attr.second, ',')) {
    ...
    gOFS->_attr_ls(gOFS->MgmProcPath.c_str(), errInfo, vid, (const char*) 0, attributes);
  ...
  if (mSubCmd == "config") {
    if(pOpaque->Get("mgm.accounting.expired")) {
      try {
        ...
        accountingCache.SetExpiredAfter(
          std::chrono::duration_cast<std::chrono::seconds>(std::chrono::minutes(minutes)));
    ...
    auto options = std::string(pOpaque->Get("mgm.option") ? pOpaque->Get("mgm.option") : "");
    bool forceUpdate = options.find('f') != std::string::npos;

    try{
      auto json = accountingCache.getCachedObject(forceUpdate, generateAccountingJson, std::ref(*pVid));
      stdOut += json.c_str();
