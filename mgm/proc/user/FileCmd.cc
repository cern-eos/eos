...
// ! TODO Tavi
// ! in share, mHttpFormat, still needed???
// ! enable json too
//------------------------------------------------------------------------------
// Process request
//------------------------------------------------------------------------------
...
FileCmd::ProcessRequest() noexcept
{
  ...
  if (mReqProto.has_file()) {
    ...
    if (file.has_adjustreplica()) {
      return AdjustReplicaSubcmd(file);
    } else if (file.has_check()) {
      return GetMdLocationSubcmd(file);
    } else if (file.has_convert()) {
      return ConvertSubcmd(file);
    } else if (file.has_copy()) {
      return CopySubcmd(file);
    } else if (file.has_drop()) {
      return DropSubcmd(file);
    } else if (file.has_fileinfo()) {
      return FileinfoSubcmd(file);
    } else if (file.has_layout()) {
      return LayoutSubcmd(file);
    } else if (file.has_move()) {
      return MoveSubcmd(file);
    } else if (file.has_purge()) {
      return PurgeSubcmd(file);
    } else if (file.has_rename()) {
      return RenameSubcmd(file);
    } else if (file.has_rename_with_symlink()) {
      return RenameWithSymlinkSubcmd(file);
    } else if (file.has_replicate()) {
      return ReplicateSubcmd(file);
    } else if (file.has_share()) {
      return ShareSubcmd(file);
    } else if (file.has_symlink()) {
      return SymlinkSubcmd(file);
    } else if (file.has_tag()) {
      return TagSubcmd(file);
    } else if (file.has_touch()) {
      return TouchSubcmd(file);
    } else if (file.has_verify()) {
      return VerifySubcmd(file);
    } else if (file.has_version()) {
      return VersionSubcmd(file);
    } else if (file.has_versions()) {
      return VersionsSubcmd(file);
    } else if (file.has_workflow()) {
      return WorkflowSubcmd(file);
    } else {
...
FileCmd::FileinfoSubcmd(const eos::console::FileProto& file) noexcept
{
  ...
  // Build options string from protobuf flags
  XrdOucString option = "";

  if (fileinfo.monitoring()) {
    option += "-m";
  }
  ...
  // Build opaque info string
  XrdOucString opaque_info = "mgm.cmd=fileinfo&mgm.path=";
  opaque_info += spath;

  if (option.length()) {
    opaque_info += "&mgm.file.info.option=";
    opaque_info += option;
  }

  // Create ProcCommand and execute
  XrdOucErrInfo error;
  std::unique_ptr<ProcCommand> procCmd(new ProcCommand());
  int rc = procCmd->open("/proc/user", opaque_info.c_str(), mVid, &error);

  if (rc == SFS_OK) {
    // Read the result
    char buffer[65536];
    XrdSfsXferSize bytes_read = 0;
    std::string result;

    while ((bytes_read = procCmd->read(result.length(), buffer,
                                       sizeof(buffer) - 1)) > 0) {
      buffer[bytes_read] = '\0';
      result += buffer;
    }
    ...
    procCmd->close();
    // Parse the result
    XrdOucEnv resultEnv(result.c_str());
    const char* stdout_str = resultEnv.Get("mgm.proc.stdout");
    const char* stderr_str = resultEnv.Get("mgm.proc.stderr");
    const char* retc_str = resultEnv.Get("mgm.proc.retc");
    const char* json_str = resultEnv.Get("mgm.proc.json");

    if (json_str) {
      XrdOucString json_out = json_str;
      eos::common::StringConversion::UnSeal(json_out);
      reply.set_std_out(json_out.c_str());
    } else if (stdout_str) {
      XrdOucString stdout_out = stdout_str;
      eos::common::StringConversion::UnSeal(stdout_out);
      reply.set_std_out(stdout_out.c_str());
    }

    if (stderr_str) {
      XrdOucString stderr_out = stderr_str;
      eos::common::StringConversion::UnSeal(stderr_out);
      reply.set_std_err(stderr_out.c_str());
    }

    if (retc_str) {
      reply.set_retc(atoi(retc_str));
    } else {
      reply.set_retc(0);
    }
  } else {
    reply.set_std_err("error: failed to execute fileinfo command");
    reply.set_retc(rc);
  }
