  ...
  else
  {
    ...
    if (fs)
    {
      ...
      if (((key == "configstatus") && (eos::common::FileSystem::GetConfigStatusFromString(value.c_str()) != eos::common::FileSystem::kUnknown)) ||
          (((key == "headroom") || (key == "scaninterval") || (key == "graceperiod") || (key == "drainperiod") || (key == "proxygroup") || (key == "filestickyproxydepth") )))
      {
        ...
        else
        {
          ...
          else
          {
            if ((key == "configstatus") && (value == "empty"))
            {
              ...
              try
              {
                const eos::IFsView::FileList& filelist = gOFS->eosFsView->getFileList(fs->GetId());
                if (filelist.size())
                {
                  isempty = false;
                }
