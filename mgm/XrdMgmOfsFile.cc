...
void XrdMgmOfsFile::getCksumFromOpaque(std::string & cksumType, std::string & cksumValue)
{
  ...
  if(const char * cksumtype = getFirst({"eos.checksumtype", "oss.cksumtype"})) {
    cksumType = cksumtype;
  }
  if(const char * cksumvalue = getFirst({"eos.checksum","oss.cksumvalue"})) {
                    ...
                    const char* ininfo)
{
  ...
  if (isPio) {
    ...
    if (openOpaque->Get("eos.checksum") || openOpaque->Get("oss.cksumvalue") || openOpaque->Get("eos.cloneid")) {
