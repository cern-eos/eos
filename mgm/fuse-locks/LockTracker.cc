...
int
/*----------------------------------------------------------------------------*/
LockTracker::setlk(pid_t pid, struct flock* lock, int sleep, const std::string& owner)
/*----------------------------------------------------------------------------*/
{
  if (!sleep) return addLock(pid, lock, owner);
...
/*----------------------------------------------------------------------------*/
{
  ...
  if (wlocks.getconflict(lock)) return false;
