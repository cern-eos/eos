...
public:
  ...
  //!
  //! @param fid id of the file to version, 0 if only the path is known
  //! @param inpath path of the file, only used when fid is 0
  //! @param error error object
  ...
  //! @return SFS_OK if success otherwise SFS_ERROR
  //----------------------------------------------------------------------------
  int ResolveVersionInfo(eos::common::FileId::fileid_t fid, const std::string& inpath,
                         XrdOucErrInfo& error, const eos::common::VirtualIdentity& vid,
                         std::string& path, std::string& vdir,
                         eos::common::VirtualIdentity& fidvid, time_t& filectime);
  ...
  //!
  //! @param fid id of the file to create the version directory for, 0 if only
  //!        the path is known
  //! @param inpath path of the file, only used when fid is 0
  //! @param error error object
  //! @param vid virtual identity of the caller
  ...
  //----------------------------------------------------------------------------
  int CreateVersionDirectory(eos::common::FileId::fileid_t fid, const std::string& inpath,
...
#ifdef HAVE_XROOTD_METRICS
  //! XrdMetrics collector for native XRootD monitoring
  std::unique_ptr<monitoring::XrdMetricsCollector> mMetricsCollector;
#endif
