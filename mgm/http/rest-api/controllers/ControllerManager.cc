...
Controller* ControllerManager::getController(const std::string& urlFromClient)
const
{
  URLParser urlFromClientParser(urlFromClient);
  const auto controllerItor = std::find_if(mControllers.begin(),
                              mControllers.end(), [&urlFromClientParser](const
  std::pair<const std::string, std::unique_ptr<Controller>>& keyValue) {
    return urlFromClientParser.startsBy(keyValue.first);
  });

  if (controllerItor == mControllers.end()) {
    std::ostringstream ss;
    ss << "The URL provided (" << urlFromClient <<
       ") does not allow to identify an existing resource and its version";
