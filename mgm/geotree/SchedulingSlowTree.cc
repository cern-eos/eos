...
ostream& SlowTreeNode::recursiveDisplayAccess(ostream &os, bool useColors, const string &prefix) const
{
  std::string consoleEscapeCode,consoleReset;
  if(useColors)
  {
...
SlowTreeNode* SlowTree::insert( const TreeNodeInfo *info, const TreeNodeStateFloat *state, bool addFsIdLevel,bool allowUpdate)
{
  SlowTreeNode* startFrom=&pRootNode;
  ostringstream oss;
  oss<<info->geotag;
  if(addFsIdLevel) oss<<"::"<<info->fsId;
...
SlowTreeNode* SlowTree::insert( const TreeNodeInfo *info, const TreeNodeStateFloat *state, std::string &fullGeotag, const std::string &partialGeotag, SlowTreeNode* startFrom, SlowTreeNode* startedConstructingAt, bool allowUpdate)
{
    ...
    FastGatewayAccessTree *fgat, Host2TreeIdxMap *host2idx, FastTreeInfo *fastinfo, GeoTag2NodeIdxMap *geo2node) const
{
  ...
  {
    fgat->pNodes[i].fsData.mStatus = ((*fastinfo)[i].proxygroup.empty()?SchedTreeBase::Disabled:SchedTreeBase::Available) ;
  }
  fgat->updateTree();
  return true;
}
