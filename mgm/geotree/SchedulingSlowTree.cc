...
SlowTreeNode* SlowTree::insert(const TreeNodeInfo* info,
                               const TreeNodeStateFloat* state)
{
...
SlowTreeNode* SlowTree::insert(const TreeNodeInfo* info,
                               ...
                               SlowTreeNode* startedConstructingAt)
{
  ...
  GeoTag2NodeIdxMap* geo2node) const
{
  ...
  while (godeeper) {
    ...
    for (vector<const SlowTreeNode*>::const_iterator it = (nodesByDepth.end() -
         2)->begin(); it != (nodesByDepth.end() - 2)->end(); it++) {
      godeeper = false;

      // iterate through the children of each of those nodes
      for (SlowTreeNode::tNodeMap::const_iterator cit = (*it)->pChildren.begin();
