...
int PopulateSchedGroupFromFile (const string &fileName, size_t groupSize, size_t nFsPerBox,
  ...
  for (size_t fs = 0; fs < nFsPerBox; fs++)
  {
    for (map<string, string>::const_iterator it = items.begin (); it != items.end (); it++)
    {
      ...
      { // create a new group if necessary
        if (schedGroups.size () == nGroups) goto end;
...
template<typename T1, typename T2, typename T3, typename T4>
  void functionalTestFastTree (FastTree<T1, T2> *fptree, FastTree<T3, T4> *fatree, GeoTag2NodeIdxMap *geomap,
                               SchedTreeBase::FastTreeInfo *treeinfo, size_t nMaxReplicas)
  {
    ...
    for (size_t loop = 0; loop < 1000; loop++)
    {
      ...
      set_symmetric_difference (repIdxs.begin (), repIdxs.end (), allreplicasset.begin (), allreplicasset.end (),
...
int main ()
{
  ...
  tns->dlScore = 99;
  tns->ulScore = 99;
  tns->mStatus = SchedTreeBase::Available | SchedTreeBase::Writable | SchedTreeBase::Readable;
  ...
  FastGatewayAccessTree *ft = new FastGatewayAccessTree(); ft->selfAllocate(255);
  SchedTreeBase::FastTreeInfo *fti = new SchedTreeBase::FastTreeInfo(); fti->reserve(255);
  Host2TreeIdxMap *ftmap = new Host2TreeIdxMap(); ftmap->selfAllocate(255);
  GeoTag2NodeIdxMap *geomap = new GeoTag2NodeIdxMap(); geomap->selfAllocate(255);
...
int main2()
{
  ...
  FastGatewayAccessTree ft; ft.selfAllocate(255);
  SchedTreeBase::FastTreeInfo fti; fti.reserve(255);
  Host2TreeIdxMap ftmap; ftmap.selfAllocate(255);
  GeoTag2NodeIdxMap geomap; geomap.selfAllocate(255);
...
int mainFull ()
{
  ...
  vector<map<eos::common::FileSystem::fsid_t, SchedTreeBase::tFastTreeIdx> > maxDningToDnerSimil (schedGroups.size ()),
      maxBcingToBcerSimil (schedGroups.size ());

  size_t idx = 0;
  for (vector<set<pair<string, string> > >::const_iterator sgit = schedGroups.begin (); sgit != schedGroups.end ();
      sgit++)
  {
    ostringstream oss;
    oss << idx;
    ...
    for (set<pair<string, string> >::const_iterator it = sgit->begin (); it != sgit->end (); it++)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          if (r < RAND_MAX / 8)
          {
            if (state.mStatus & SchedTreeBase::Drainer) nAvailableFsDrnPlct--;
    ...
    cout << "group " << std::setw (3) << idx << "\tnAvailableFsROAccess = " << std::setw (3) << nAvailableFsROAccess
        << "\tnAvailableFsRWAccess = " << std::setw (3) << nAvailableFsRWAccess << "\tnAvailableFsPlct = "
        << std::setw (3) << nAvailableFsPlct << "\tnAvailableFsBlcPlct = " << std::setw (3) << nAvailableFsBlcPlct
        << "\tnAvailableFsDrnPlct = " << std::setw (3) << nAvailableFsDrnPlct << "\tnUnavailFs = " << std::setw (3)
  ...
  cout << "speed        : " << 3 * schedGroups.size () * nbIter / (float (elapsed) / CLOCKS_PER_SEC)
      << " placements/sec " << endl;
  cout << "----------------------------" << endl << endl;
  ...
  cout << "elapsed time : " << float (elapsed) / CLOCKS_PER_SEC << " sec." << endl;
  cout << "speed        : " << 6 * schedGroups.size () * nbIter / (float (elapsed) / CLOCKS_PER_SEC) << " copies/sec "
      << endl;
  cout << "----------------------" << endl << endl;
  ...
  cout << "elapsed time : " << float (elapsed) / CLOCKS_PER_SEC << " sec." << endl;
  cout << "speed        : " << 6 * schedGroups.size () * nbIter / (float (elapsed) / CLOCKS_PER_SEC) << " repop/sec "
      << endl;
  cout << "----------------------" << endl << endl;
  ...
  for (size_t i = 0; i < schedGroups.size () * nbIter; i++)
  {
    ...
    const string &clientGeoString =
        ftinfos[i % schedGroups.size ()][rand () % ftinfos[i % schedGroups.size ()].size ()].fullGeotag;
    SchedTreeBase::tFastTreeIdx closestNode = geomaps[i % schedGroups.size ()].getClosestFastTreeNode (
  ...
  cout << "elapsed time : " << float (elapsed) / CLOCKS_PER_SEC << " sec." << endl;
  cout << "speed        : " << 3 * schedGroups.size () * nbIter / (float (elapsed) / CLOCKS_PER_SEC) << " access/sec "
      << endl;
  cout << "----------------------" << endl << endl;
  ...
  for (size_t idx = 0; idx < schedGroups.size (); idx++)
  {
    ...
    fsize = fptrees[idx].findFreeSlotsAll (&balancers[idx][0], balancers[idx].size (), 0, false,
  ...
  for (size_t i = 0; i < schedGroups.size () * nbIter; i++)
  {
    ...
    SchedTreeBase::tFastTreeIdx rfs = fsIdxBegV[i % schedGroups.size ()]
        + rand () % (fsIdxEndV[i % schedGroups.size ()] - fsIdxBegV[i % schedGroups.size ()]);
    ftree->updateBranch (rfs);
  }
  elapsed = clock () - begin;
  cout << "UPDATE FAST TREE TEST (ONE BRANCH) " << endl;
  cout << "elapsed time : " << float (elapsed) / CLOCKS_PER_SEC << " sec." << endl;
  cout << "speed        : " << schedGroups.size () * nbIter / (float (elapsed) / CLOCKS_PER_SEC) << " updates/sec "
      << endl;
  cout << "----------------------" << endl << endl;
  ...
  cout << "elapsed time : " << float (elapsed) / CLOCKS_PER_SEC << " sec." << endl;
  cout << "speed        : " << schedGroups.size () * nbIter / (float (elapsed) / CLOCKS_PER_SEC) << " builds/sec "
      << endl;
  cout << "----------------------" << endl << endl;
