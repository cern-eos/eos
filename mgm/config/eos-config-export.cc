...
#include "namespace/ns_quarkdb/tools/CLI11.hpp"
...
int main(int argc, char* argv[]) {
  ...
  std::cerr << "--- Successfully read configuration file, size in bytes: " << fullContents.size() << std::endl;
  ...
  if(!eos::mgm::ConfigParsing::parseConfigurationFile(fullContents, configuration, err)) {
  ...
  //----------------------------------------------------------------------------
  qclient::IntegerParser existsResp(qcl.exec("EXISTS", "eos-config:default").get());

  if(!existsResp.ok()) {
    std::cerr << "Received unexpected response in EXISTS check: " << existsResp.err() << std::endl;
    return 1;
  }
  ...
  std::cerr << "--- Prepared write batch towards QDB, " << multiBuilder.size() << " commands" << std::endl;
  std::cerr << "--- Executing write batch: " <<  qclient::describeRedisReply(qcl.execute(multiBuilder.getDeque()).get()) << std::endl;
  return 0;
}
