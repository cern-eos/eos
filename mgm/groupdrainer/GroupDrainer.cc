...
GroupDrainer::populateFids(eos::common::FileSystem::fsid_t fsid)
{
  ...
  if (const auto kv = mFsidRetryCtr.find(fsid);
      ...
      try {
        ...
        if (e.getErrno() == ENOENT) {
          ...
          eos_err("msg=\"Got an unexpected exception while populating FIDs\" "
                  "fsid=%d fid=%lu errc=%d emsg=\"%s\"",
                  fsid, fid, e.getErrno(), e.getMessage().str().c_str());
        }
  ...
  for (const auto fid : ghosts_to_delete) {
    eos_warning("msg=\"Deleting ghost file from FS view\" fsid=%d fid=%lu", fsid, fid);
