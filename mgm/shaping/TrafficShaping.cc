...
TrafficShapingManager::UpdateLimits()
{
  ...
  auto should_emit_reservation_delay =
      [limits_enabled,
       reservations_enabled](const TrafficShapingPolicy& policy, const bool is_write,
                             const double node_rate_bps, const double io_pressure) {
        if (!limits_enabled) {
          return false;
        }
