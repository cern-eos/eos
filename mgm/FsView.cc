...
FsView::UnRegister(eos::common::FileSystem* fs) 
{
  ...
  if (fs->SnapShotFileSystem(snapshot)) {  
    ...
    if (mNodeView.count(snapshot.mQueue)) {
      ...
      eos_debug("unregister node %s from view", node->GetMember("name").c_str());
      if (!node->size()) {
    ...
    //----------------------------------------------------------------
    if (mGroupView.count(snapshot.mQueue)) {
      FsGroup* group = mGroupView[snapshot.mQueue];
      group->erase(snapshot.mId);
      eos_debug("unregister group %s from view", group->GetMember("name").c_str());
      if (!group->size()) {
	mGroupView.erase(snapshot.mQueue);
	delete group;
      }
    }
    ...
    //----------------------------------------------------------------
    if (mSpaceView.count(snapshot.mQueue)) {
      FsSpace* space = mSpaceView[snapshot.mQueue];
      space->erase(snapshot.mId);
      eos_debug("unregister space %s from view", space->GetMember("name").c_str());
      if (!space->size()) {
	mSpaceView.erase(snapshot.mQueue);
	delete space;
      }
    }
    return true;
...
FsView::UnRegisterNode(const char* nodename)
{
  ...
  if (mNodeView.count(nodename)) {
    while (mNodeView[nodename]->begin()!= mNodeView[nodename]->end()) {
