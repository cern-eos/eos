  ...
  for (auto it = pPxyGrp2DpTME.begin(); it != pPxyGrp2DpTME.end(); it++) {
    ...
    if (dispSnaps && (schedgroup.empty() || schedgroup == "*" ||
      ...
      ostr << "### scheduling snapshot for proxy group " << it->first << " :" <<
           std::endl;
      it->second->foregroundFastStruct->gWAccessTree->recursiveDisplay(ostr,
          useColors) << endl;
    }
    ...
    vector<string>* forceGeoTags)
{
  ...
  if (dataProxys) {
    if (!findProxy(newReplicasIdx, entries, inode, dataProxys, "",
  ...
  if (firewallEntryPoint) {
    ...
    if (!findProxy(newReplicasIdx, entries, inode, firewallEntryPoint,
                   "firewallentrypoint", pProxyCloseToFs ? "" : clientGeoTag, any)) {
      success = false;
      goto cleanup;
    }
  }
                              ...
                              tProxySchedType proxyschedtype)
{
  ...
  if (fsproxygroup) {
    ...
    // copy the fasttree
    if (pxyentry->foregroundFastStruct->gWAccessTree->copyToBuffer((
...
bool GeoTreeEngine::accessReplicasOneGroup(FsGroup* group,
    const size_t& nAccessReplicas,
    ...
    vector<string>* forceGeoTags)
{
  ...
  if (nAccessReplicas == existingReplicas->size()) {
    ...
    accessedReplicas->insert(accessedReplicas->begin(), existingReplicas->begin(),
                             existingReplicas->end());
    return true;
  }
  ...
  entry->doubleBufferMutex.LockRead();
  // locate the existing replicas and the excluded fs in the tree
  vector<SchedTreeBase::tFastTreeIdx> accessedReplicasIdx(nAccessReplicas),
         *existingReplicasIdx = NULL, *excludeFsIdx = NULL, *forceBrIdx = NULL;
  ...
  case balancing:
    success = accessReplicas(entry, nAccessReplicas, &accessedReplicasIdx,
                             accesserNode, existingReplicasIdx,
                             entry->foregroundFastStruct->blcAccessTree, excludeFsIdx, forceBrIdx,
  ...
  for (auto it = accessedReplicasIdx.begin(); it != accessedReplicasIdx.end();
       ++it) {
    const SchedTreeBase::tFastTreeIdx* idx = NULL;
    const unsigned int fsid = (*entry->foregroundFastStruct->treeInfo)[*it].fsId;
                                                 ...
                                                 )
{
  ...
  if (dataProxys) {
    if (!findProxy(ERIdx, entries, inode, dataProxys, "",
  ...
  if (firewallEntryPoint) {
    ...
    if (!findProxy(ERIdx, entries, inode, firewallEntryPoint, "firewallentrypoint",
...
#define unsetOneStateVarStatusInAllFastTrees(flag) \
  ...
  if (keys & sfgFileStickPxy) {
    ...
    if (ftIdx) {
      (*entry->backgroundFastStruct->treeInfo)[ftIdx].fileStickyProxyDepth =
        fs->mFileStickyProxyDepth;
    }
  }
...
bool GeoTreeEngine::updateTreeInfo(GwTMEBase* entry,
...
#define unsetOneStateVarStatusInAllFastTrees(flag) \
  ...
  if (keys & (sfgActive)) {
    if (hs->mActiveStatus == eos::common::FileSystem::kOnline) {
      ...
      // the fs is unavailable
      eos_debug("node %s is getting unavailable  ftidx=%d  stn=%p",
                hs->mHostPort.c_str(), (int)ftIdx, stn);
  ...
  if (keys & (sfgInratemib)) {
    double dlScore = (1.0 - ((hs->mNetEthRateMiB) ? (hs->mNetInRateMiB /
                             hs->mNetEthRateMiB) : 0.0));
  ...
  if (keys & (sfgOutratemib)) {
    double ulScore = (1.0 - ((hs->mNetEthRateMiB) ? (hs->mNetOutRateMiB /
                             hs->mNetEthRateMiB) : 0.0));
  ...
  if (keys & (sfgInratemib | sfgOutratemib | sfgEthmib)) {
    ...
    if (!na.saturated) {
      if (na.fsCount == 1) {
        ...
        pPenaltySched.pMaxNetSpeedClass = std::max(pPenaltySched.pMaxNetSpeedClass ,
                                          netSpeedClass);
        na.netOutWeight += (1.0 - ((hs->mNetEthRateMiB) ? (hs->mNetOutRateMiB /
                                   hs->mNetEthRateMiB) : 0.0));
        na.netInWeight += (1.0 - ((hs->mNetEthRateMiB) ? (hs->mNetInRateMiB /
                                  hs->mNetEthRateMiB) : 0.0));
  ...
  for (auto it = updatesDp.begin(); it != updatesDp.end(); ++it) {
    ...
    for (auto setit = entryset.begin(); setit != entryset.end(); setit++) {
      ...
      if (!entry->backgroundFastStruct->host2TreeIdx->get(host.c_str(), idx)) {
        ...
        eos_debug("fast tree available for fs %s : not updating slowtree",
                  host.c_str());
      }
...
//          *(fscount[netSpeedClass]/hostcount[netSpeedClass]);
          ...
          if (updateSched < 1 || updateSched > 99) { // could be more restrictive
            ...
            AtomicCAS(reinterpret_cast<uint32_t&>
                      ...
                      uf.u);
            uf.f = 0.01 * ((100 - pPenaltyUpdateRate) *
                           pPenaltySched.pGwScorePenaltyF[netSpeedClass] + pPenaltyUpdateRate * updateGw);
            AtomicCAS(reinterpret_cast<uint32_t&>
                      (pPenaltySched.pGwScorePenaltyF[netSpeedClass]) ,
                      reinterpret_cast<uint32_t&>(pPenaltySched.pGwScorePenaltyF[netSpeedClass]) ,
                      uf.u);
            ...
            AtomicCAS(pPenaltySched.pAccessUlScorePenalty[netSpeedClass],
                      ...
                      (SchedTreeBase::tFastTreeIdx)
                      pPenaltySched.pAccessUlScorePenaltyF[netSpeedClass]);
            AtomicCAS(pPenaltySched.pGwScorePenalty[netSpeedClass],
                      pPenaltySched.pGwScorePenalty[netSpeedClass],
                      (SchedTreeBase::tFastTreeIdx) pPenaltySched.pGwScorePenaltyF[netSpeedClass]);
          }
        } else {
...
bool GeoTreeEngine::setGwScorePenalty(char value, int netSpeedClass,
                                      bool setconfig)
{
  return setScorePenalty(pPenaltySched.pGwScorePenaltyF,
                         pPenaltySched.pGwScorePenalty, value, netSpeedClass,
...
{
  return setScorePenalty(pPenaltySched.pGwScorePenaltyF,
                         pPenaltySched.pGwScorePenalty, value, setconfig ? "gwscorepenalty" : "");
}
                                        ...
                                        bool updateFastStruct)
{
  ...
  // check if there is still some space for a new host
  {
    ...
    for (size_t offset = hsn.mGeoTag.find(sub); offset != std::string::npos;
         offset = hsn.mGeoTag.find(sub, offset + sub.length())) {
      depth++;
    }
...
bool GeoTreeEngine::matchHostPxyGr(FsNode* host , const std::string& status,
                                   bool lockFsView, bool updateFastStructures)
{
  eos::common::RWMutexWriteLock lock(pAddRmFsMutex);
  eos::common::FileSystem::host_snapshot_t hsn;
  ...
  for (auto it = pPxyGrp2DpTME.begin(); it != pPxyGrp2DpTME.end(); it++) {
    ...
    bool alreadythere = !(!pPxyGrp2DpTME.count(proxygroup) ||
                          !pPxyHost2DpTMEs.count(url) ||
                          !pPxyHost2DpTMEs[url].count(pPxyGrp2DpTME[proxygroup]));
