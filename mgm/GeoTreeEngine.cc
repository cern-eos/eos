    ...
    vector<string> *forceGeoTags)
{
  ...
  if(dataProxys)
  {
    if(!findProxy(newReplicasIdx, entries, inode,dataProxys,NULL,pProxyCloseToFs?"":clientGeoTag,filesticky))
    {
      success = false;
      goto cleanup;
    }
  ...
  if(firewallEntryPoint)
  {
    ...
    for(size_t i=0; i<newReplicasIdx.size(); i++ )
    {
      if( clientGeoTag.empty() ||
          accessReqFwEP((*entries[i]->foregroundFastStruct->treeInfo)[newReplicasIdx[i]].fullGeotag , clientGeoTag ))
      {
        firewallProxyGroups[i]=accessGetProxygroup( (*entries[i]->foregroundFastStruct->treeInfo)[newReplicasIdx[i]].fullGeotag);
      }
    }
    ...
    if(!findProxy(newReplicasIdx, entries, inode, firewallEntryPoint,&firewallProxyGroups,pProxyCloseToFs?"":clientGeoTag,any))
    {
      success = false;
      goto cleanup;
    }
  ...
  if(dataProxys)
  {
    ...
    if(!findProxy(newReplicasIdx, entries, inode, dataProxys,NULL,pProxyCloseToFs?"":clientGeoTag,regular))
    {
      success = false;
      goto cleanup;
    }
    ...
    tProxySchedType proxyschedtype)
{
  ...
  for(size_t i = 0; i<fsIdxs.size(); i++)
  {
    ...
    fsproxygroup = & (*entries[i]->foregroundFastStruct->treeInfo)[fsIdxs[i]].proxygroup;
...
)
{
  ...
  if(dataProxys)
  {
    if(!findProxy(ERIdx, entries, inode, dataProxys,NULL,pProxyCloseToFs?"":accesserGeotag,filesticky))
    {
      returnCode = ENONET;
      goto cleanup;
    }
  ...
  if(firewallEntryPoint)
  {
    ...
    for(size_t i=0; i<ERIdx.size(); i++ )
    {
      if( accesserGeotag.empty() ||
          accessReqFwEP((*entries[i]->foregroundFastStruct->treeInfo)[ERIdx[i]].fullGeotag , accesserGeotag ))
      {
        firewallProxyGroups[i]=accessGetProxygroup( (*entries[i]->foregroundFastStruct->treeInfo)[ERIdx[i]].fullGeotag);
      }
    }

    if(dataProxys)
      *firewallEntryPoint=*dataProxys;
    if(!findProxy(ERIdx, entries, inode, firewallEntryPoint,&firewallProxyGroups,pProxyCloseToFs?"":accesserGeotag,any))
    {
      returnCode = ENONET;
      goto cleanup;
    }
  ...
  if(dataProxys)
  {
    if(firewallEntryPoint)
      *dataProxys=*firewallEntryPoint;
    if(!findProxy(ERIdx, entries, inode, dataProxys,NULL,pProxyCloseToFs?"":accesserGeotag,regular))
    {
      returnCode = ENONET;
      goto cleanup;
    }
...
#define unsetOneStateVarStatusInAllFastTrees(flag) \
  ...
  if(keys&sfgFileStickPxy)
  {
    if(stn)
    {
      stn->pNodeInfo.fileStickyProxyDepth = fs->mFileStickyProxyDepth;
    }
    if(ftIdx)
      (*entry->backgroundFastStruct->treeInfo)[ftIdx].fileStickyProxyDepth = fs->mFileStickyProxyDepth;
  }
...
std::string GeoTreeEngine::AccessStruct::getMappingStr() const
{
  ...
  {
    if(it != accessGeotagMap.begin()) ret.append(";");
    ret.append(it->first);
    ret.append("=>");
...
bool GeoTreeEngine::AccessStruct::setMapping (const std::string& mapping, bool setconfig)
{
  ...
  while(std::getline(ss,mappingelement,';'))
  {
    ...
    {
      eos_static_err("error parsing config entry while restoring config : %s",mappingelement.c_str());
      return false;
    }
  ...
  if(!geotag.empty())
    return setMapping(geotag,geotaglist,true,setconfig); // to rebuild the tree and set the config
  else
    return true;
}

bool GeoTreeEngine::AccessStruct::setMapping (const std::string& geotag, const std::string &geotaglist, bool updateFastStruct, bool setconfig)
{
  ...
  if(updateFastStruct) accessST->buildFastStrcturesAccess(accessFT,accessHost2Idx,accessFTI,accessTag2Idx);
...
bool GeoTreeEngine::AccessStruct::clearMapping (const std::string& geotag, bool updateFastStruct, bool setconfig)
{
  RWMutexWriteLock lock(accessMutex);
  ...
  if(inuse)
  {
    ...
    if(!geotag.empty() && !accessST->remove(&tni,false))
      return false;
    if(!geotag.empty()) accessGeotagMap.erase(geotag);
    ...
    else if(updateFastStruct) accessST->buildFastStrcturesAccess(accessFT,accessHost2Idx,accessFTI,accessTag2Idx);
  }
...
bool GeoTreeEngine::accessReqFwEP(const std::string &targetGeotag, const std::string &accesserGeotag) const
{
  // if no direct access geotag mapping is defined, all accesses are direct
  if(!pAccessGeotagMapping.inuse) return false;

  // first get the parent node giving the access rule
  auto idx  = pAccessGeotagMapping.accessTag2Idx->getClosestFastTreeNode(accesserGeotag.c_str());
  ...
  size_t beg = std::numeric_limits<size_t>::max (), end = std::numeric_limits<size_t>::max ();
  for (size_t i = 0; i < accessible.size (); i++)
  {
    if (accessible[i] == ',')
    {
      if(beg==std::numeric_limits<size_t>::max ()) continue;
      end = i;
      ...
      if(beg==std::numeric_limits<size_t>::max ()) beg = i;
  }
...
std::string GeoTreeEngine::accessGetProxygroup(const std::string &toAccess) const
{
  // if no access proxygroup mapping is defined, there is no proxygroup to return
  if(!pAccessProxygroup.inuse) return "";

  // first get the parent node giving the proxygroup
  auto idx  = pAccessProxygroup.accessTag2Idx->getClosestFastTreeNode(toAccess.c_str());
  SchedTreeBase::tFastTreeIdx idx2 = 0;
