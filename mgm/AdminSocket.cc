...
AdminSocket::Run(ThreadAssistant& assistant) noexcept {
  ...
  while (!assistant.terminationRequested()) {
    ...
    if (items[0].revents & ZMQ_POLLIN) {
      ...
      std::unique_ptr<IProcCommand> proccmd= ProcInterface::GetProcCommand("adminsocket@localhost",root_vid, input.c_str(), info.c_str(), "adminsocket");
      size_t size=0;
      std::string result;
      
      if (proccmd) {
	XrdOucErrInfo error;
	int rcode = proccmd->open(input.c_str() ,info.c_str() , root_vid, &error);
	struct stat buf;
	proccmd->stat(&buf);
	size = buf.st_size;
