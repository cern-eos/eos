...
grpc::Status GrpcWncInterface::File()
{
  ...
  default: {
    eos::console::RequestProto req = *mRequest;
    eos::mgm::FileCmd filecmd(std::move(req), *mVid);
    *mReply = filecmd.ProcessRequest();
    return grpc::Status::OK;
  }
  }
...
grpc::Status GrpcWncInterface::Fileinfo()
{
  ...
  if (mRequest->file().fileinfo().path() || mRequest->file().fileinfo().fid() ||
      mRequest->file().fileinfo().fxid() || mRequest->file().fileinfo().size() ||
      mRequest->file().fileinfo().checksum() || mRequest->file().fileinfo().fullpath() ||
      mRequest->file().fileinfo().proxy() || mRequest->file().fileinfo().monitoring() ||
      mRequest->file().fileinfo().wnc() || mRequest->file().fileinfo().env()) {
  ...
  if (mRequest->file().fileinfo().monitoring() || mRequest->file().fileinfo().wnc()) {
    cmd_in += "-m";
  }
...
{
  std::string path = mRequest->file().touch().md().path();
