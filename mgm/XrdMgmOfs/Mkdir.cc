                  ...
                  bool nopermissioncheck)
{
  ...
  if (!errno && mAudit) {
    ...
    mAudit->audit(eos::audit::MKDIR, apath, vid, std::string(logId), std::string(cident), "mgm", std::string(), nullptr, &afterStat, std::string(), std::string(), std::string(), __FILE__, __LINE__, VERSION);
  }
  return SFS_OK;
}
