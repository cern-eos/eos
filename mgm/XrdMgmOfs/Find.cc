		 ...
		 int max_ctime_file_min_deepness)
{
  ...
  do {
    ...
    for (unsigned int i = 0; i < found_dirs[deepness].size(); i++) {
      ...
      if (cmd && max_ctime_dir && (deepness >= max_ctime_dir_min_deepness) && (ctime.tv_sec > max_ctime_dir)) {
        ...
        if (!no_files) {
          ...
          for (auto fit = eos::FileMapIterator(cmd); fit.valid(); fit.next()) {
            ...
            if (fmd) {
              ...
              if (max_ctime_file && (deepness >= max_ctime_file_min_deepness) && (ctime.tv_sec > max_ctime_file)) {
