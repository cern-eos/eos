...

{
    ...
    // lock the view and get the filesystem information for the target where be drain to
  {
    ...
    for (size_t n = 0; n < group->size(); n++) {
      ...
      if ((eos::common::FileSystem::GetDrainStatusFromString(
             FsView::gFsView.mIdView[*group_iterator]->GetString("stat.drain").c_str()) !=
           eos::common::FileSystem::kDraining) &&
          (eos::common::FileSystem::GetDrainStatusFromString(
             FsView::gFsView.mIdView[*group_iterator]->GetString("stat.drain").c_str()) !=
           eos::common::FileSystem::kDrainStalling)) {
        source_fs = 0;
        group_iterator++;
          ...
          if (((eos::common::LayoutId::GetLayoutType(lid) ==
            ...
            if (((eos::common::LayoutId::GetLayoutType(lid) !=
              ...
              if (!acsargs.isValid()) {
                ...
                eos_thread_err("cmd=schedule2drain msg=\"invalid argument to FileAccess %llx retc=%d\"",
                               fid, retc);
                ...
                // inaccessible files we retry after 60 seconds
                eos_thread_err("cmd=schedule2drain msg=\"no access to file %llx retc=%d\"", fid,
                               retc);
                ScheduledToDrainFid[fid] = time(NULL) + 60;
                // try with next file
                fit++;
            ...
            if (size < freebytes) {
              ...
              replica_source_capability += eos::common::SecEntity::ToKey(0,
                                           "eos/draining").c_str();
