...
/*----------------------------------------------------------------------------*/
int
TransferFsDB::Ls(XrdOucString& sid, XrdOucString& option, XrdOucString& group,
    ...
    if (transfer["sync"] != "1") {
      ...
      if (!Archive(id, out, err, true)) {
        if (!Cancel(id, out, err, true)) {
          ...
          eos_static_err("failed to cancel id=%lld in auto-archiving after <done> state",
                         id);
          return false;
        }
      } else {
        eos_static_err("failed to archive id=%lld in auto-archiving after <done> state",
                       id);
        return false;
      }
    }
...
/*----------------------------------------------------------------------------*/
bool
TransferFsDB::SetCredential(long long id, std::string credential,
                            time_t exptime)
{
  XrdSysMutexHelper lock(Lock);
  XrdOucString query = "";
...
/*----------------------------------------------------------------------------*/
int
TransferFsDB::Submit(XrdOucString& src, XrdOucString& dst, XrdOucString& rate,
                     XrdOucString& streams, XrdOucString& group, XrdOucString& stdOut,
                     XrdOucString& stdErr, uid_t uid, gid_t gid, time_t exptime,
                     XrdOucString& credential, XrdOucString& submissionhost, bool sync, bool noauth)
{
  XrdSysMutexHelper lock(Lock);
  Qr.clear();
  XrdOucString insert = "";
...
/*----------------------------------------------------------------------------*/
int
TransferFsDB::Archive(long long id, XrdOucString& stdOut, XrdOucString& stdErr,
                      bool nolock)
{
  if (!nolock) {
    Lock.Lock();
  ...
  if (Qr.size()) {
    ...
    if ((fprintf(fdArchive,
                 "# id=%s uid=%s gid=%s group=%s rate=%s streams=%s state=%s\n",
                 Qr[0]["id"].c_str(), Qr[0]["uid"].c_str(), Qr[0]["gid"].c_str(),
                 Qr[0]["groupname"].c_str(), Qr[0]["rate"].c_str(), Qr[0]["streams"].c_str(),
                 Qr[0]["status"].c_str())) < 0) {
      ferror = true;
    }

    if ((fprintf(fdArchive, "# executionhost=%s\n",
                 Qr[0]["exechost"].c_str())) < 0) {
      ferror = true;
    }

    if ((fprintf(fdArchive, "# submissionhost=%s\n",
                 Qr[0]["submissionhost"].c_str())) < 0) {
      ferror = true;
    }
