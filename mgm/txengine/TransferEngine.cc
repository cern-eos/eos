...
{
  int retc = 0;
  std::string scheduling = FsView::gFsView.GetGlobalConfig(
                             TransferEngine::gConfigSchedule);
                       ...
                       time_t exptime, XrdOucString credentials, bool sync, bool noauth)
{
  ...
  return xDB->Submit(src, dst, rate, streams, group, stdOut, stdErr, vid.uid,
                     vid.gid, time(NULL) + exptime, credentials, submissionhost, sync, noauth);
}
                    ...
                    eos::common::VirtualIdentity& vid)
{
  ...
  if (transfer.count("log")) {
    ...
    stdErr += "error: there is no log available for id=";
    stdErr += sid;
    stdErr += "\n";
    return EINVAL;
  }
    ...
    if (transfer.count("uid")) {
      ...
      if (((!vid.uid) || ((int)vid.uid == atoi(transfer["uid"].c_str())))) {
        ...
        if (!group.length() && (id)) {
          ...
          stdOut += "warning: skipping transfer id=";
          stdOut += transfer["id"].c_str();
          stdOut += " - you are not the owner!\n";
        }
      }
    }
  }
  ...
  for (size_t i = 0; i < ids.size(); i++) {
    ...
    if (transfer.count("id") && transfer.count("uid")) {
      ...
      if (transfer["status"] == "failed") {
        ...
        stdOut += "success: resubmitted transfer id=";
        stdOut += transfer["id"].c_str();
        stdOut += "\n";
      } else {
        if (transfer["status"] == "done") {
          if (!group.length()) {
            stdErr += "error: cannot resubmit <done> transfer with id=";
            stdErr += transfer["id"].c_str();
            stdErr += "\n";
            return EINVAL;
          }
        }
      ...
      stdErr += "error: cannot get a transfer with id=";
      stdErr += transfer["id"].c_str();
      stdErr += "\n";
    }
  }
  ...
  for (size_t i = 0; i < ids.size(); i++) {
    ...
    if (transfer.count("uid")) {
      ...
      if (id) {
        ...
        stdOut += "success: reset transfer id=";
        stdOut += transfer["id"].c_str();
      }
    }
  }
...
TransferEngine::Scheduler(ThreadAssistant& assistant) noexcept
{
  ...
  while (!assistant.terminationRequested()) {
    // schedule here
    {
      ...
      if (transfer.count("error")) {
        eos_static_debug("GetNextTransfer(kInserted) returned %s",
                         transfer["error"].c_str());
      } else {
        if (transfer.count("id")) {
          ...
          if (transfer.count("id")) {
            ...
            if (src_tok.size() && dst_tok.size()) {
              transferjob    = "source.url=";
              transferjob += src_tok[0].c_str();
              ...
              transferjob  += "&tx.id=";
              transferjob += transfer["id"].c_str();
              transferjob  += "&tx.streams=";
              transferjob += transfer["streams"].c_str();
              transferjob  += "&tx.rate=";
              transferjob += transfer["rate"].c_str();
              transferjob  += "&tx.exp=";
              transferjob += transfer["expires"].c_str();
              transferjob  += "&tx.uid=";
              transferjob += transfer["uid"].c_str();
              transferjob  += "&tx.gid=";
              transferjob += transfer["gid"].c_str();
              transferjob  += "&tx.noauth=";
              transferjob += transfer["noauth"].c_str();
              fprintf(stderr, "%s\n", transferjob.c_str());
