...
public:
    ...
    std::map<long long, unsigned long long>::const_iterator Begin() {
        return Quota.begin();
    }

    std::map<long long, unsigned long long>::const_iterator End() {
        return Quota.end();
    }
    ...
    void SetPhysicalTmpFreeBytes(unsigned long long bytes) {
        PhysicalTmpFreeBytes = bytes;
    }

    void SetPhysicalTmpFreeFiles(unsigned long long files) {
        PhysicalTmpFreeFiles = files;
    }

    void SetPhysicalFreeBytes(unsigned long long bytes) {
        PhysicalFreeBytes = bytes;
    }

    void SetPhysicalFreeFiles(unsigned long long files) {
        PhysicalFreeFiles = files;
    }

    void SetPhysicalTmpMaxBytes(unsigned long long bytes) {
        PhysicalTmpMaxBytes = bytes;
    }

    void SetPhysicalTmpMaxFiles(unsigned long long files) {
        PhysicalTmpMaxFiles = files;
    }

    void SetPhysicalMaxBytes(unsigned long long bytes) {
        PhysicalMaxBytes = bytes;
    }

    void SetPhysicalMaxFiles(unsigned long long files) {
        PhysicalMaxFiles = files;
    }

    void ResetPhysicalFreeBytes() {
        SetPhysicalFreeBytes(0);
    }

    void ResetPhysicalFreeFiles() {
        SetPhysicalFreeFiles(0);
    }

    void ResetPhysicalMaxBytes() {
        SetPhysicalMaxBytes(0);
    }

    void ResetPhysicalMaxFiles() {
        SetPhysicalMaxFiles(0);
    }

    void ResetPhysicalTmpFreeBytes() {
        SetPhysicalTmpFreeBytes(0);
    }

    void ResetPhysicalTmpFreeFiles() {
        SetPhysicalTmpFreeFiles(0);
    }

    void ResetPhysicalTmpMaxBytes() {
        SetPhysicalTmpMaxBytes(0);
    }

    void ResetPhysicalTmpMaxFiles() {
        SetPhysicalTmpMaxFiles(0);
    }

    void AddPhysicalFreeBytes(unsigned long long bytes) {
        PhysicalFreeBytes += bytes;
    }

    void AddPhysicalFreeFiles(unsigned long long files) {
        PhysicalFreeFiles += files;
    }

    void AddPhysicalMaxBytes(unsigned long long bytes) {
        PhysicalMaxBytes += bytes;
    }

    void AddPhysicalMaxFiles(unsigned long long files) {
        PhysicalMaxFiles += files;
    }

    void AddPhysicalTmpFreeBytes(unsigned long long bytes) {
        PhysicalTmpFreeBytes += bytes;
    }

    void AddPhysicalTmpFreeFiles(unsigned long long files) {
        PhysicalTmpFreeFiles += files;
    }

    void AddPhysicalTmpMaxBytes(unsigned long long bytes) {
        PhysicalTmpMaxBytes += bytes;
    }

    void AddPhysicalTmpMaxFiles(unsigned long long files) {
        PhysicalTmpMaxFiles += files;
    }

    void PhysicalTmpToFreeBytes() {
        PhysicalFreeBytes = PhysicalTmpFreeBytes;
    }

    void PhysicalTmpToFreeFiles() {
        PhysicalFreeFiles = PhysicalTmpFreeFiles;
    }

    void PhysicalTmpToMaxBytes() {
        PhysicalMaxBytes = PhysicalTmpMaxBytes;
    }

    void PhysicalTmpToMaxFiles() {
        PhysicalMaxFiles = PhysicalTmpMaxFiles;
    }

    unsigned long long GetPhysicalFreeBytes() {
        return PhysicalFreeBytes;
    }

    unsigned long long GetPhysicalFreeFiles() {
        return PhysicalFreeFiles;
    }

    unsigned long long GetPhysicalMaxBytes() {
        return PhysicalMaxBytes;
    }

    unsigned long long GetPhysicalMaxFiles() {
