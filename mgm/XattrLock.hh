...
public:
  ...
  bool Lock(const char* path, bool shrd, time_t lifetime, eos::common::VirtualIdentity& vid, bool userwildcard, bool appwildcard) {
    ...
    XrdOucErrInfo error;
    XrdOucString value;
    if (!gOFS->_attr_get(path, error, vid, "", eos::common::EOS_APP_LOCK_ATTR, value, false)) {
      // parse the lock
      Parse(value.c_str());
    ...
    if (gOFS->_attr_set(path, error, vid, "", eos::common::EOS_APP_LOCK_ATTR, Value().c_str(), false, false)) {
      return false;
    }
    return true;
  }
  ...
  bool Unlock(const char* path, eos::common::VirtualIdentity& vid) { 
    ...
    if (gOFS->_attr_rem(path, error, vid, "", eos::common::EOS_APP_LOCK_ATTR, false)) {
      return false;
    }
    return true;
  }
  ...
  std::string Dump() {
    ...
    ss << "valid:" << valid << " expires:" << expires << " shared:" << isshared << std::endl;
    return ss.str();
  }
