...
ProcCommand::open(const char* inpath, const char* ininfo, eos::common::Mapping::VirtualIdentity &vid_in, XrdOucErrInfo   *error) 
{
  ...
  if (adminCmd) {
...
#endif
      if ( (cmd == "ns" ) && (subcmd != "mutex") && (subcmd != "compact") ) {
	XrdOucString option = opaque.Get("mgm.option");
	bool details=false;
	bool monitoring=false;
	...
	if (!monitoring) {
	  ...
	  gOFS->MgmMaster.PrintOutCompacting(stdOut); stdOut += "\n";
	  stdOut+="# ....................................................................................\n";
	  ...
	  stdOut += "uid=all gid=all "; gOFS->MgmMaster.PrintOutCompacting(stdOut); stdOut += "\n";
	...
	if (vid_in.uid==0) {
	  ...
	  if ( (!action.length()) ) {
		...
		stdOut += "success: configured online compacting to run in "; stdOut += delay.c_str(); stdOut += " seconds from now ( might be delayed upto 60 seconds )";
		if (interval != "0") {
		  stdOut += " (re-compact every "; stdOut += interval; stdOut += " seconds)\n";
		} else {
		  stdOut += "\n";
		}
	      } 
