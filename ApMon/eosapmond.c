...
int main(int argc, char* argv[])
{
  if (argc != 7) {
    fprintf(stderr,
            "ERROR: missing arguments to run: eos_apmond <script> <logfile> <monalisa-host> <apmon-log-level> <cluster-name> <host-name> [%d args given]\n",
            argc);
    exit(-1);
  }
  ...
  execlp("/usr/bin/perl", "eos_apmond", argv[1], argv[2], argv[3], argv[4],
         argv[5], argv[6], NULL);
}
