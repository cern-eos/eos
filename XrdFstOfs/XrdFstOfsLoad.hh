#ifndef __XRDFSTOFS_LOAD_HH__
#define __XRDFSTOFS_LOAD_HH__
...
public:
  ...
  virtual ~XrdFstOfsLoad_DiskStat(){}
};

class XrdFstOfsLoad_NetStat {
private:
  // map from device name to map of key/values
  std::map<std::string, std::map<std::string, std::string > > values_t2;
  std::map<std::string, std::map<std::string, std::string > > values_t1;
...
public:
  ...
  void SetBandWidth(unsigned long long bw) {maxbandwidth = bw;}

  XrdFstOfsLoad_NetStat(){
...
private:
  ...
  pthread_t tid;
  XrdSysMutex Mutex;
  XrdFstOfsLoad_DiskStat DiskStat;
  XrdFstOfsLoad_NetStat NetStat;
  unsigned int interval;
    
public:
  XrdFstOfsLoad(unsigned int ival=5) {
    tid=0;
    interval = ival;
    if (interval==0)
      interval = 1;
  ...
  void SetBandWidth(unsigned long long bw) {NetStat.SetBandWidth(bw);}

  bool Monitor() {
    int rc=0;
    
    if ((rc = XrdSysThread::Run(&tid, XrdFstOfsLoad::StartLoadThread, static_cast<void *>(this),
				0, "Scrubber"))) {
      return false;
    } else {
      return true;
  ...
  double GetDiskRate(const char* devpath, const char* tag) { 
    ...
    Mutex.Lock();
    double val = DiskStat.GetRate(dev,tag);
    Mutex.UnLock();
    return val;
  }

  double GetDiskUtilization()         {return 0;}
  double GetNetRate(const char* dev, const char* tag)  { 
    Mutex.Lock();
    double val = NetStat.GetRate(dev,tag);
    Mutex.UnLock();
    return val;
  }

  static void* StartLoadThread(void *pp) {
    XrdFstOfsLoad* load = (XrdFstOfsLoad*)pp;
    load->Measure();
    return 0;
  }
};
