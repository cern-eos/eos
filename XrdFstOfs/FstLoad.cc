#include "XrdFstOfs/XrdFstOfsLoad.hh"


int main() {
  
  XrdFstOfsLoad load(1);
  load.Monitor();
  ...
  while(1) {
    ...
    printf("rd %.02f MB/s \twd %.02f MB/s\n", load.GetDiskRate("/tmp/","readSectors")*512.0/1000000.0, load.GetDiskRate("/tmp/","writeSectors")*512.0/1000000.0);
    sleep(1);
  }
