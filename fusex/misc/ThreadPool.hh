...
class ThreadPool
{
  public:
    ...
    ThreadPool( int min, int max ) : min( min ), max( max ), active( true ), busy( 0 ), idle( 0 ) { }
    ...
    void Stop()
    {
      ...
      for( itr = threads.begin(); itr != threads.end(); ++itr )
      {
        ...
        {
          if( rc == ESRCH ) continue; // the thread doesn't exist
        ...
        {
          if( rc == EINVAL || rc == ESRCH ) continue; // the thread doesn't exist or is not joinable
          throw FuseException( rc );
        }
      }
