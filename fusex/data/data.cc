...
/* -------------------------------------------------------------------------- */
{
  ...
  if (isRW) {
    ...
    if (!mFile->xrdioro(freq) || mFile->xrdioro(freq)->IsClosing() ||
...
/* -------------------------------------------------------------------------- */
{
  ...
  if (!mPrefetchHandler && mFile->file() && !mFile->file()->size() &&
      mMd->size()) {
    XrdCl::Proxy* proxy = mFile->xrdioro(req) ? mFile->xrdioro(
  ...
  if (proxy) {
    ...
    if (proxy->Read(offset + br ,
      ...
      if (mFile->journal()) {
        ...
        chunks = ((journalcache*)(mFile->journal()))->get_chunks(offset + br + jr,
                 count - br - jr);
