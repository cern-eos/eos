  ...
  if (XReadAheadStrategy != NONE) {
    ...
    if (ChunkRMap().size()) {
      ...
      for (auto it = ChunkRMap().begin(); it != ChunkRMap().end(); ++it) {
        off_t match_offset;
        uint32_t match_size;
        eos_debug("----: eval offset=%lu chunk-offset=%lu", offset,
                  it->second->offset());
        ...
        if (it->second->matches(current_offset, current_size, match_offset,
          ...
          eos_debug("----: considering chunk address=%lx offset=%ld", it->first,
                    it->second->offset());

          if (!it->second->successor(offset, size)) {
            eos_debug("----: delete chunk address=%lx offset=%ld", it->first,
                      it->second->offset());
    ...
    if (request_next) {
      ...
      off_t align_offset = aligned_offset(ChunkRMap().size() ? offset +
                                          XReadAheadNom : offset);
      ...
      if (ChunkRMap().count(align_offset)) {
        ...
        ReadCondVar().UnLock();
        XrdCl::Proxy::read_handler rahread = ReadAsyncPrepare(align_offset,
                                             XReadAheadNom);
