...
/* -------------------------------------------------------------------------- */
{
  ...
  snprintf(systemline, sizeof(systemline), "umount -fl %s >& /dev/null; fusermount -u -z %s >& /dev/null",
           fs.source.c_str(),
	   fs.mount.c_str()
	   );
...
static void sfs_rmdir(fuse_req_t req, fuse_ino_t parent, const char *name) {
  ...
  auto res =
    (fs.recyclebin && fs.recycle.shouldRecycle(fsid.getUid(), parent, inode_p.fd, name)) ?
...
static void sfs_getxattr(fuse_req_t req, fuse_ino_t ino, const char *name,
  ...
  FsID fsid(req);
  std::string vattr = cfsvattr::vattr(name, fsid.getName(), fsid.getUid(), fsid.getGid(), fs.quota.hasquota(fsid.getUid(),fsid.getGid()));
  if (!vattr.empty()) {
...
#include "keychange.hh"
  ...
  if (buf.st_uid || ((buf.st_mode&0x1ff) != S_IRUSR) ) {
    fprintf(stderr,"error: %sdoes not have correct ownership (root) or 400 permission! [%d/%d/%x/%x]\n", keylocation.c_str(), buf.st_uid, buf.st_mode, buf.st_mode, S_IRUSR);
    exit(-1);
  }
...
Fs::DumpStatistic(ThreadAssistant& assistant)
{
  ...
  while (!assistant.terminationRequested()) {
	...
	fprintf(stderr,"error: failed to rename <json> stats file from temporary to final name '%s'=>'%s'\n",
		tmpjsonfile.c_str(),
		fs.jsonpath.c_str());
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    fprintf(stderr,"# warning: couldn't/didn't parse /etc/eos/cfsd/eoscfsd.conf\n");    
  }
