...
metad::forget(fuse_req_t req, fuse_ino_t ino, int nlookup)
{
  ...
  if (has_flush(ino)) {
    ...
    eos_static_debug("flush - ino=%#lx", ino);
    return 0;
  }

  if (pmd->cap_count()) {
    stat.inodes_stacked_inc();
    eos_static_debug("caps %d - ino=%#lx", pmd->cap_count(), ino);
    return 0;
  }

  if (pmd->opendir_is()) {
    stat.inodes_stacked_inc();
    eos_static_debug("opendir %d - ino=%#lx", pmd->opendir_is(), ino);
    return 0;
  }
...
metad::mdcommunicate(ThreadAssistant& assistant)
{
  ...
  hb.mutable_heartbeat_()->set_automounted(EosFuse::Instance().Config().options.automounted);
