...
/* -------------------------------------------------------------------------- */
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
        eos_static_info("returning cap entry via parent lookup");
        eos_static_debug("MD:\n%s", dump_md(md).c_str());
        return md;
      }
    }
...
/* -------------------------------------------------------------------------- */
{
  ...
  if (p1md->id() != p2md->id())
  {
    ...
    p1md->setop_localstore();
    p2md->setop_localstore();
    p1md->setop_update();
    p2md->setop_update();
  ...
  {
    {
      ...
      std::string authid = it->second;
      mdqueue.erase(it->first);
      mdflush.UnLock();
      {
        shared_md md;
        {
