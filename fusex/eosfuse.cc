 ...
 */
{
  ...
  if (!fi->fh) {
    ...
    // make sure the directory offset does not exceed the size of the child map
    if ((size_t) (in_off > 1) ? (in_off - 2) : 0 < md->pmd_children.size()) {
    ...
    eos_static_info("size=%lu off=%llu reply-size=%lu loop=%lu\n",
                    size, off, b_size, loop);
  }
