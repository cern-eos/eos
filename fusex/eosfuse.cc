...
#ifndef USE_FUSE3
  sprintf(systemline, "fusermount -u -z %s", EosFuse::Instance().Config().localmountdir.c_str());;
#else
  sprintf(systemline, "fusermount3 -u -z %s", EosFuse::Instance().Config().localmountdir.c_str());;
#endif
  system(systemline);
