                          ...
                          const std::string& fileLog)
{
  ...
  ((eos::ChangeLogFileMDSvc*)fileSvc)->setContMDService((
...
//------------------------------------------------------------------------------
void PrintStatus(eos::IView* view, const char* f2, const char* f1,
                 eos::common::LinuxStat::linux_stat_t& st1,
...
public:
  RThread():
    i(0), n_j(0), n_k(0), n_files(0), dolock(false), view(0) {};
...
int main(int argc, char** argv)
{
  ...
  if (argc != 5) {
    ...
    std::cerr <<
              "  eos-namespace-benchmark directory.log file.log <level1-dirs> <level3-files> "
              << std::endl;
    return 1;
  };
    ...
    for (size_t i = 0; i < n_i; i++) {
      ...
      for (size_t j = 0; j < n_j; j++) {
        for (size_t k = 0; k < n_k; k++) {
          ...
          std::shared_ptr<eos::IContainerMD> cont = view->createContainer(container_path,
              true);
