...
public:
  ...
  DsiRcvResponseHandler(globus_l_gfs_xrood_handle_s *handle) :
    AsyncMetaHandler(),
    mNumRegRead(0) , mNumCbRead(0),
    mNumRegWrite(0),mNumCbWrite(0),
    mHandle(handle),
    mAllBufferMet(false),
    mOver(false),
   ...
   */
  size_t GetActiveCount() const { return mActiveBufferSet.size(); }
   ...
   */
  size_t GetBufferCount() const { return mMetBufferSet.size(); }
  ...
  bool IsOver() const {
    return
        (GetActiveCount()==0)
        && (GetBufferCount()!=0)
        && ((int)GetBufferCount()==mNumExpectedBuffers)
        && (mNumCbRead==mNumRegRead)
      ...
      ChunkHandler* chunk)
  {
    ...
    if(pStatus->IsError()) { // if there is a xrootd write error
      if(mHandle->cached_res==GLOBUS_SUCCESS) { //if it's the first error
        globus_gfs_log_message(GLOBUS_GFS_LOG_ERR,
            "%s: XRootd write issued an error response : %s \n",
            func,pStatus->ToStr().c_str());
        mHandle->cached_res =
            globus_l_gfs_make_error(pStatus->ToStr().c_str(),pStatus->errNo);
      ...
      if(spawn && (mHandle->done==GLOBUS_FALSE) )
      {
        ...
        globus_result_t result = globus_gridftp_server_register_read(
            mHandle->op,
            buffer,
            mHandle->block_size,
        ...
        if(result != GLOBUS_SUCCESS)  {
          ...
          (GLOBUS_GFS_LOG_ERR,
              "%s: register Globus read has finished with a bad result \n",
              func);
      ...
      else { // if not spawning, delete the buffer
        DisableBuffer(buffer);
      }
    }
...
public:
  ...
  void DisableBuffer(globus_byte_t* buffer) {
    ...
    if(!mAllBufferMet) { // check the expected number of buffers
      mMetBufferSet.insert( buffer);
      // to cope with the fact that a buffer might be unregistered without having being used in any callback (typically small files)
      if((int)mMetBufferSet.size()==mNumExpectedBuffers) {
        mAllBufferMet=true;
   ...
   */
  void HandleResponse (bool isErr, uint32_t errNo, uint64_t offset, uint32_t len, uint32_t rlen, XrdCl::XRootDStatus* pStatus=0,ChunkHandler* chunk=0)
  {
    mNumCbRead++;
    ...
    size_t nbread=rlen;
    if(isErr && errNo!=EFAULT) { // if there is a xrootd read error which is not bad (offset,len)
      if(mHandle->cached_res==GLOBUS_SUCCESS) { //if it's the first one
        globus_gfs_log_message(GLOBUS_GFS_LOG_ERR,
            "%s: XRootd read issued an error response : %s \n",
            func,pStatus->ToStr().c_str());
        mHandle->cached_res =
            globus_l_gfs_make_error(pStatus->ToStr().c_str(),pStatus->errNo);
        mHandle->done = GLOBUS_TRUE;
    ...
    else if( isErr && errNo==EFAULT && nbread==0) { // if there is a bad (offset,len) error with an empty response
      DisableBuffer(buffer);
      mHandle->done = GLOBUS_TRUE;
    }
    else { // if there is no error or if bad (offset,len) but non empty response
      ...
      globus_result_t result = globus_gridftp_server_register_write(
          mHandle->op,
          buffer,
          nbread,
          offset, // !! this value doesn't matter
          -1,
      ...
      if(result != GLOBUS_SUCCESS)  {
        globus_gfs_log_message
        (GLOBUS_GFS_LOG_ERR,
            "%s: register Globus write has finished with a bad result \n",
            func);
    ...
    if(pStatus!=0 && chunk!=0) AsyncMetaHandler::HandleResponse(pStatus,chunk); // MUST be called AFTER the actual processing of the inheriting class
    else
      mNumReceivedResp++;
    SignalIfOver();
    pthread_mutex_unlock(&mHandle->mutex);
  ...
  static void* RunHandleResp (void* handleRespStruct) {
  ...
  void HandleResponseAsync(bool isErr, uint32_t errNo, uint64_t offset, uint32_t len, uint32_t rlen) {
    ...
    hrs->_this=this; hrs->isErr=isErr; hrs->errNo=errNo; hrs->offset=offset; hrs->len=len; hrs->rlen=rlen;
    pthread_t thread;
    mNumExpectedResp++;
...
int next_read_chunk(globus_l_gfs_xrood_handle_s *xrootd_handle, int64_t &nextreadl){
  ...
  if (xrootd_handle->blk_length == 0) { // for initialization and next block
    ...
    if (xrootd_handle->blk_length != -1) {
      ...
      xrootd_handle->blk_offset += ( (xrootd_handle->blk_length>=(globus_off_t)xrootd_handle->block_size)?(globus_off_t)xrootd_handle->block_size:(globus_off_t)xrootd_handle->blk_length );
      xrootd_handle->blk_length -= xrootd_handle->blk_length>=(globus_off_t)xrootd_handle->block_size?(globus_off_t)xrootd_handle->block_size:(globus_off_t)xrootd_handle->blk_length;
    }
    else {
...
globus_l_gfs_xrootd_start(globus_gfs_operation_t op,
  ...
  if(!xrootd_handle) GlobusGFSErrorMemory("xroot_handle");
  try {
...
(char *path, int flags, int mode,
  ...
  try {
    ...
    st=xrootd_handle->fileIo->Open(myPath, (XrdCl::OpenFlags::Flags)XrootStatUtils::mapFlagsPos2Xrd(flags), (XrdCl::Access::Mode)XrootStatUtils::mapModePos2Xrd(mode));

    if ( !st.IsOK()) {
      globus_gfs_log_message(GLOBUS_GFS_LOG_ERR,
          "%s: XrdCl::File::Open error : %s\n",
          func, st.ToStr().c_str());
    }
  } catch (...) {
    globus_gfs_log_message(GLOBUS_GFS_LOG_ERR,
...
static void globus_l_gfs_file_net_read_cb(globus_gfs_operation_t op,
  ...
  if( (result != GLOBUS_SUCCESS) || (nbytes==0) ) {
    ...
    if(ret<0) {
      ...
      globus_gfs_log_message
      (GLOBUS_GFS_LOG_ERR,
          "%s: register XRootD write has finished with a bad result \n",
          func);
      xrootd_handle->cached_res = globus_l_gfs_make_error("Error registering XRootD write",result);
      xrootd_handle->done = GLOBUS_TRUE;
      RcvRespHandler->DisableBuffer(buffer);
    }
    else {
...
(globus_l_gfs_xrootd_handle_t *xrootd_handle) {
  ...
  for(c=0;c<xrootd_handle->optimal_count;c++) {
    ...
    if(result != GLOBUS_SUCCESS)  {
      ...
      (GLOBUS_GFS_LOG_ERR,
          "%s: register Globus read has finished with a bad result \n",
          func);
      xrootd_handle->cached_res = globus_l_gfs_make_error("Error registering globus read",result);
      xrootd_handle->done = GLOBUS_TRUE;
      pthread_mutex_unlock(&xrootd_handle->mutex);
  ...
  if(buffers) {
    for(int c=0;c<xrootd_handle->optimal_count;c++)
      if(buffers[c]) globus_free(buffers[c]);
    globus_free(buffers);
  }
  pthread_mutex_unlock(&xrootd_handle->mutex);
  return;
...
/* receive from client */
void
globus_l_gfs_net_write_cb_lock(globus_gfs_operation_t op,
    globus_result_t result,
    globus_byte_t *buffer,
    globus_size_t nbwrite,
  ...
  if( (result != GLOBUS_SUCCESS) ) { // if the write failed, we are done the buffer is not needed anymore
    if(xrootd_handle->cached_res != GLOBUS_SUCCESS) { // don't overwrite the first error
      xrootd_handle->cached_res = result;
      xrootd_handle->done = GLOBUS_TRUE;
    }
  ...
  if(nbwrite==0) // don't update on the first call
    globus_gridftp_server_update_bytes_written(xrootd_handle->op,SendRespHandler->mRevBufferMap[buffer].first,SendRespHandler->mRevBufferMap[buffer].second);

  if(xrootd_handle->done == GLOBUS_FALSE) { // if we are not done, look for something else to copy
    if(next_read_chunk(xrootd_handle,read_length)) { // if return is non zero, no more source to copy from
      xrootd_handle->cached_res = GLOBUS_SUCCESS;
      xrootd_handle->done = GLOBUS_TRUE;
      SendRespHandler->DisableBuffer(buffer);
      SendRespHandler->SignalIfOver();
    ...
    globus_gfs_log_message
    (GLOBUS_GFS_LOG_DUMP,
        "%s: register XRootD read from globus_l_gfs_net_write_cb \n",
        func);
    ...
    if(nbread<0) {
      globus_gfs_log_message
      (GLOBUS_GFS_LOG_ERR,
          "%s: register XRootD read has finished with a bad result %d\n",
          func,nbread);
    ...
    else if(nbread==0) { // empty read, EOF is reached
      xrootd_handle->done = GLOBUS_TRUE;
      SendRespHandler->DisableBuffer(buffer);
      SendRespHandler->SignalIfOver();
    }
      ...
      if(usedReadCallBack) globus_gfs_log_message
          (GLOBUS_GFS_LOG_DUMP,"%s: register XRootD read from globus_l_gfs_net_write_cb ==> usedReadCallBack\n",func);
    }
  }
  else { // we are done, just drop the buffer
    SendRespHandler->DisableBuffer(buffer);
    SendRespHandler->SignalIfOver();
    if(lock) pthread_mutex_unlock(&xrootd_handle->mutex);
    return ;
  ...
  if((!usedReadCallBack) && (nbread>0)) {
    ...
    globus_gfs_log_message
    (GLOBUS_GFS_LOG_DUMP,"%s: %p register XRootD read from globus_l_gfs_net_write_cb ==> Explicit Callback %d %d\n",func,buffer,(int)read_length,(int)nbread);
  }
  if(lock) pthread_mutex_unlock(&xrootd_handle->mutex);
}
...
globus_l_gfs_net_write_cb(globus_gfs_operation_t op,
    ...
    globus_size_t nbwrite,
    void * user_arg) {
  globus_l_gfs_net_write_cb_lock(op,
      result,
      buffer,
      nbwrite,
      user_arg, true);
