...
void usage()
{
  ...
  fprintf(stderr, "                      [Fsck] [ERROR] [ MISSING ] fsid:1 cxid:00000006 fxid:0013da3f ... file was suppossed to be here, but is missing on disk\n");
...
ParseMgmConfig() {
  ...
  while (std::getline(inFile, dumpentry)) {
    if (dumpentry.substr(0, std::string("EOS_MGM_ALIAS=").length()) == "EOS_MGM_ALIAS=") {
...
ParseQdbConfig(eos::QdbContactDetails& qdb) {
  ...
  while (std::getline(inFile, dumpentry)) {
    ...
    if (dumpentry.substr(0, std::string("fstofs.qdbcluster").length()) == "fstofs.qdbcluster") {
      qdbcluster = dumpentry.substr(std::string("fstofs.qdbcluster").length());
    }
    if (dumpentry.substr(0, std::string("fstofs.qdbpassword_file").length()) == "fstofs.qdbpassword_file") {
      qdbpassword_file = dumpentry.substr(std::string("fstofs.qdbpassword_file").length());
    }
  }
  ...
  return ( qdb.members.parse(qdbcluster) && eos::common::PasswordHandler::readPasswordFile(qdbpassword_file, qdb.password) );
}
...
main(int argc, char* argv[])
{
  ...
  eos::fst::Fsck* sd = new eos::fst::Fsck(dirName.c_str(), strtol(sfsid.c_str(),0,10), &fstLoad,
