...
static void
logexcept (const eos::common::LogId* lid, const kio::LoggingException& e)
{
...
int
KineticIo::Open (const std::string& path, XrdSfsFileOpenMode flags,
                 mode_t mode, const std::string& opaque, uint16_t timeout)
{
  ...
  try
  {
    ...
    eos_debug("msg=\"opened\" path=\"%s\"", path.c_str());
    return SFS_OK;
  }
  KIO_CATCH
  return SFS_ERROR;
...
int64_t
KineticIo::Read (XrdSfsFileOffset offset, char* buffer, XrdSfsXferSize length,
                 uint16_t timeout)
{
...
int64_t
KineticIo::Write (XrdSfsFileOffset offset, const char* buffer,
...
int64_t
KineticIo::ReadAsync (XrdSfsFileOffset offset, char* buffer,
...
int64_t
KineticIo::WriteAsync (XrdSfsFileOffset offset, const char* buffer,
                       XrdSfsXferSize length, uint16_t timeout)
{
...
int
KineticIo::Fdeallocate (XrdSfsFileOffset fromOffset, XrdSfsFileOffset toOffset)
{
...
int
KineticIo::Statfs (const char* path, struct statfs* statFs)
{
...
int
KineticIo::Delete (const char* path)
{
  eos_debug("path: %s", path);
  try
  {
...
int
KineticIo::Exists (const char* path)
{
  eos_debug("path: %s", path);
  try
  {
...
FileIo::FtsHandle*
KineticIo::ftsOpen (std::string subtree)
{
  ...
  try
  {
    handle->mHandle = kio->ftsOpen(std::move(subtree));
...
bool
KineticIo::Attr::Set (const char* name, const char* value, size_t len)
{
...
bool
KineticIo::Attr::Set (std::string key, std::string value)
{
...
bool
KineticIo::Attr::Get (const char* name, char* value, size_t &size)
{
  bool rv = false;
  try
  {
...
std::string
KineticIo::Attr::Get (std::string name)
{
  std::string rv = "";
  try
  {
...
KineticIo::Attr*
KineticIo::Attr::OpenAttr (const char* path)
{
  ...
  {
    eos_static_err(e.what());
  }
  return NULL;
}

KineticIo::Attr*
KineticIo::Attr::OpenAttribute (const char* path)
{
