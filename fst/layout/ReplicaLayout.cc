                     ...
                     const char*                opaque )
{
  ...
  if ( index ) {
    ...
    if ( ( replicaIndex < 0 ) || ( replicaIndex > eos::common::LayoutId::kSixteenStripe ) ) {
      ...
      return gOFS.Emsg( "ReplicaOpen", *mError, EINVAL, "open replica - illegal replica index found", index );
    }
  ...
  if ( replicaIndex < nReplica ) {
    if ( !rep ) {
      ...
      return gOFS.Emsg( "ReplicaOpen", *mError, EINVAL, "open replica - missing url for replica ", reptag.c_str() );
    }
  }
  ...
  if ( !mOfsFile->isRW ) {
    ...
    if ( ioLocal ) {
      // read from this box
      return mOfsFile->openofs( path, open_mode, create_mode, client, opaque );
    } else {
      // read from remote box
      // create remote url
      replicaUrl += "?";
      replicaUrl += remoteOpenOpaque;
      // open next replicas
      ...
      if ( !replicaClient->Open( 0, 0, false ) ) {
        ...
        return gOFS.Emsg( "ReplicaOpen", *mError, EREMOTEIO, "open replica - remote open failed ", replicaUrl.c_str() );
      }
    }
...
ReplicaLayout::read( XrdSfsFileOffset offset, char* buffer, XrdSfsXferSize length )
{
  ...
  if ( ioLocal )
    ...
    if ( !rc ) {
      ...
      return gOFS.Emsg( "ReplicaRead", *mError, EREMOTEIO, "read remote replica - read failed", replicaUrl.c_str() );
    }
...
ReplicaLayout::write( XrdSfsFileOffset offset, char* buffer, XrdSfsXferSize length )
{
  ...
  if ( rc1 < 0 ) {
    ...
    return gOFS.Emsg( "ReplicaWrite", *mError, errno, "write local replica - write failed", replicaUrl.c_str() );
  }

  if ( !rc2 ) {
    eos_err( "Failed to write remote replica - write failed - %llu %llu %s", offset, length, replicaUrl.c_str() );
    return gOFS.Emsg( "ReplicaWrite", *mError, EREMOTEIO, "write remote replica - write failed", replicaUrl.c_str() );
  }
...
ReplicaLayout::truncate( XrdSfsFileOffset offset )
{
  ...
  if ( rc1 < 0 ) {
    ...
    return gOFS.Emsg( "ReplicaTruncate", *mError, errno, "truncate local replica", replicaUrl.c_str() );
  }

  if ( !rc2 ) {
    eos_err( "Failed to truncate local replica - %llu %s", offset, replicaUrl.c_str() );
    return gOFS.Emsg( "Repl}icaTruncate", *mError, EREMOTEIO, "truncate remote replica", replicaUrl.c_str() );
  }
...
ReplicaLayout::sync()
{
  ...
  if ( rc1 < 0 ) {
    ...
    return gOFS.Emsg( "ReplicaSync", *mError, errno, "sync local replica", replicaUrl.c_str() );
  }

  if ( !rc2 ) {
    eos_err( "Failed to sync local replica - %s", replicaUrl.c_str() );
    return gOFS.Emsg( "Repl}icaSync", *mError, EREMOTEIO, "sync remote replica", replicaUrl.c_str() );
  }
...
ReplicaLayout::close()
{
  ...
  if ( rc1 < 0 ) {
    ...
    return gOFS.Emsg( "ReplicaClose", *mError, errno, "close local replica", replicaUrl.c_str() );
  }

  if ( !rc2 ) {
    eos_err( "Failed to close local replica - %s", replicaUrl.c_str() );
    return gOFS.Emsg( "ReplicaClose", *mError, EREMOTEIO, "close remote replica", replicaUrl.c_str() );
  }
