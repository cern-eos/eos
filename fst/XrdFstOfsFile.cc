  ...
  if (opaqueCheckSum != "ignore") {
    ...
    if (!opaqueCheckSum.empty() && (val = mOpenOpaque->Get("mgm.checksumtypereq"))) {
      ...
      if (!requestedCksum) {
        ...
        mChecksumErr =
            "the checksum type requested does not exist: " + opaqueCheckSumTypeReq;
        return gOFS.Emsg(epname, error, EINVAL,
                         (std::string("open - ") + *mChecksumErr).c_str());
      }
      if (!mCheckSum) {
        // Add the case where a user requested a checksum but the filesystem layout does
        // not have any checksum configured
        mChecksumErr = "the filesystem layout does not have checksum configured, "
                       "requested checksum was: " +
                       opaqueCheckSumTypeReq;
        return gOFS.Emsg(epname, error, EINVAL,
                         (std::string("open - ") + *mChecksumErr).c_str());
      }
      // The checksum was found, ensure it matches the one that was retrieved from the
      // layout ID
      if (requestedCksum && mCheckSum &&
          strcmp(requestedCksum->GetName(), mCheckSum->GetName()) != 0) {
        // We don't want to copy a file with a checksum that is different from the EOS
        // configured one
        std::ostringstream cksumErr;
        cksumErr << "the checksum type requested (" << val
                 << ") does not match the one supported by the filesystem layout ("
                 << mCheckSum->GetName() << ")";
    ...
    if (mCheckSum) {
      eos_debug("msg=\"will apply checksum\" xs_ptr=%p lid=%u cksum_type=\"%s\" "
                "mgm.checksum=\"%s\" mgm.checksumtypereq=\"%s\"",
                mCheckSum.get(), mLid, mCheckSum->GetName(), opaqueCheckSum.c_str(),
                opaqueCheckSumTypeReq.c_str());
    }
    // Hand the checksum object over to the checksum group - the group is what
    // the read/write path uses to compute, store and verify the checksum
    ...
    if (mIsRW) {
      ...
      if (mOpenOpaque->Get("mgm.checksum")) {
        ...
        if ((opaqueChecksum != "disable") && (opaqueChecksum != hexChecksum)) {
          ...
          cksumErr << "requested checksum " << opaqueChecksum
                   << " does not match checksum " << hexChecksum << " of uploaded file";
          mChecksumErr = cksumErr.str();
          eos_err("msg=\"%s\"", cksumErr.str().c_str());
          mChecksumGroup->Clear();
          return true;
        }
      }
    ...
    std::string& archive_req_id)
{
  ...
  std::string protowfusegrpctlskey;
  {
    ...
    protowfusegrpctlskey = gConfif.protowfeusegrpctlskey;
  }
  ...
  try {
    ...
    RequestSenderConfig cf(protowfusegrpc, endPoint, resource, root_certs, tokenPath, protowfusegrpctls);
