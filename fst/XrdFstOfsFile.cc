  ...
  mTpcCancel(false)
{
  ...
  rStart.tv_sec = wStart.tv_sec = rvStart.tv_sec = rTime.tv_sec = lrTime.tv_sec = rvTime.tv_sec = lrvTime.tv_sec = 0;
  rStart.tv_usec = wStart.tv_usec = rvStart.tv_usec = rTime.tv_usec = lrTime.tv_usec = rvTime.tv_usec = lrvTime.tv_usec = 0;
...
XrdFstOfsFile::AddLayoutReadTime()
{
  ...
  timeToRead += ((nowtime.tv_sec - rStart.tv_sec)*1000) + ((nowtime.tv_usec - rStart.tv_usec)/1000.0);
}
...
XrdFstOfsFile::AddLayoutReadVTime()
{
  struct timeval nowtime;
  gettimeofday(&nowtime, &tz);
  timeToReadV += ((nowtime.tv_sec - rvStart.tv_sec)*1000) + ((nowtime.tv_usec - rvStart.tv_usec)/1000.0);
}
...
XrdFstOfsFile::AddLayoutWriteTime()
{
  struct timeval nowtime;
  gettimeofday(&nowtime, &tz);
  timeToWrite += ((nowtime.tv_sec - wStart.tv_sec)*1000) + ((nowtime.tv_usec - wStart.tv_usec)/1000.0);
}
...
XrdFstOfsFile::MakeReportEnv(XrdOucString& reportString)
{
  ...
  bool ioprio_default = false;
  {
    ...
    float iot = (float) ( ((closeTime.tv_sec - openTime.tv_sec)*1000.0) + ((closeTime.tv_usec - openTime.tv_usec)/1000.0));
    ...
    float idt = iot - timeToOpen-timeToClose-timeToRead-timeToReadV-timeToWrite;
