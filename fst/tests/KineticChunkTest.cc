...
#include "fst/io/KineticSingletonCluster.hh"
...
SCENARIO("Chunk integration test.", "[Chunk]"){
  ...
  GIVEN ("An empty chunk."){
    ...
    THEN("Illegal writes to the chunk fail."){
      ...
      REQUIRE(c.write(buf, cluster->limits().max_value_size+1, 1) == EINVAL);
    }
    ...
    WHEN("Something is written to the chunk."){
      char in[] = "0123456789";
      REQUIRE(c.write(in, 0, sizeof(in)) == 0);

      THEN("It can be read again from memory."){
        char out[sizeof(in)];
        REQUIRE(c.read(out,0,sizeof(out)) == 0);
