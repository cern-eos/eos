...
public:
  ...
  FstFileTest() : mLogger(FstTestsEnv::verbose) {}

  virtual void SetUp() override
  {
    mEnv = new TestEnv(FstTestsEnv::instanceName);
  }
...
TEST_F(FstFileTest, WriteTest)
{
  ...
  XRootDStatus status = mFile->Open(file_url, OpenFlags::Update | OpenFlags::Delete,
                                    Access::Mode::None);
...
TEST_F(FstFileTest, ReadVTest)
{
  ...
  std::string file_url = address + "/" + file_path;
  GLOG << "Opening file: " << file_url << std::endl;
  XRootDStatus status = mFile->Open(file_url, OpenFlags::Read, Access::Mode::None);
  ASSERT_TRUE(status.IsOK());
  ...
  ASSERT_TRUE(stat->TestFlags(StatInfo::IsReadable)) ;
  delete stat;
...
//------------------------------------------------------------------------------
TEST_F(FstFileTest, SplitReadVTest)
{
...
//----------------------------------------------------------------------------
TEST_F(FstFileTest, DeleteFlagTest)
{
  using namespace XrdCl;
  std::unique_ptr<File> mFile(new File());
  ...
  std::string file_url = address + "/" + file_path;
  GLOG << "Opening file: " << file_url << std::endl;
  XRootDStatus status = mFile->Open(file_url, OpenFlags::Delete | OpenFlags::Update,
...
// Read async test
//------------------------------------------------------------------------------
TEST_F(FstFileTest, ReadAsyncTest)
{
  // Initialize
  std::string address = "root://root@" + mEnv->GetMapping("server");
  std::string file_path = mEnv->GetMapping("plain_file");
...
// Read async test
//------------------------------------------------------------------------------
TEST_F(FstFileTest, ReadAsyncTestRA)
{
  // Initialize
  std::string address = "root://root@" + mEnv->GetMapping("server");
  std::string file_path = mEnv->GetMapping("plain_file");
