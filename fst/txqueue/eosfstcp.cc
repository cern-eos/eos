...
MD5_CTX md5ctx;
const char* protocols[] = {"file","xroot","xroot",NULL};
...
void usage() {
  ...
  fprintf(stderr, "       -t <mb/s>    : reduce the traffic to an everage of <mb/s> mb/s\n");
...
int main(int argc, char* argv[]) {
  ...
  while (1) {
    ...
    if (computemd5) {
      MD5_Update(&md5ctx, buffer, nread);
    }
  ...
  if (dosymlink) {
    ...
    if (appendmode) {
      sprintf(rangedestname,"%s %llu:%llu",destination[0],(unsigned long long)startwritebyte,(unsigned long long)stopwritebyte);
    } else {
