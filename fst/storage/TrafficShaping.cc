...
uint64_t
IoDelayConfig::CalculateReservationDelay(const uint64_t delay_us, const uint64_t bytes,
                                         const bool is_write, uint64_t& remainder) const
{
  ...
  uint64_t denominator = kIoDelayReferenceBytes;
  __uint128_t numerator =
      static_cast<__uint128_t>(delay_us) * bytes * kIoDelayParallelReservationFactor;

  if (!is_write) {
    numerator *= kReadDelaySafetyNumerator;
    denominator *= kReadDelaySafetyDenominator;
  }

  numerator += remainder;

  constexpr uint64_t kMaxReservationDelayUs =
      kMaxScaledIoDelayUs * kIoDelayParallelReservationFactor;
  const __uint128_t max_numerator =
      static_cast<__uint128_t>(kMaxReservationDelayUs) * denominator;

  if (numerator >= max_numerator) {
    remainder = 0;
    return kMaxReservationDelayUs;
  }

  const uint64_t reservation_delay_us = static_cast<uint64_t>(numerator / denominator);
  remainder = static_cast<uint64_t>(numerator % denominator);
  return reservation_delay_us;
}
                                        ...
                                        const uint64_t bytes, const bool is_write)
{
...

  {
    ...
    for (const auto& entry : entries) {
      ...
      const uint64_t reservation_delay_us = CalculateReservationDelay(
          entry.delay_us, bytes, is_write, reservation.delay_remainder);
      reservation.last_used = now;

      if (reservation_delay_us == 0) {
        continue;
      }
