...
Storage::ImportScan()
{
  ...
  while (1) {
    ...
    capOpaqueFile += "&mgm.import.status.timestamp=";
    capOpaqueFile += std::to_string(time(NULL)).c_str();
    ...
    if (rc) {
      ...
      eos_static_info("ImportScan[id=%s] starting import of dest=%s",
                      scan->id.c_str(), scan->extPath.c_str());
    }

    // Scan the directory found at extPath
    while ((file = io->ftsRead(handle)) != "") {
    ...
    if (rc) {
      ...
      eos_static_info("ImportScan[id=%s] finished successfully",
                      scan->id.c_str());
    }
