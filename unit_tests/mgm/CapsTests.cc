...
TEST_F(CapsTest, GetAllCaps)
{
  ...
  mCaps.Store(make_cap(4, "client4", "auth1"), &vid4);
  EXPECT_EQ(mCaps.ncaps(),
            4); // TimeCaps will still have entry corresponding to unexpiredtimes
  EXPECT_EQ(mCaps.GetCaps().size(), 3);
  {
    auto results = mCaps.GetAllCaps();
    EXPECT_EQ(results.size(), 3);
...
TEST_F(CapsTest, ImplyCapsMulti)
{
  ...
  for (int i = 0; i < limit; i++) {
    ...
    mCaps.Store(make_cap(i, "client" + std::to_string(i),
                         "auth" + std::to_string(i)), &vid);
  }

  std::thread deleter([&]() {
    for (int i = 0; i < limit; i++) {
      EXPECT_EQ(mCaps.Delete(i), 0);
    }
  });
  std::thread implier([&]() {
    for (int i = 0; i < limit; i++) {
      std::string indp = std::to_string(i - 1);
