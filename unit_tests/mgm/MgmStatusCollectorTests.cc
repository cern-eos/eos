...
TEST(MgmStatusCollector, ExposesObservedLeaseHolderWithoutMasterFiltering)
{
  const MgmStatusCollector leader_collector("test-cluster", []() {
    return std::vector<MgmStatusSnapshot>{
        {"mgm-01.example:1094", "mgm-01.example:1094", "5.5.0-test", true}};
  });
  const MgmStatusCollector follower_collector("test-cluster", []() {
    return std::vector<MgmStatusSnapshot>{
        {"mgm-02.example:1094", "mgm-01.example:1094", "5.5.0-test", false}};
  });
  ...
  const auto* leader_info = FindFamily(leader, "eos_mgm_info");
  ASSERT_NE(leader_info, nullptr);
  ASSERT_EQ(leader_info->metric.size(), 1);
  EXPECT_EQ(Labels(leader_info->metric[0]).at("eos_version"), "5.5.0-test");
}
...
TEST(MgmStatusCollector, PreservesRoleAndLeaseDisagreement)
{
  const MgmStatusCollector collector("test-cluster", []() {
    return std::vector<MgmStatusSnapshot>{
        {"mgm-01.example:1094", "mgm-02.example:1094", "5.5.0-test", true}};
  });
...
TEST(MgmStatusCollector, ExposesConfiguredCandidatesAndLocalMgm)
{
  const auto snapshots =
      BuildMgmStatusSnapshots("mgm-02.example:1094", true, "mgm-02.example:1094",
                              "5.5.0-test", 1094, {"mgm-01.example", "mgm-03.example"});
  ...
  const auto* info = FindFamily(families, "eos_mgm_info");
  ASSERT_NE(info, nullptr);
  ASSERT_EQ(info->metric.size(), 1);
  EXPECT_EQ(Labels(info->metric[0]).at("mgm_id"), "mgm-02.example:1094");
}
