...
class TapeRestApiBusinessTestEnvMgr
{
  std::unordered_map<std::string, char*> mEnv;
  std::vector<std::string> mKeys;

public:
  explicit TapeRestApiBusinessTestEnvMgr(std::vector<std::string> keys)
    : mKeys(std::move(keys))
  {
    for (const auto& key : mKeys) {
      mEnv[key] = getenv(key.c_str());
      setenv(key.c_str(), "0", 1);
    }
  }

  ~TapeRestApiBusinessTestEnvMgr()
  {
    for (const auto& key : mKeys) {
      if (auto it = mEnv.find(key); it != mEnv.end()) {
        if (it->second) {
          setenv(key.c_str(), it->second, 1);
        } else {
          unsetenv(key.c_str());
        }
      }
...
class TapeRestApiBusinessTestFakeStats
{
public:
...
class TapeRestApiBusinessTestFakeXrdMgmOfs : public XrdMgmOfs
{
public:
  explicit TapeRestApiBusinessTestFakeXrdMgmOfs(XrdSysError* lp)
    : XrdMgmOfs(lp) {}

  ~TapeRestApiBusinessTestFakeXrdMgmOfs() override
  {
    if (mZmqContext) {
      mZmqContext->close();
      delete mZmqContext;
  ...
  TapeRestApiBusinessTestFakeStats MgmStats;
};
...
public:
  ...
  int prepare(XrdSfsPrep& pargs, XrdOucErrInfo& error,
              const common::VirtualIdentity* vid) noexcept override
  {
    (void)pargs;
    (void)vid;
...
protected:
  ...
  {
    mEnv = new TapeRestApiBusinessTestEnvMgr(
      {"EOS_MGM_HTTP_PORT", "EOS_MGM_GRPC_PORT"});
    mSysErr = new XrdSysError(nullptr, "tape-rest-api-business-test");
    mOfs = new TapeRestApiBusinessTestFakeXrdMgmOfs(mSysErr);
    gOFS = mOfs;
  }
  ...
  void SetUp() override
  {
    ...
    mIssuer.uid = 1000;
    mIssuer.tident = "issuer@localhost";
    mOtherUser.uid = 2000;
    mOtherUser.tident = "other@localhost";
  }
  ...
  static TapeRestApiBusinessTestFakeXrdMgmOfs* mOfs;
  static TapeRestApiBusinessTestEnvMgr* mEnv;
...
TapeRestApiBusinessTestFakeXrdMgmOfs* TapeRestApiBusinessTest::mOfs = nullptr;
TapeRestApiBusinessTestEnvMgr* TapeRestApiBusinessTest::mEnv = nullptr;
